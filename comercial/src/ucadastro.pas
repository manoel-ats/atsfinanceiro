...
uses
  ...
  Dialogs, uPai, DB, Menus,  StdCtrls, Buttons, ExtCtrls, MMJPanel,
  FMTBcd, SqlExpr, DBCtrls, Mask, XPMenu,DBXPress;
...
//    DMSaude.cdsClienteSITUACAOESCOLAR.AsString := DBEdit32.Text;
  ...
  if(DBEdit1.Text <> DMSaude.cdsClienteRAZAOSOCIAL.AsString)then
   ...
   begin
     ...
     str1 := ' UPDATE CLIENTES SET RAZAOSOCIAL = ' + QuotedStr(DBEdit1.Text) +
             ...
             ' and GRUPO_CLIENTE = ' + QuotedStr('PARTICULAR');
   ...
   //  str1 := str1 + ')';
     ...
     try
       ...
       MessageDlg('Cadastro Alterado ,'+#13+#10+'  , '+#13+#10+'', mtWarning, [mbOK], 0);
  ...
  inherited;
   if (DMSaude.cdsEndereco.State in [dsEdit]) then
   begin
      DMSaude.cdsEndereco.ApplyUpdates(0);
   end;

   if (DMSaude.cdsEndereco.State in [dsInsert]) then
   begin
   if dm.c_6_genid.Active then
      dm.c_6_genid.Close;
...
procedure Tfcadastro.dsEnderecoStateChange(Sender: TObject);
begin
 // inherited;
