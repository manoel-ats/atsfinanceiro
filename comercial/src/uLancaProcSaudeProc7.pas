...
type
  TfLancaProcSaudeProc = class(TForm)
    ...
    btn1: TButton;
    sqlN: TSQLDataSet;
    dspN: TDataSetProvider;
    dsN: TClientDataSet;
    sqlNCODVENDA: TIntegerField;
    sqlNCODMOVIMENTO: TIntegerField;
    sqlNCODCLIENTE: TIntegerField;
    sqlNDATAVENDA: TDateField;
    sqlNDATAVENCIMENTO: TDateField;
    sqlNNUMEROBORDERO: TIntegerField;
    sqlNVALOR: TFloatField;
    sqlNNOTAFISCAL: TIntegerField;
    sqlNSERIE: TStringField;
    sqlNDESCONTO: TFloatField;
    sqlNCODCCUSTO: TSmallintField;
    sqlNN_PARCELA: TSmallintField;
    sqlNMULTA_JUROS: TFloatField;
    sqlNENTRADA: TFloatField;
    dsNCODVENDA: TIntegerField;
    dsNCODMOVIMENTO: TIntegerField;
    dsNCODCLIENTE: TIntegerField;
    dsNDATAVENDA: TDateField;
    dsNDATAVENCIMENTO: TDateField;
    dsNNUMEROBORDERO: TIntegerField;
    dsNVALOR: TFloatField;
    dsNNOTAFISCAL: TIntegerField;
    dsNSERIE: TStringField;
    dsNDESCONTO: TFloatField;
    dsNCODCCUSTO: TSmallintField;
    dsNN_PARCELA: TSmallintField;
    dsNMULTA_JUROS: TFloatField;
    dsNENTRADA: TFloatField;
...
begin
  ...
  sql2 := 'SELECT tit, emissao, vence, assocTitular, assocDependente, ' +
    'codConta , conta, hist, vlrRec,  contratado, codTitular, ' +
    'codContratado, codDependente, codrec, codpag, codMovi, codVen, codMovDet, ' +
    'faixaEtaria ,datasistema ,outras_desp , n_boleto FROM LANCAPROCSAUDEPROC(';
 ...
 // TFloatField(cds.Fields[9]).DisplayFormat := ',##0.00';
  JvDBGrid1.Columns[1].FieldName := cds.Fields[0].FieldName;  // tit
  JvDBGrid1.Columns[2].FieldName := cds.Fields[1].FieldName;  // emissao
  JvDBGrid1.Columns[3].FieldName := cds.Fields[2].FieldName;  // vence
  JvDBGrid1.Columns[4].FieldName := cds.Fields[3].FieldName;  // assocTitular
  JvDBGrid1.Columns[5].FieldName := cds.Fields[4].FieldName;  // assocDependente
  JvDBGrid1.Columns[6].FieldName := cds.Fields[5].FieldName;  // codConta
  JvDBGrid1.Columns[7].FieldName := cds.Fields[6].FieldName;  // conta
  JvDBGrid1.Columns[8].FieldName := cds.Fields[8].FieldName;  // vlrRec
 // JvDBGrid1.Columns[9].FieldName := cds.Fields[9].FieldName;  // vlrPagto
  JvDBGrid1.Columns[9].FieldName := cds.Fields[7].FieldName; // hist
 // JvDBGrid1.Columns[11].FieldName := cds.Fields[21].FieldName;// n_boleto
  JvDBGrid1.Columns[10].FieldName := cds.Fields[19].FieldName;// datasistema
  JvDBGrid1.Columns[11].FieldName := cds.Fields[20].FieldName;// outras_desp

  {  0 tit,1 emissao, 2 vence, 3 assocTitular,4 assocDependente,
     5 codConta ,6 conta,7 hist,8 vlrRec,9 vlrPagto, 10 contratado, 11 codTitular,
     12 codContratado, 13 codDependente, 14 codrec, 15 codpag,16 codMovi,17 codVen,18 codMovDet,
     19 faixaEtaria ,20 n_boleto ,21 datasistema ,22 outras_desp
  }


  Label15.Caption := IntToStr(cds.RecordCount);
  JvCalcEdit3.Text :=  sSoma.Fields[0].Text;
...
begin
  ...
  begin
    ...
    begin
      ...
      fLancaProcSaude.Edit2.Text    := cds.Fields[21].asString;
      fLancaProcSaude.data_recibo.Date :=  cds.Fields[19].AsDateTime;
      fLancaProcSaude.JvCalcEdit3.Value := cds.Fields[20].AsFloat;
...
begin
  ...
  TFloatField(cds.Fields[8]).DisplayFormat := ',##0.00';
  TFloatField(cds.Fields[9]).DisplayFormat := ',##0.00';
  TFloatField(cds.Fields[13]).DisplayFormat := ',##0.00';
...
function TfLancaProcSaudeProc.Selecionado(Codigos: TStringList;  Codigo: String): Integer;
var
  x : integer;
begin
...
procedure TfLancaProcSaudeProc.FormClose(Sender: TObject;
...
begin
  ...
  str := ' INSERT INTO RECEBIMENTO (CODRECEBIMENTO, TITULO, CODCLIENTE ,DATAVENCIMENTO, STATUS ,VIA ,FORMARECEBIMENTO, ' +
         ' VALOR_PRIM_VIA, VALOR_RESTO, VALORTITULO )'+
  ' VALUES (' +
         IntToStr(newrec) + ',' +  //  codrecebimento
         IntToStr(cds.Fields[11].AsInteger) + ','+  //  titulo
         IntToStr(cds.Fields[11].AsInteger) + ','+  //  codtitular
         QuotedStr(FormatDateTime('mm/dd/yyyy',dtnewrec.Date)) + ','+ // venc
         QuotedStr('5-') + ','+
         QuotedStr('1') + ','+
         QuotedStr('1') + ','+
         FloatToStr(cds.Fields[8].AsFloat) + ','+
         FloatToStr(cds.Fields[8].AsFloat) + ','+
         FloatToStr(cds.Fields[8].AsFloat) +
         ')';
  ...
  DecimalSeparator := ',';
 {  tit, emissao, vence, assocTitular, assocDependente, ' +
    'codConta , conta, hist, vlrRec, vlrPagto, contratado, codTitular, ' +
    'codContratado, codDependente, codrec, codpag, codMovi, codVen, codMovDet, ' +
    'faixaEtaria , n_boleto ,datasistema ,outras_desp }
