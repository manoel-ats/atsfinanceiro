...
uses
  ...
  rpvclreport, jpeg, UCBase, ActnList, Grids, DBGrids;
...
type
  TfLaboratorio = class(TForm)
    ...
    UserLab: TUserControl;
    Label3: TLabel;
    btnVer: TdxButton;
    btc: TBitBtn;
    btContratado: TdxButton;
    ExamesValores1: TMenuItem;
    DBGrid1: TDBGrid;
    dsE: TDataSource;
    cdsE: TClientDataSet;
    dspE: TDataSetProvider;
    sqlE: TSQLDataSet;
    sqlECODCLIENTE: TIntegerField;
    sqlEDATASOLICITACAO: TDateField;
    sqlERAZAOSOCIAL: TStringField;
    cdsECODCLIENTE: TIntegerField;
    cdsEDATASOLICITACAO: TDateField;
    cdsERAZAOSOCIAL: TStringField;
    btnatu: TBitBtn;
    sqlENUMERO: TStringField;
    cdsENUMERO: TStringField;
    sqlEIMPRESSO: TStringField;
    sqlEINCLUIU: TStringField;
    cdsEIMPRESSO: TStringField;
    cdsEINCLUIU: TStringField;
    ...
    procedure btContratadoClick(Sender: TObject);
    procedure ExamesValores1Click(Sender: TObject);
...
begin
  ...
  try
   ...
   fcadastro.Label8.Visible := True;
   fcadastro.DBEdit6.Visible := True;
  ...
  // fcadastro.DBEdit3.Visible := false;
   fcadastro.Label14.Caption := 'CPF';
   fcadastro.Label13.Caption := 'RG';
...
begin
  fLabColetaMateriais := TfLabColetaMateriais.Create(Application);
  try
    fLabColetaMateriais.ShowModal;
  finally
    fLabColetaMateriais.Free;
  end     
...
begin
 if DMSaude.cdsCombo.Active then
    DMSaude.cdsCombo.Close;
     ...
     DMSaude.cdsCombo.Open;
  fComboLab:=TfComboLab.Create(Application);
  uso := 'EXAMES';
  fComboLab.Label1.Caption := 'EXAMES';
  fComboLab.Label2.Caption := 'EXAMES';
  FComboLab.Label5.Caption := 'R$ : PCMSO';
  fComboLab.DBGrid1.Columns[2].Title.Caption :='PCMSO';
  FComboLab.Label8.Caption := 'R$ : PARTICULAR';
  fComboLab.DBGrid1.Columns[3].Title.Caption :='PARTICULAR';
  fComboLab.DBGrid1.Columns[4].Title.Caption :='1-Sim 2-não';
  fComboLab.MMJPanel2.Background.StartColor := clMaroon ;
  fComboLab.MMJPanel1.Background.EndColor := clMaroon ;
  //fCombo.DBGrid1.Columns[3].Visible := false;
//  fCombo.Label6.Visible := false;
 // fCombo.DBEdit4.Visible := false;
...
begin
  ...
  try
    ...
    fLabExameLocaliza.btnImp.Visible := True;
    fLabExameLocaliza.btnCon.Visible := True;
    fLabExameLocaliza.btnana.Visible := True;
    fLabExameLocaliza.btnDev.Visible := True;
    fLabExameLocaliza.btnImpRD.Visible := True;
    //fLabExameLocaliza.btnImp30.Visible := True;
    fLabExameLocaliza.hoje.Visible := True;
    fLabExameLocaliza.Label6.Caption := '.';
    fLabExameLocaliza.Label9.Caption := '.';
...
procedure TfLaboratorio.SpeedButton1Click(Sender: TObject);
begin
  UserLab.Logoff;
end;

procedure TfLaboratorio.Sair1Click(Sender: TObject);
begin
  close;
end;

procedure TfLaboratorio.btnVerClick(Sender: TObject);
begin
 if DMSaude.cdsCombo.Active then
    DMSaude.cdsCombo.Close;
     DMSaude.cdsCombo.Params[0].AsString := 'EXAMES';
  ...
  fComboLab.MMJPanel2.Background.StartColor := clMaroon ;
  fComboLab.MMJPanel1.Background.EndColor := clMaroon ;
  fComboLab.btnProcurar.Visible := False;
  fComboLab.btnIncluir.Visible := False;
  fComboLab.btnExcluir.Visible := False;
  fComboLab.btnGravar.Visible := False;
  fComboLab.btnGravar.Visible := False;
  fComboLab.MMJPanel1.Visible := False;
  fComboLab.DBEdit1.ReadOnly := True;
  fComboLab.DBEdit2.ReadOnly := True;
  fComboLab.DBEdit3.ReadOnly := True;
  fComboLab.DBEdit5.ReadOnly := True;
  fComboLab.DBEdit5.ReadOnly := True;
  fComboLab.DBGrid1.ReadOnly := True;
  fComboLab.Label7.Visible := False;
  fComboLab.DBEdit4.Visible := False;
  fComboLab.DBRadioGroup1.Visible := False;
...
procedure TfLaboratorio.btContratadoClick(Sender: TObject);
begin
...
procedure TfLaboratorio.ExamesValores1Click(Sender: TObject);
begin
  VCLReport1.FileName := str_relatorio + 'exames_valores.rep';
  VCLReport1.Title := str_relatorio + 'exames_valores.rep';
  VCLReport1.Report.DatabaseInfo.Items[0].SQLConnection := dm.sqlsisAdimin;
  VCLReport1.Execute;
end;

procedure TfLaboratorio.DBGrid1DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
begin
  ShowScrollBar(TDBGrid(Sender).Handle,SB_VERT,False); //Remove barra Vertical 
end;

procedure TfLaboratorio.btnatuClick(Sender: TObject);
begin
