...
uses
  ...
  JvFormPlacement, JvAppStorage, JvAppXMLStorage, JvBaseEdits, DBLocal,
  DBLocalS;
...
type
  TfLancaProcSaudeProc = class(TForm)
    ...
    BitBtn12: TBitBtn;
    BitBtn13: TBitBtn;
    dsAviso: TClientDataSet;
    sAviso: TSQLDataSet;
    dAviso: TDataSetProvider;
    sAvisoCODCLI: TIntegerField;
    dsAvisoCODCLI: TIntegerField;
...
begin
  ...
  sql2 := 'SELECT tit, emissao, vence, assocTitular, assocDependente, ' +
    ...
    'faixaEtaria , n_boleto FROM LANCAPROCSAUDEPROC(';
  ...
  lbl1.Caption := IntToStr(cds.RecordCount);
...
begin
  codsoc := 0;
  if(dsAviso.Active)then
  dsAviso.Close;
  dsAviso.Open;

  while not dsAviso.Eof do
  begin

    codsoc :=  dsAvisoCODCLI.Value;
