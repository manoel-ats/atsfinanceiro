...
uses
  ...
  dxCore, dxButton, JvExDBGrids, JvDBGrid;
...
begin
  ...
  tabnf.TabVisible := False; // nota de compra ainda não está em uso
  usaLote := 'N';
  jaCarregados := 'NAO';
end;
...
begin
    ...
    begin
     { if (dmnf.cds_nf.Active) then
      ...
      dmnf.cds_nf.Params[1].AsInteger := dmnf.cdsCompraCODCOMPRA.asInteger;
      dmnf.cds_nf.Open;
...
begin
  ...
  begin
    ...
    diferenca := 0;
    DMNF.cdsCompraCODFORNECEDOR.AsInteger := DMNF.cds_MovimentoCODCLIENTE.AsInteger;
    ...
    if (dmnf.cdsCompraNOTAFISCAL.IsNull) then
      dmnf.cdsCompraNOTAFISCAL.AsInteger := StrToInt(formatdatetime('yyyy',dmnf.cdsCompraDATACOMPRA.AsDateTime) +
     (IntToStr(dmnf.cdsCompraCODCOMPRA.AsInteger)));
