...
begin
  ...
  try
    fAudReceber.dataAu.Text      := fcrProcSaude.meDta1.Text;
    fAudReceber.MaskEdit1.Text   := fcrProcSaude.meDta2.Text;
    fAudReceber.edCodCCusto.Text := fcrProcSaude.edCodCCusto.Text;
  ...
  //==============================================================================
{
 ...
 //     SqlCr := SqlCr + ' AND rec.VALORTITULO <> 0 ';
  ...
  //------------------------------------------------------------------------------
  if (cbGeraAviso.Text = 'BOLETO COM REGISTRO') then
  begin
    if SqlCr='' then
      SqlCr := SqlCr + ' WHERE GERADEBITOAU = '
    else
      SqlCr := SqlCr + ' and ((geradebitoau is null)or(geradebitoau =' + QuotedStr('')+ '))';
...
procedure TfcrProcSaude.Button1Click(Sender: TObject);
VAR sqltexto1 :String ;
begin
  if cds.Active then
    cds.Close;
  cds.Params[0].AsInteger := 431808;
  cds.Open;
  if cds.IsEmpty then
  begin
    cds.Close;
