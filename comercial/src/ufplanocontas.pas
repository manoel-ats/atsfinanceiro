...
uses
  ...
  rpcompobase, rpvclreport, JvExControls, JvLabel ,DBXpress;
...
begin
   ...
   sqlR  := ' insert into produtos (CODPRODUTO, PRODUTO , CONTA_DESPESA , CODPRO ,TIPO,CONTA_ESTOQUE)' +
            ' select plano.codreduzido,  plano.nome, plano.conta, plano.codreduzido,'
            + QuotedStr('DESP') + ', ' + QuotedStr(DBEdit1.Text) +
            ' from plano where plano.contapai = ' + QuotedStr(DBEdit2.Text) +
            ' and plano.codigo = ' + QuotedStr(dm.c_1_planocCODIGO.AsString);
...
{   sqlR  := ' INSERT INTO VALOR_NA_VENDAD (RA , NOMECLIENTE ,VALOR, NUMEROBORDERO )'+
 ...
 {
   dm.c_1_planocCONTA.AsString
  insert into produtos (CODPRODUTO, PRODUTO , CONTA_DESPESA , CODPRO ,TIPO,CONTA_ESTOQUE)
select
    plano.codreduzido,  plano.nome, plano.conta, plano.codreduzido, 'DESP',    DBEdit1

from plano where plano.contapai = DBEdit1   and plano.codigo = dm.c_1_planocCODIGO.AsInteger
}
end;
