object fRelatorioCaixa: TfRelatorioCaixa
  Left = 436
  Top = 137
  Width = 559
  Height = 427
  Caption = 'Relat'#243'rio Caixa'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object MMJPanel3: TMMJPanel
    Left = 0
    Top = 0
    Width = 543
    Height = 389
    Align = alClient
    TabOrder = 0
    Silhuette.Shape.ShapeText = 'Shape text'
    Silhuette.PictureShape.Picture.Data = {07544269746D617000000000}
    Silhuette.PictureShape.DisplayPicture = True
    Background.StartColor = clSilver
    Background.EndColor = clTeal
    Background.FillType = GradUpDown
    object Label4: TLabel
      Left = 152
      Top = 16
      Width = 129
      Height = 20
      Caption = 'Relat'#243'rio de Caixa'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object Label5: TLabel
      Left = 208
      Top = 58
      Width = 8
      Height = 16
      Caption = #224
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object Label6: TLabel
      Left = 24
      Top = 61
      Width = 70
      Height = 16
      Caption = 'Per'#237'odo de:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object Label1: TLabel
      Left = 24
      Top = 92
      Width = 83
      Height = 16
      Caption = 'Caixa / Banco'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object Label2: TLabel
      Left = 25
      Top = 120
      Width = 86
      Height = 16
      Caption = 'Forma Receb.'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object Label11: TLabel
      Left = 24
      Top = 156
      Width = 95
      Height = 16
      Caption = 'Centro de Custo'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object Label3: TLabel
      Left = 336
      Top = 311
      Width = 103
      Height = 13
      Caption = 'Valor Fechamento CX'
      FocusControl = DBEdit1
      Transparent = True
    end
    object Bevel1: TBevel
      Left = 4
      Top = 302
      Width = 531
      Height = 82
      Style = bsRaised
    end
    object BitBtn2: TBitBtn
      Left = 413
      Top = 225
      Width = 89
      Height = 59
      Caption = '&Fechar'
      TabOrder = 5
      OnClick = BitBtn2Click
      Glyph.Data = {
        
        BFBFBFBFBFBFBFBFBF00}
      Layout = blGlyphTop
      Margin = 0
    end
    object MaskEdit2: TMaskEdit
      Left = 221
      Top = 58
      Width = 77
      Height = 21
      BevelKind = bkFlat
      BorderStyle = bsNone
      EditMask = '!99/99/00;1;_'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      MaxLength = 8
      ParentFont = False
      TabOrder = 1
      Text = '  /  /  '
      OnKeyPress = MaskEdit1KeyPress
    end
    object MaskEdit1: TMaskEdit
      Left = 128
      Top = 58
      Width = 79
      Height = 21
      BevelKind = bkFlat
      BorderStyle = bsNone
      EditMask = '!99/99/00;1;_'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      MaxLength = 8
      ParentFont = False
      TabOrder = 0
      Text = '  /  /  '
      OnKeyPress = MaskEdit1KeyPress
    end
    object ComboBox1: TComboBox
      Left = 126
      Top = 88
      Width = 264
      Height = 24
      BevelKind = bkFlat
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ItemHeight = 16
      ParentFont = False
      TabOrder = 2
      OnChange = ComboBox1Change
      OnKeyPress = MaskEdit1KeyPress
    end
    object ComboBox2: TComboBox
      Left = 126
      Top = 120
      Width = 264
      Height = 24
      BevelKind = bkFlat
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ItemHeight = 16
      ParentFont = False
      TabOrder = 3
      OnKeyPress = MaskEdit1KeyPress
    end
    object edCodCCusto: TComboBox
      Left = 126
      Top = 152
      Width = 264
      Height = 24
      BevelKind = bkFlat
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ItemHeight = 16
      ParentFont = False
      TabOrder = 4
      OnKeyPress = MaskEdit1KeyPress
    end
    object BitBtn3: TBitBtn
      Left = 40
      Top = 225
      Width = 89
      Height = 59
      Hint = 
        'Preecha o Per'#237'odo, escolha o Caixa, opcional (Forma e Centro de ' +
        'Custo)'
      Caption = 'Total Caixa'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 6
      OnClick = BitBtn3Click
      Glyph.Data = {
        
        C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
      Layout = blGlyphTop
      Margin = 0
    end
    object BitBtn4: TBitBtn
      Left = 133
      Top = 225
      Width = 89
      Height = 59
      Hint = 'Preecha o Per'#237'odo e escolha o CAIXA.'
      Caption = 'Mov. Detalhado'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 7
      OnClick = BitBtn4Click
      Glyph.Data = {
        
        C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
      Layout = blGlyphTop
      Margin = 0
    end
    object BitBtn5: TBitBtn
      Left = 227
      Top = 225
      Width = 89
      Height = 59
      Hint = 'Preecha o Per'#237'odo e escolha o CAIXA.'
      Caption = 'Caixa Sintetico'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 8
      OnClick = BitBtn5Click
      Glyph.Data = {
        
        C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
      Layout = blGlyphTop
      Margin = 0
    end
    object RadioGroup1: TRadioGroup
      Left = 127
      Top = 178
      Width = 262
      Height = 44
      Caption = 'Tipo de Data do Caixa Analitico'
      Columns = 2
      ItemIndex = 0
      Items.Strings = (
        'Data Pag/Rec.'
        'Data Consolida')
      TabOrder = 9
    end
    object BitBtn1: TBitBtn
      Left = 320
      Top = 225
      Width = 89
      Height = 59
      Hint = 'Preecha o Per'#237'odo e escolha o CAIXA.'
      Caption = 'Caixa Analitico'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 10
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
      Layout = blGlyphTop
      Margin = 0
    end
    object DBEdit1: TDBEdit
      Left = 336
      Top = 328
      Width = 101
      Height = 21
      DataField = 'UDF_ROUNDDEC'
      DataSource = DataSource1
      ReadOnly = True
      TabOrder = 11
    end
    object btnFechar: TBitBtn
      Left = 216
      Top = 328
      Width = 75
      Height = 25
      Caption = 'Fechar CX'
      Enabled = False
      TabOrder = 12
      OnClick = btnFecharClick
    end
    object BitBtn6: TBitBtn
      Left = 40
      Top = 352
      Width = 107
      Height = 25
      Caption = 'Dados do Contador'
      TabOrder = 13
      OnClick = BitBtn6Click
    end
  end
  object VCLReport1: TVCLReport
    AsyncExecution = False
    Title = 'Untitled'
    Left = 64
    Top = 224
  end
  object JvAppXMLFileStorage1: TJvAppXMLFileStorage
    StorageOptions.BooleanStringTrueValues = 'TRUE, YES, Y'
    StorageOptions.BooleanStringFalseValues = 'FALSE, NO, N'
    StorageOptions.InvalidCharReplacement = '_'
    FileName = 'RelatorioCaixa.xml'
    RootNodeName = 'Configuration'
    SubStorages = <>
    Left = 328
    Top = 24
  end
  object JvFormStorage1: TJvFormStorage
    AppStorage = JvAppXMLFileStorage1
    AppStoragePath = 'fRelatorioCaixa\'
    StoredProps.Strings = (
      'ComboBox1.Text'
      'ComboBox2.Text'
      'MaskEdit1.Text'
      'MaskEdit2.Text'
      'edCodCCusto.Text')
    StoredValues = <>
    Left = 360
    Top = 24
  end
  object cds: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftDate
        Name = 'dtaini'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'dtafim'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'cod_caixa'
        ParamType = ptInput
      end>
    ProviderName = 'dsp'
    Left = 128
    Top = 304
    object cdsUDF_ROUNDDEC: TFloatField
      FieldName = 'UDF_ROUNDDEC'
      ReadOnly = True
      DisplayFormat = ',##0.00'
      EditFormat = ',##0.00'
    end
  end
  object dsp: TDataSetProvider
    DataSet = sql
    Options = [poAllowCommandText]
    Left = 80
    Top = 304
  end
  object sql: TSQLDataSet
    CommandText = 
      'select  udf_rounddec(valor,2) from SP_mov_caixa_ordem (:dtaini,:' +
      'dtafim,:cod_caixa)'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftDate
        Name = 'dtaini'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'dtafim'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'cod_caixa'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Left = 32
    Top = 304
    object sqlUDF_ROUNDDEC: TFloatField
      FieldName = 'UDF_ROUNDDEC'
      ReadOnly = True
    end
  end
  object DataSource1: TDataSource
    DataSet = cds
    Left = 408
    Top = 136
  end
  object sqlIn: TSQLDataSet
    CommandText = 
      'select  DTAPAGTO , ORDEM, DESCRICAO,udf_rounddec(VALORC,2) as VA' +
      'LORC, udf_rounddec(VALORD,2) as VALORD , udf_rounddec(valor,2) a' +
      's VALOR ,CONTACONTABIL , '#13#10'CAIXA , CODCONTA, FORMA ,N_DOC, ORDEN' +
      'A , COMPENSADO  '#13#10'from SP_mov_caixa_ordem (:dtaini,:dtafim,:cod_' +
      'caixa)'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftDate
        Name = 'dtaini'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'dtafim'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'cod_caixa'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Left = 168
    Top = 352
    object sqlInDTAPAGTO: TDateField
      FieldName = 'DTAPAGTO'
    end
    object sqlInORDEM: TSmallintField
      FieldName = 'ORDEM'
    end
    object sqlInDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 300
    end
    object sqlInVALORC: TFloatField
      FieldName = 'VALORC'
    end
    object sqlInVALORD: TFloatField
      FieldName = 'VALORD'
    end
    object sqlInVALOR: TFloatField
      FieldName = 'VALOR'
    end
    object sqlInCONTACONTABIL: TStringField
      FieldName = 'CONTACONTABIL'
      Size = 200
    end
    object sqlInCAIXA: TStringField
      FieldName = 'CAIXA'
      Size = 60
    end
    object sqlInCODCONTA: TStringField
      FieldName = 'CODCONTA'
    end
    object sqlInFORMA: TStringField
      FieldName = 'FORMA'
    end
    object sqlInN_DOC: TStringField
      FieldName = 'N_DOC'
    end
    object sqlInORDENA: TSmallintField
      FieldName = 'ORDENA'
    end
    object sqlInCOMPENSADO: TStringField
      FieldName = 'COMPENSADO'
      Size = 10
    end
  end
  object dspIn: TDataSetProvider
    DataSet = sqlIn
    Options = [poAllowCommandText]
    Left = 216
    Top = 352
  end
  object cdsIn: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftDate
        Name = 'dtaini'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'dtafim'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'cod_caixa'
        ParamType = ptInput
      end>
    ProviderName = 'dspIn'
    Left = 264
    Top = 352
    object cdsInDTAPAGTO: TDateField
      FieldName = 'DTAPAGTO'
    end
    object cdsInORDEM: TSmallintField
      FieldName = 'ORDEM'
    end
    object cdsInDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 300
    end
    object cdsInVALORC: TFloatField
      FieldName = 'VALORC'
    end
    object cdsInVALORD: TFloatField
      FieldName = 'VALORD'
    end
    object cdsInVALOR: TFloatField
      FieldName = 'VALOR'
    end
    object cdsInCONTACONTABIL: TStringField
      FieldName = 'CONTACONTABIL'
      Size = 200
    end
    object cdsInCAIXA: TStringField
      FieldName = 'CAIXA'
      Size = 60
    end
    object cdsInCODCONTA: TStringField
      FieldName = 'CODCONTA'
    end
    object cdsInFORMA: TStringField
      FieldName = 'FORMA'
    end
    object cdsInN_DOC: TStringField
      FieldName = 'N_DOC'
    end
    object cdsInORDENA: TSmallintField
      FieldName = 'ORDENA'
    end
    object cdsInCOMPENSADO: TStringField
      FieldName = 'COMPENSADO'
      Size = 10
    end
  end
end
