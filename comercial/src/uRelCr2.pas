...
var data1,data2,situacao,tipo , centro :TRpLabel;
begin
  sqlGrupoCR := '';
  ...
  data1:= TRpLabel(fcrProcSaude.repContasReceber.Report.FindComponent('TRpLabel2'));
  data2:= TRpLabel(fcrProcSaude.repContasReceber.Report.FindComponent('TRpLabel3'));
  situacao:= TRpLabel(fcrProcSaude.repContasReceber.Report.FindComponent('TRpLabel12'));
  tipo:= TRpLabel(fcrProcSaude.repContasReceber.Report.FindComponent('TRpLabel16'));
  centro:= TRpLabel(fcrProcSaude.repContasReceber.Report.FindComponent('TRpLabel18'));

   // Situacao
  if((fcrProcSaude.cbStatus.Text <> '')or(fcrProcSaude.cbStatus.Text = '') ) then
  begin
    situacao.Text := fcrProcSaude.cbStatus.Text;
  end;
  if(fcrProcSaude.cbStatus.Text = '1-A Vencer')  then
  situacao.Text := ' Situação = A Vencer';
  if(fcrProcSaude.cbStatus.Text = '2-Vencido')  then
  situacao.Text := ' Situação = Vencido';
   if(fcrProcSaude.cbStatus.Text = '5-A Receber')  then
  situacao.Text := ' Situação = A Receber';
  if(fcrProcSaude.cbStatus.Text = '7-Recebido')  then
  situacao.Text := ' Situação = Recebido';
  if(fcrProcSaude.cbStatus.Text = '8-Crédito Duvidoso')  then
  situacao.Text := ' Situação = Crédito Duvidoso';
  if(fcrProcSaude.cbStatus.Text = '9-Em Cobrança')  then
  situacao.Text := ' Situação = Em Cobrança';
  if(fcrProcSaude.cbStatus.Text = '10-Novo Título')  then
  situacao.Text := ' Situação = Novo Título';
  if(fcrProcSaude.cbStatus.Text = '11-Todos')  then
  situacao.Text := ' Situação = Todos';
  if(fcrProcSaude.cbStatus.Text = '3-Em Protesto')  then
  situacao.Text := ' Situação = Em Protesto';
  if(fcrProcSaude.cbStatus.Text = '14-Cancelado')  then
  situacao.Text := ' Situação = Cancelado';


  //recebimento
  if (fcrProcSaude.meDta5.text <> '  /  /  ') then
  begin
    data1.Text := fcrProcSaude.meDta5.Text;
    data2.Text := fcrProcSaude.meDta6.Text;
    fcrProcSaude.ComboBox1.Text := 'Data de Recebimento';
  end;

    // Centro custo
  if((fcrProcSaude.edCodCCusto.Text <> '')or(fcrProcSaude.edCodCCusto.Text = '') ) then
  begin
    centro.Text := fcrProcSaude.edCodCCusto.Text;
  end;
  ...
  begin
   ...
   fcrProcSaude.ComboBox1.Text := 'Data de Vencimento';
  end;

 // Emissão
  if (fcrProcSaude.meDta1.text <> '  /  /  ') then
  begin
    data1.Text := fcrProcSaude.meDta1.Text;
    data2.Text := fcrProcSaude.meDta2.Text;
    fcrProcSaude.ComboBox1.Text := 'Data de Emissão';
  end;

  // Tipo de data
  if((fcrProcSaude.ComboBox1.Text <> '')or(fcrProcSaude.ComboBox1.Text = '')) then
  begin
    tipo.Text := fcrProcSaude.ComboBox1.Text;
  end;

  fcrProcSaude.repContasReceber.Title := fcrProcSaude.repContasReceber.FileName;
  fcrProcSaude.repContasReceber.Report.DataInfo.Items[0].SQL:= sqlGrupoCR;
  fcrProcSaude.repContasReceber.Execute;
  fcrProcSaude.ComboBox1.Text := '';
  data1.Text:= '';
  data2.Text:= '';
  situacao.Text:= '';
  tipo.Text:= '';
