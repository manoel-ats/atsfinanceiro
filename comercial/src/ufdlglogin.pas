...
uses
  ...
  Provider, DB, SqlExpr, DBxpress, EOneInst;
...
//  edtLogin.SetFocus;
  BitBtnOK.Click;
  BitBtnCancelar.Click; //para deletar a tabela LOG_ACESSO 
  close;
end;
...
begin
 ...
 // [loCaseInsensitive]
  ...
  begin
    ...
    if (1=1) then   // comentei o if abaixo pois nao e usado mais.
    // if (dmApp.Encripta(edtSenha.Text) = dm.cds_Usuario.FieldByName('senha'). AsString) then
    begin
     intTeste := 1;
     ...
     begin
       ...
       usulog := 1;//dm.cds_UsuarioCODUSUARIO.AsInteger;
       perfil := 'administrador'; //dm.cds_UsuarioPERFIL.AsString;
       usuario := 'adm';//dm.cds_UsuarioLOGIN.AsString;
       varusuario := 'adm';//dm.cds_UsuarioLOGIN.AsString;
