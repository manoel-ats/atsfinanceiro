...
begin
  ...
  {     dm.sqlsisAdimin.StartTransaction(TD);
       dm.sqlsisAdimin.ExecuteDirect('ALTER TRIGGER PROIBE_ALTERACAO_REC INACTIVE');
       ...
       dm.sqlsisAdimin.ExecuteDirect('ALTER TRIGGER PROIBE_ALTERACAO_REC ACTIVE');
       dm.sqlsisAdimin.Commit(TD);
  }
end;
...
begin
 ...
 if((Edit1.Text = 'RENATA')or(Edit1.Text = 'ANA PAULA')or (Edit1.Text = 'elis') or (Edit1.Text = 'adm'))then

 begin
  aut := 1 ;
 end;

 if((Edit1.Text = 'RENATA')or(Edit1.Text = 'ANA PAULA'))then
 begin
 ...
 if(aut = 0 ) then
 begin
   MessageDlg('O usuario não tem Autorização.', mtError, [mbOK], 0);
   exit;
...
begin
  ...
  Try
    ...
    begin
      ...
      str_sql := str_sql + ', USERID = ' + IntToStr(fFinanceiro.UserControlFinanceiro.CurrentUser.UserID);
...
var
str_sql: String;
codigo, i, num , aut : integer;
begin
         /////////////////////////
 aut := 0;

 if(dm.cds_ccustoCODIGO.AsInteger = 52) then
 begin
 if((Edit1.Text = 'ANA PAULA') or (Edit1.Text = 'RENATA') or (Edit1.Text = 'adm'))then
 begin
  aut := 1 ;
  ...
  Try
    ...
    for i:=1 to length(nrec) do
    begin
      str_sql := 'UPDATE RECEBIMENTO SET DP = 1';
      str_sql := str_sql + ', USERID = ' + IntToStr(fFinanceiro.UserControlFinanceiro.CurrentUser.UserID);
      str_sql := str_sql + ' WHERE CODRECEBIMENTO = ';
      num := nrec[i - 1];
      str_sql := str_sql + IntToStr(num);
 ...
 if(dm.cds_ccustoCODIGO.AsInteger = 51) then
 begin

 if((Edit1.Text = 'RENATA') or (Edit1.Text = 'elis') or (Edit1.Text = 'adm'))  then
 begin
 aut := 1 ;
  codigo := dm.cds_crCODRECEBIMENTO.AsInteger;
  if  MessageDlg('Confirma a exclusão da baixa do Título : ''' + dm.cds_crTITULO.AsSTring + '''',
 ...
 if(dm.cds_ccustoCODIGO.AsInteger = 53) then
 begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        dm.cds_cr.Edit;
        dm.cds_crJUROS.AsFloat := dm.cds_crVALOR_RESTO.AsFloat * ((juros * (((dbEdit2.Date - dm.cds_crDATAVENCIMENTO.AsDateTime))/30))/100);
        dm.cds_crJUROS.AsFloat := dm.cds_crJUROS.AsFloat ;
      end;
      if ((dbEdit2.Date - dm.cds_crDATAVENCIMENTO.AsDateTime) >= carencia) then
      begin
        mu := 2;
        multa := (dm.cds_crVALOR_RESTO.AsFloat *(mu/100)) ;
        if (dm.cds_cr.State in [dsBrowse]) then 
        dm.cds_cr.Edit;
        dm.cds_crJUROS.AsFloat := dm.cds_crVALOR_RESTO.AsFloat * ((juros * (((dbEdit2.Date - dm.cds_crDATAVENCIMENTO.AsDateTime))/30))/100);
...
begin
  ...
  Try
    ...
    str_sql := str_sql + ' and CODRECEBIMENTO = ' + IntToStr(codigo) ;
...
procedure TfcrTitulo.btnCancelarClick(Sender: TObject);
var  aut  : integer;
begin
 // inherited;
  ...
  if(dm.cds_ccustoCODIGO.AsInteger = 52) then
  begin
    if((Edit1.Text = 'ANA PAULA')or (Edit1.Text = 'RENATA'))then
    begin
      aut := 1 ;
      inherited;
    end;
