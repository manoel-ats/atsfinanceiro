...
//  JvCalcEdit2.Text := FloatToStr(cdsVALOR_PRAZO.AsFloat);
  ...
  if(dbcodFuncionario.Text = '') then
  begin
   MessageDlg('Escolha um Funcionario!!!!!', mtError, [mbOK], 0);
   exit;
  end;
  ...
  begin
    try
      ...
      //str1 := str1 + '0';
      str1 := str1 + ',';
      str1 := str1 + QuotedStr(memo1.Text); // lOTE
      str1 := str1 + ',';
      ...
      str1 := str1 + ',';
      str1 := str1 + 'null';// DATA RECIBO usado no ASH

      str1 := str1 + ',';
      str1 := str1 + 'null';// DATA ENTRDA REEMBOLSO usado no ASH

      // valor recibo Associado    ASH

      str1 := str1 + ',';
      str1 := str1 + 'null';

      str1 := str1 + ',';
      str1 := str1 + QuotedStr(JvDBSearchComboBox1.Text); // Historico
      ...
      Try
         ...
         MessageDlg('Registro gravado com sucesso.', mtConfirmation,[mbOk], 0);
         label16.Enabled := False;
         JvDBSearchComboBox1.Text := '';
         JvDBSearchComboBox1.Enabled := False;
      except

        dm.sqlsisAdimin.Rollback(TD); {on failure, undo the changes};
        MessageDlg('Erro no sistema, registro não alterado?', mtError,[mbOk], 0);
      end;
    ...
    try
      ...
      // Valor , Produto , Funcionario , Fornecedor , Historico
...
begin
  ...
  ccusto := 'PCMSO';
  sqltexto := 'select CODCLIENTE, NOMECLIENTE, RAZAOSOCIAL, RA , TEM_IE , DATACADASTRO from CLIENTES where status = 0 and GRUPO_CLIENTE = ';
...
begin
  ...
  codMov := 0;
   ...
   if (cdsexame.Active) then
    cdsexame.close;
  cdsexame.Params[0].AsString := 'PREVISAO';
  cdsexame.Open;
...
begin
  ...
  if (JvDBSearchEdit3.Text = '103') then
  begin
   label16.Enabled := True;
   label17.Enabled := True;
   JvDBSearchComboBox1.Enabled := True;
...
begin
  inherited;
  JvCalcEdit2.Text := FloatToStr(cdsVALOR_PRAZO.AsFloat);
   if((JvDBSearchEdit3.Text = '103') and(JvDBSearchComboBox1.Text = '')) then
   begin
  //   MessageDlg('Escolha um Exame !!!!!!!!!!!!!', mtWarning, [mbOK], 0);
     label16.Enabled := True;
     JvDBSearchComboBox1.Enabled := True;
...
procedure TfLancaProcPcmso.JvDBSearchComboBox1Exit(Sender: TObject);
begin
 // inherited;
