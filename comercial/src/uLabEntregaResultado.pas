...
uses
  ...
  StdCtrls, JvExStdCtrls, JvCombobox, JvDBSearchComboBox, Menus, 
  DB, Buttons, MMJPanel, FMTBcd, DBClient, Provider, SqlExpr, Mask,
  JvExMask, JvToolEdit, JvMaskEdit, JvCheckedMaskEdit, JvDatePickerEdit,
  rpcompobase, rpvclreport, DBXpress;
...
begin
 ...
 //     ' and lab.DATAENTREGAMATERIAL = ' + QuotedStr(FormatDateTime('mm/dd/yyyy',dt1.Date)) +
  ...
  sqltexto := '';
  cds.CommandText := '';
  sqltexto := 'select lab.* ,cli.NOMECLIENTE , cli.SEXO  from LABEXAME lab left outer join CLIENTES cli on cli.CODCLIENTE = lab.CODCLIENTE ';
  if (RadioGroup1.ItemIndex = 1) then
      sqltexto := sqltexto + 'where lab.SITUACAO <> ' + QuotedStr('FINALIZADO') +
      ' and lab.SITUACAO <> ' + QuotedStr('COLETADO') +
      ' and lab.DATAENTREGAMATERIAL = ' + QuotedStr(FormatDateTime('mm/dd/yyyy',dt1.Date)) +
 ...
 // begin
   ...
   if (cdsSITUACAO.AsString = 'LAUDADO') then
    ...
    try
        ...
        dm.sqlsisAdimin.ExecuteDirect('Update LABEXAME set SITUACAO = '+ QuotedStr('FINALIZADO') +
        ', DATARETIRADA = ' +  QuotedStr(FormatDateTime('mm/dd/yyyy', dataentrega.Date)) +
        ' where CODEXAME = ' + QuotedStr(cdsCODEXAME.AsString));
...
begin
  ...
  if(cdsSEXO.AsString = '')then
  begin
    MessageDlg('Falta Cadastrar "Sexo" no Cadastro !!'+#13+#10+'         Confira o Cadastro ', mtError, [mbOK], 0);
    exit;
  end;
  ...
  begin
   { if (JvDBGrid1.SelectedRows.CurrentRowSelected = true) then
    begin
        ...
        if ((cdsTIPOEXAME.AsString = 'HEMOGRAMA') and (cdsSEXO.AsString = 'M')) then
        begin
          VCLReport1.FileName := str_relatorio + 'ash_examesHH.rep';
          VCLReport1.Report.DatabaseInfo.Items[0].SQLConnection := dm.sqlsisAdimin;
          VCLReport1.Report.Params.ParamByName('HEMOGRAMA').Value := cdsCODEXAME.AsInteger;
          VCLReport1.Title := VCLReport1.Filename;
          VCLReport1.Execute;
        ...
        if ((cdsTIPOEXAME.AsString = 'HEMOGRAMA') and (cdsSEXO.AsString = 'F')) then
        begin
          VCLReport1.FileName := str_relatorio + 'ash_examesHM.rep';
          VCLReport1.Report.DatabaseInfo.Items[0].SQLConnection := dm.sqlsisAdimin;
          VCLReport1.Report.Params.ParamByName('HEMOGRAMA').Value := cdsCODEXAME.AsInteger;
          VCLReport1.Title := VCLReport1.Filename;
          VCLReport1.Execute;
        ...
        if (cdsTIPOEXAME.AsString = 'PROTO')then
        begin
         // VCLReport1.FileName := str_relatorio + 'ash_examesP.rep';
        //  VCLReport1.Report.DatabaseInfo.Items[0].SQLConnection := dm.sqlsisAdimin;
        // VCLReport1.Report.Params.ParamByName('PROTO').Value := cdsCODEXAME.AsInteger;
        //  VCLReport1.Title := VCLReport1.Filename;
        //  VCLReport1.Execute;
...
begin
  ...
  try
    ...
    BEGIN
      ...
      dataentrega.Text := DMSaude.cdsExameDATARETIRADA.AsString;
