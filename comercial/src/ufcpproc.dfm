object fCpProc: TfCpProc
  Left = 274
  Top = 134
  BorderStyle = bsSingle
  Caption = 'Filtro Contas a Pagar'
  ClientHeight = 526
  ClientWidth = 792
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  PopupMenu = PopupMenu1
  Position = poDesktopCenter
  WindowState = wsMaximized
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object MMJPanel1: TMMJPanel
    Left = 0
    Top = 0
    Width = 792
    Height = 60
    Align = alTop
    BevelInner = bvLowered
    PopupMenu = PopupMenu1
    TabOrder = 0
    Silhuette.Shape.ShapeText = 'Shape text'
    Silhuette.PictureShape.Picture.Data = {07544269746D617000000000}
    Silhuette.PictureShape.DisplayPicture = True
    Background.StartColor = clSilver
    Background.EndColor = clTeal
    Background.FillType = GradUpDown
    object Label16: TLabel
      Left = 761
      Top = -1
      Width = 29
      Height = 13
      Caption = 'Forma'
      Transparent = True
    end
    object btnPagar: TBitBtn
      Left = 4
      Top = 2
      Width = 54
      Height = 55
      Caption = 'PagarII'
      PopupMenu = PopupMenu1
      TabOrder = 22
      OnClick = btnPagarClick
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
    end
    object btnGravar: TBitBtn
      Left = 317
      Top = -88
      Width = 77
      Height = 55
      Caption = 'Gravar(F4)'
      TabOrder = 4
      Visible = False
      Glyph.Data = {
        
        BFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBF}
      Layout = blGlyphTop
    end
    object btnIncluir: TBitBtn
      Left = 4
      Top = 2
      Width = 54
      Height = 55
      Caption = 'F2-Pagar'
      PopupMenu = PopupMenu1
      TabOrder = 1
      OnClick = btnIncluirClick
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
    end
    object btnCancelar: TBitBtn
      Left = 59
      Top = 2
      Width = 61
      Height = 55
      Caption = 'F3-Cancelar'
      PopupMenu = PopupMenu1
      TabOrder = 5
      Glyph.Data = {
        
        C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
      Layout = blGlyphTop
    end
    object btnExcluir: TBitBtn
      Left = 404
      Top = -89
      Width = 77
      Height = 55
      Caption = 'Excluir(F7)'
      Enabled = False
      TabOrder = 2
      Visible = False
      Glyph.Data = {
        
        0000}
      Layout = blGlyphTop
    end
    object btnProcurar: TBitBtn
      Left = 126
      Top = 2
      Width = 64
      Height = 55
      Caption = 'F8-Procurar'
      PopupMenu = PopupMenu1
      TabOrder = 0
      OnClick = btnProcurarClick
      Glyph.Data = {
        
        C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
      Layout = blGlyphTop
    end
    object btnSair: TBitBtn
      Left = 513
      Top = 2
      Width = 47
      Height = 55
      Caption = 'F9-Sair'
      PopupMenu = PopupMenu1
      TabOrder = 3
      OnClick = btnSairClick
      Glyph.Data = {
        
        C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
      Layout = blGlyphTop
    end
    object Panel2: TPanel
      Left = 121
      Top = 2
      Width = 4
      Height = 56
      BevelInner = bvRaised
      Color = clTeal
      PopupMenu = PopupMenu1
      TabOrder = 6
    end
    object Panel1: TPanel
      Left = 369
      Top = 2
      Width = 4
      Height = 56
      BevelInner = bvRaised
      Color = clTeal
      PopupMenu = PopupMenu1
      TabOrder = 7
    end
    object btnImprimir: TBitBtn
      Left = 191
      Top = 2
      Width = 48
      Height = 42
      Caption = '&Imprimir'
      PopupMenu = PopupMenu1
      TabOrder = 8
      OnClick = btnImprimirClick
      Glyph.Data = {
        
        C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
      Layout = blGlyphTop
    end
    object Panel4: TPanel
      Left = 240
      Top = 2
      Width = 4
      Height = 56
      BevelInner = bvRaised
      Color = clTeal
      PopupMenu = PopupMenu1
      TabOrder = 9
    end
    object BitBtn9: TBitBtn
      Left = 449
      Top = 2
      Width = 62
      Height = 55
      Caption = 'F4-Confirma'
      Enabled = False
      PopupMenu = PopupMenu1
      TabOrder = 10
      OnClick = BitBtn9Click
      Glyph.Data = {
        
        3F3F3F3F3F3F3F3F3F3F3F3F3F3F3F3F3F3F3F3F3F37373F3F3F}
      Layout = blGlyphTop
    end
    object rbTipoConta: TJvRadioGroup
      Left = 561
      Top = 2
      Width = 200
      Height = 31
      Caption = 'Tipo de Conta'
      Columns = 3
      ItemIndex = 0
      Items.Strings = (
        'Despesas'
        'Compras'
        'Todas')
      TabOrder = 11
      CaptionVisible = True
    end
    object BitBtn5: TBitBtn
      Left = 253
      Top = 2
      Width = 12
      Height = 55
      Caption = '(SIP-PROC)'
      PopupMenu = PopupMenu1
      TabOrder = 12
      Visible = False
      OnClick = BitBtn5Click
      Glyph.Data = {
        
        C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
      Layout = blGlyphTop
    end
    object BitBtn13: TBitBtn
      Left = 244
      Top = 2
      Width = 58
      Height = 55
      Caption = '(SIP-FORN)'
      PopupMenu = PopupMenu1
      TabOrder = 13
      OnClick = BitBtn13Click
      Glyph.Data = {
        
        C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
      Layout = blGlyphTop
    end
    object BitBtn14: TBitBtn
      Left = 303
      Top = 2
      Width = 65
      Height = 55
      Caption = '(SIP-GERAL)'
      PopupMenu = PopupMenu1
      TabOrder = 14
      OnClick = BitBtn14Click
      Glyph.Data = {
        
        C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
      Layout = blGlyphTop
    end
    object BtnCh: TBitBtn
      Left = 561
      Top = 35
      Width = 77
      Height = 22
      Caption = 'N'#186' Ch/Conta/ Data'
      TabOrder = 15
      OnClick = BtnChClick
    end
    object JvDateEdit2: TJvDatePickerEdit
      Left = 725
      Top = 35
      Width = 62
      Height = 21
      AllowNoDate = True
      Checked = True
      DateFormat = 'dd/MM/yy'
      DateSeparator = '/'
      StoreDateFormat = True
      TabOrder = 18
    end
    object numero: TEdit
      Left = 637
      Top = 35
      Width = 49
      Height = 22
      BevelKind = bkFlat
      BorderStyle = bsNone
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      PopupMenu = PopupMenu1
      TabOrder = 16
      OnKeyPress = cbStatusKeyPress
    end
    object BitBtn16: TBitBtn
      Left = 373
      Top = 3
      Width = 36
      Height = 54
      Caption = 'Proc.'
      TabOrder = 20
      OnClick = BitBtn16Click
    end
    object BitBtn18: TBitBtn
      Left = 411
      Top = 3
      Width = 37
      Height = 54
      Caption = 'Imp.'
      TabOrder = 21
      OnClick = BitBtn18Click
    end
    object conta: TEdit
      Left = 686
      Top = 35
      Width = 40
      Height = 22
      BevelKind = bkFlat
      BorderStyle = bsNone
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      PopupMenu = PopupMenu1
      TabOrder = 17
      OnKeyPress = cbStatusKeyPress
    end
    object edForma: TEdit
      Left = 760
      Top = 11
      Width = 25
      Height = 21
      TabOrder = 19
    end
    object BitBtn19: TBitBtn
      Left = 374
      Top = 3
      Width = 36
      Height = 54
      Caption = 'Proc.'
      TabOrder = 23
      OnClick = BitBtn19Click
    end
    object BitBtn20: TBitBtn
      Left = 411
      Top = 3
      Width = 37
      Height = 54
      Caption = 'Imp.'
      TabOrder = 24
      OnClick = BitBtn20Click
    end
    object BitBtn21: TBitBtn
      Left = 190
      Top = 45
      Width = 48
      Height = 13
      Caption = 'exportar'
      TabOrder = 25
      OnClick = BitBtn21Click
    end
  end
  object MMJPanel2: TMMJPanel
    Left = 0
    Top = 491
    Width = 792
    Height = 35
    Align = alBottom
    BevelInner = bvLowered
    PopupMenu = PopupMenu1
    TabOrder = 1
    Silhuette.Shape.ShapeText = 'Shape text'
    Silhuette.PictureShape.Picture.Data = {07544269746D617000000000}
    Silhuette.PictureShape.DisplayPicture = True
    Background.StartColor = clSilver
    Background.EndColor = clTeal
    Background.FillType = GradUpDown
    object Label14: TLabel
      Left = 237
      Top = 10
      Width = 55
      Height = 13
      Caption = 'Total T'#237'tulo'
      Transparent = True
    end
    object Label15: TLabel
      Left = 421
      Top = 10
      Width = 73
      Height = 13
      Caption = 'Total Pendente'
      Transparent = True
    end
    object Label17: TLabel
      Left = 631
      Top = 10
      Width = 52
      Height = 13
      Caption = 'Total Pago'
      Transparent = True
    end
    object Label18: TLabel
      Left = 8
      Top = 18
      Width = 122
      Height = 13
      Caption = 'Ctrl+D = Desmarca Todos'
      Transparent = True
      Visible = False
    end
    object Label21: TLabel
      Left = 8
      Top = 4
      Width = 103
      Height = 13
      Caption = 'Ctrl+A = Marca Todos'
      Transparent = True
      Visible = False
    end
    object lbl3: TLabel
      Left = 140
      Top = 3
      Width = 77
      Height = 13
      Caption = 'Total Registros :'
    end
    object Label22: TLabel
      Left = 157
      Top = 19
      Width = 12
      Height = 13
      Caption = '....'
    end
    object BitBtn10: TBitBtn
      Left = 317
      Top = -88
      Width = 77
      Height = 55
      Caption = 'Gravar(F4)'
      TabOrder = 1
      Visible = False
      Glyph.Data = {
        
        BFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBF}
      Layout = blGlyphTop
    end
    object BitBtn15: TBitBtn
      Left = 404
      Top = -89
      Width = 77
      Height = 55
      Caption = 'Excluir(F7)'
      Enabled = False
      TabOrder = 0
      Visible = False
      Glyph.Data = {
        
        0000}
      Layout = blGlyphTop
    end
    object JvCalcEdit1: TJvCalcEdit
      Left = 300
      Top = 7
      Width = 97
      Height = 21
      BevelKind = bkFlat
      Flat = False
      ParentFlat = False
      BorderStyle = bsNone
      ButtonFlat = True
      DisplayFormat = ',0.00'
      TabOrder = 2
      DecimalPlacesAlwaysShown = False
    end
    object JvCalcEdit3: TJvCalcEdit
      Left = 692
      Top = 7
      Width = 97
      Height = 21
      BevelKind = bkFlat
      Flat = False
      ParentFlat = False
      BorderStyle = bsNone
      ButtonFlat = True
      DisplayFormat = ',0.00'
      TabOrder = 3
      DecimalPlacesAlwaysShown = False
    end
    object JvCalcEdit2: TJvCalcEdit
      Left = 505
      Top = 7
      Width = 100
      Height = 21
      BevelKind = bkFlat
      Flat = False
      ParentFlat = False
      BorderStyle = bsNone
      ButtonFlat = True
      DisplayFormat = ',0.00'
      TabOrder = 4
      DecimalPlacesAlwaysShown = False
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 60
    Width = 792
    Height = 132
    Align = alTop
    TabOrder = 2
    object Label3: TLabel
      Left = 3
      Top = 13
      Width = 42
      Height = 13
      Caption = 'Situa'#231#227'o'
    end
    object Label5: TLabel
      Left = 185
      Top = 5
      Width = 28
      Height = 13
      Caption = 'T'#237'tulo'
    end
    object Label9: TLabel
      Left = 167
      Top = 47
      Width = 33
      Height = 13
      Caption = 'C'#243'digo'
    end
    object Label10: TLabel
      Left = 229
      Top = 47
      Width = 100
      Height = 13
      Caption = 'Nome do Fornecedor'
    end
    object Label11: TLabel
      Left = 377
      Top = 6
      Width = 76
      Height = 13
      Caption = 'Centro de Custo'
    end
    object Label12: TLabel
      Left = 261
      Top = 103
      Width = 51
      Height = 13
      Caption = 'Comprador'
      Visible = False
    end
    object Label13: TLabel
      Left = 286
      Top = 87
      Width = 33
      Height = 13
      Caption = 'C'#243'digo'
      Visible = False
    end
    object Label1: TLabel
      Left = 487
      Top = 6
      Width = 24
      Height = 13
      Caption = 'Valor'
    end
    object Label4: TLabel
      Left = 3
      Top = 87
      Width = 73
      Height = 13
      Caption = 'Conta Despesa'
    end
    object Label23: TLabel
      Left = 3
      Top = 47
      Width = 29
      Height = 13
      Caption = 'Forma'
      PopupMenu = PopupMenu1
    end
    object Label19: TLabel
      Left = 240
      Top = 7
      Width = 38
      Height = 13
      Caption = 'N'#186' Doc.'
    end
    object Label20: TLabel
      Left = 165
      Top = 87
      Width = 26
      Height = 13
      Caption = 'Caixa'
    end
    object Label2: TLabel
      Left = 379
      Top = 85
      Width = 78
      Height = 13
      Caption = 'Data do Sistema'
    end
    object Label24: TLabel
      Left = 407
      Top = 107
      Width = 19
      Height = 16
      Caption = 'at'#233
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object Edit1: TEdit
      Left = 340
      Top = 23
      Width = 99
      Height = 21
      BevelKind = bkFlat
      BorderStyle = bsNone
      TabOrder = 0
      Text = 'Edit1'
      Visible = False
    end
    object cbStatus: TComboBox
      Left = 3
      Top = 27
      Width = 131
      Height = 24
      BevelKind = bkFlat
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ItemHeight = 16
      ParentFont = False
      PopupMenu = PopupMenu1
      TabOrder = 1
      OnKeyPress = cbStatusKeyPress
    end
    object BitBtn4: TBitBtn
      Left = 138
      Top = 26
      Width = 26
      Height = 26
      PopupMenu = PopupMenu1
      TabOrder = 2
      TabStop = False
      OnClick = BitBtn4Click
      Glyph.Data = {
        
        0000}
    end
    object edTitulo: TEdit
      Left = 182
      Top = 21
      Width = 52
      Height = 24
      BevelKind = bkFlat
      BorderStyle = bsNone
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      PopupMenu = PopupMenu1
      TabOrder = 3
      OnKeyPress = cbStatusKeyPress
    end
    object edCodCliente: TEdit
      Left = 165
      Top = 62
      Width = 62
      Height = 24
      BevelKind = bkFlat
      BorderStyle = bsNone
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      PopupMenu = PopupMenu1
      TabOrder = 4
      OnExit = edCodClienteExit
      OnKeyPress = cbStatusKeyPress
    end
    object edCliente: TEdit
      Left = 226
      Top = 62
      Width = 258
      Height = 24
      TabStop = False
      BevelKind = bkFlat
      BorderStyle = bsNone
      Color = 13562879
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      PopupMenu = PopupMenu1
      ReadOnly = True
      TabOrder = 5
    end
    object BitBtn8: TBitBtn
      Left = 485
      Top = 48
      Width = 42
      Height = 39
      PopupMenu = PopupMenu1
      TabOrder = 6
      TabStop = False
      OnClick = BitBtn8Click
      Glyph.Data = {
        
        0707070707070707070707070707070707070707070707070707}
    end
    object edCodCCusto: TComboBox
      Left = 314
      Top = 21
      Width = 168
      Height = 24
      BevelKind = bkFlat
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ItemHeight = 16
      ParentFont = False
      PopupMenu = PopupMenu1
      TabOrder = 7
      OnKeyPress = cbStatusKeyPress
    end
    object edVendedor: TEdit
      Left = 195
      Top = 94
      Width = 121
      Height = 23
      TabStop = False
      BevelKind = bkFlat
      BorderStyle = bsNone
      Color = 13562879
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      PopupMenu = PopupMenu1
      ReadOnly = True
      TabOrder = 8
      Visible = False
    end
    object edcodVendedor: TEdit
      Left = 254
      Top = 86
      Width = 45
      Height = 23
      BevelKind = bkFlat
      BorderStyle = bsNone
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      PopupMenu = PopupMenu1
      TabOrder = 9
      Visible = False
      OnKeyPress = cbStatusKeyPress
    end
    object BitBtn12: TBitBtn
      Left = 253
      Top = 89
      Width = 42
      Height = 39
      PopupMenu = PopupMenu1
      TabOrder = 10
      TabStop = False
      Visible = False
      OnClick = BitBtn12Click
      Glyph.Data = {
        
        C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
    end
    object BitBtn11: TBitBtn
      Left = 527
      Top = 89
      Width = 42
      Height = 39
      PopupMenu = PopupMenu1
      TabOrder = 11
      TabStop = False
      OnClick = BitBtn11Click
      Glyph.Data = {
        
        0000}
    end
    object GroupBox1: TGroupBox
      Left = 572
      Top = 30
      Width = 220
      Height = 34
      Caption = 'Per'#237'odo de Emiss'#227'o'
      PopupMenu = PopupMenu1
      TabOrder = 12
      object Label6: TLabel
        Left = 78
        Top = 14
        Width = 19
        Height = 16
        Caption = 'at'#233
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object BitBtn2: TBitBtn
        Left = 190
        Top = 9
        Width = 27
        Height = 20
        PopupMenu = PopupMenu1
        TabOrder = 2
        TabStop = False
        OnClick = BitBtn2Click
        Glyph.Data = {
          
          0000}
      end
      object meDta1: TJvDateEdit
        Left = 9
        Top = 11
        Width = 59
        Height = 21
        ShowButton = False
        TabOrder = 0
        OnKeyPress = meDta1xKeyPress
      end
      object meDta2: TJvDateEdit
        Left = 122
        Top = 10
        Width = 59
        Height = 21
        ShowButton = False
        TabOrder = 1
        OnKeyPress = meDta1xKeyPress
      end
    end
    object GroupBox2: TGroupBox
      Left = 572
      Top = 63
      Width = 220
      Height = 34
      Caption = 'Per'#237'odo de Pagamento'
      PopupMenu = PopupMenu1
      TabOrder = 13
      object Label7: TLabel
        Left = 79
        Top = 15
        Width = 19
        Height = 16
        Caption = 'at'#233
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object BitBtn6: TBitBtn
        Left = 188
        Top = 10
        Width = 27
        Height = 19
        PopupMenu = PopupMenu1
        TabOrder = 2
        TabStop = False
        OnClick = BitBtn6Click
        Glyph.Data = {
          
          0000}
      end
      object meDta5: TJvDateEdit
        Left = 9
        Top = 11
        Width = 59
        Height = 21
        ShowButton = False
        TabOrder = 0
        OnKeyPress = meDta1xKeyPress
      end
      object meDta6: TJvDateEdit
        Left = 121
        Top = 10
        Width = 59
        Height = 21
        ShowButton = False
        TabOrder = 1
        OnKeyPress = meDta1xKeyPress
      end
    end
    object GroupBox3: TGroupBox
      Left = 572
      Top = 96
      Width = 220
      Height = 36
      Caption = 'Per'#237'odo de Vencimento'
      PopupMenu = PopupMenu1
      TabOrder = 14
      object Label8: TLabel
        Left = 79
        Top = 16
        Width = 19
        Height = 16
        Caption = 'at'#233
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object BitBtn7: TBitBtn
        Left = 189
        Top = 9
        Width = 28
        Height = 20
        PopupMenu = PopupMenu1
        TabOrder = 2
        TabStop = False
        OnClick = BitBtn7Click
        Glyph.Data = {
          
          0000}
      end
      object meDta3: TJvDateEdit
        Left = 8
        Top = 12
        Width = 59
        Height = 21
        ShowButton = False
        TabOrder = 0
        OnKeyPress = meDta1xKeyPress
      end
      object meDta4: TJvDateEdit
        Left = 120
        Top = 11
        Width = 59
        Height = 21
        ShowButton = False
        TabOrder = 1
        OnKeyPress = meDta1xKeyPress
      end
    end
    object BitBtn1: TBitBtn
      Left = 527
      Top = 48
      Width = 42
      Height = 39
      PopupMenu = PopupMenu1
      TabOrder = 15
      TabStop = False
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        0000}
    end
    object cbConta: TJvComboBox
      Left = 3
      Top = 102
      Width = 159
      Height = 24
      CharCase = ecUpperCase
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ItemHeight = 16
      ParentFont = False
      TabOrder = 16
      OnKeyPress = cbStatusKeyPress
    end
    object cbForma: TComboBox
      Left = 3
      Top = 62
      Width = 158
      Height = 24
      BevelKind = bkFlat
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ItemHeight = 16
      ParentFont = False
      PopupMenu = PopupMenu1
      TabOrder = 17
      OnKeyPress = cbStatusKeyPress
    end
    object edValor: TEdit
      Left = 486
      Top = 21
      Width = 64
      Height = 21
      TabOrder = 18
      OnKeyPress = edValorKeyPress
    end
    object BitBtn3: TBitBtn
      Left = 554
      Top = 21
      Width = 18
      Height = 19
      PopupMenu = PopupMenu1
      TabOrder = 19
      TabStop = False
      OnClick = BitBtn3Click
      Glyph.Data = {
        
        0000}
    end
    object edNumeroDocumento: TEdit
      Left = 239
      Top = 21
      Width = 73
      Height = 24
      BevelKind = bkFlat
      BorderStyle = bsNone
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      PopupMenu = PopupMenu1
      TabOrder = 20
      OnKeyPress = cbStatusKeyPress
    end
    object cbTipoFornecedor: TJvComboBox
      Left = 164
      Top = 102
      Width = 152
      Height = 24
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ItemHeight = 16
      ParentFont = False
      TabOrder = 21
      OnKeyPress = cbStatusKeyPress
    end
    object CheckBox1: TCheckBox
      Left = 277
      Top = 4
      Width = 54
      Height = 17
      Caption = 'sem N'#186
      TabOrder = 22
    end
    object CheckBox2: TCheckBox
      Left = 333
      Top = 4
      Width = 54
      Height = 17
      Caption = 'com N'#186
      TabOrder = 23
    end
    object grp1: TGroupBox
      Left = 576
      Top = -4
      Width = 220
      Height = 36
      Caption = 'Per'#237'odo de Entrada'
      PopupMenu = PopupMenu1
      TabOrder = 24
      object lbl1: TLabel
        Left = 74
        Top = 16
        Width = 19
        Height = 16
        Caption = 'at'#233
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object medt1: TJvDateEdit
        Left = 5
        Top = 12
        Width = 59
        Height = 21
        ShowButton = False
        TabOrder = 0
        OnKeyPress = meDta1xKeyPress
      end
      object medt2: TJvDateEdit
        Left = 118
        Top = 11
        Width = 59
        Height = 21
        ShowButton = False
        TabOrder = 1
        OnKeyPress = meDta1xKeyPress
      end
    end
    object ComboBox1: TComboBox
      Left = -5
      Top = 116
      Width = 158
      Height = 24
      BevelKind = bkFlat
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ItemHeight = 16
      ParentFont = False
      PopupMenu = PopupMenu1
      TabOrder = 25
      Visible = False
      OnKeyPress = cbStatusKeyPress
      Items.Strings = (
        'Entrada'
        'Emiss'#227'o')
    end
    object dataAu: TMaskEdit
      Left = 97
      Top = 3
      Width = 65
      Height = 17
      BevelKind = bkFlat
      BorderStyle = bsNone
      EditMask = '!99/99/00;1;_'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      MaxLength = 8
      ParentFont = False
      PopupMenu = PopupMenu1
      TabOrder = 26
      Text = '  /  /  '
      Visible = False
      OnKeyPress = meDta1xKeyPress
    end
    object btnAu: TBitBtn
      Left = 100
      Top = 2
      Width = 43
      Height = 17
      Hint = 'Escolher Um dia a Mais ao Dia a ser Conferido'
      Caption = 'Conferir'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 27
      Visible = False
      OnClick = btnAuClick
    end
    object btnC: TBitBtn
      Left = 32
      Top = 0
      Width = 89
      Height = 25
      Caption = 'Auditoria'
      TabOrder = 28
      TabStop = False
      OnClick = btnCClick
    end
    object MaskEdit1: TJvDateEdit
      Left = 337
      Top = 103
      Width = 65
      Height = 21
      ShowButton = False
      TabOrder = 29
      OnKeyPress = meDta1xKeyPress
    end
    object MaskEdit2: TJvDateEdit
      Left = 433
      Top = 103
      Width = 65
      Height = 21
      ShowButton = False
      TabOrder = 30
      OnKeyPress = meDta1xKeyPress
    end
    object Edit2: TEdit
      Left = 8
      Top = 0
      Width = 41
      Height = 21
      TabOrder = 31
      Visible = False
    end
    object ckans: TCheckBox
      Left = 132
      Top = 5
      Width = 40
      Height = 16
      Caption = 'ans'
      TabOrder = 32
    end
  end
  object BitBtn17: TBitBtn
    Left = 762
    Top = 66
    Width = 26
    Height = 21
    PopupMenu = PopupMenu1
    TabOrder = 4
    TabStop = False
    OnClick = BitBtn17Click
    Glyph.Data = {
      
      0000}
  end
  object DBGrid2: TDBGrid
    Left = 9
    Top = 192
    Width = 792
    Height = 300
    DataSource = DataSource2
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
    ParentFont = False
    PopupMenu = PopupMenu1
    TabOrder = 5
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnCellClick = DBGrid2CellClick
    OnColEnter = DBGrid2ColEnter
    OnColExit = DBGrid2ColExit
    OnDrawColumnCell = DBGrid2DrawColumnCell
    OnKeyPress = DBGrid2KeyPress
    OnTitleClick = DBGrid2TitleClick
    Columns = <
      item
        Expanded = False
        FieldName = 'DUP_REC_NF'
        Title.Caption = 'Sel.'
        Width = 26
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'TITULO'
        Title.Caption = 'T'#237'tulo'
        Width = 73
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'VIA'
        Title.Caption = 'Parc.'
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'EMISSAO'
        Title.Caption = 'Data Emiss'#227'o'
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'DATAVENCIMENTO'
        Title.Caption = 'Data Vencto.'
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'DATAPAGAMENTO'
        Title.Caption = 'Data Pagto.'
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'CODFORNECEDOR'
        Title.Caption = 'C'#243'd.'
        Width = 30
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'NOMEFORNECEDOR'
        Title.Caption = 'Fornecedor'
        Width = 133
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'VALOR_RESTO'
        Title.Caption = 'Valor Pagar'
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'VALORREC'
        Title.Caption = 'Pendente'
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'VALORRECEBIDO'
        Title.Caption = 'Pago'
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'STATUS'
        Title.Caption = 'Situa'#231#227'o'
        Width = 74
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'N_DOCUMENTO'
        Title.Caption = 'N.doc'
        Width = 63
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'DATACONSOLIDA'
        Title.Caption = 'Data Consolida'
        Width = 71
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'CAIXA'
        Width = 55
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'FORMAPAGAMENTO'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DATASISTEMA'
        Title.Caption = 'Data Sistema'
        Width = 107
        Visible = True
      end>
  end
  object DBGrid1: TDBGrid
    Left = 0
    Top = 192
    Width = 792
    Height = 297
    DataSource = ds_Cr
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
    ParentFont = False
    PopupMenu = PopupMenu1
    TabOrder = 3
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnCellClick = DBGrid1CellClick
    OnColEnter = DBGrid1ColEnter
    OnColExit = DBGrid1ColExit
    OnDrawColumnCell = DBGrid1DrawColumnCell
    OnKeyPress = DBGrid1KeyPress
    OnTitleClick = DBGrid1TitleClick
    Columns = <
      item
        Expanded = False
        FieldName = 'DUP_REC_NF'
        Title.Caption = 'Sel.'
        Width = 26
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'TITULO'
        Title.Caption = 'T'#237'tulo'
        Width = 73
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'VIA'
        Title.Caption = 'Parc.'
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'EMISSAO'
        Title.Caption = 'Data Emiss'#227'o'
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'DATAVENCIMENTO'
        Title.Caption = 'Data Vencto.'
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'DATAPAGAMENTO'
        Title.Caption = 'Data Pagto.'
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'CODFORNECEDOR'
        Title.Caption = 'C'#243'd.'
        Width = 30
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'NOMEFORNECEDOR'
        Title.Caption = 'Fornecedor'
        Width = 133
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'VALOR_RESTO'
        Title.Caption = 'Valor Pagar'
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'VALORREC'
        Title.Caption = 'Pendente'
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'VALORRECEBIDO'
        Title.Caption = 'Pago'
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'STATUS'
        Title.Caption = 'Situa'#231#227'o'
        Width = 74
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'N_DOCUMENTO'
        Title.Caption = 'N.doc'
        Width = 63
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'DATACONSOLIDA'
        Title.Caption = 'Data Consolida'
        Width = 71
        Visible = True
      end
      item
        Color = 13562879
        Expanded = False
        FieldName = 'CAIXA'
        Width = 55
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'FORMAPAGAMENTO'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'HISTORICO'
        Title.Caption = 'Historico'
        Visible = True
      end>
  end
  object s_1: TSQLDataSet
    CommandText = 
      'select rec.*, cli.NOMECLIENTE, (VALORTITULO - VALOR_RESTO) as VA' +
      'LORREC  from RECEBIMENTO rec '#13#10'inner join CLIENTES cli on cli.CO' +
      'DCLIENTE=rec.CODCLIENTE'
    MaxBlobSize = -1
    Params = <>
    SQLConnection = DM.sqlsisAdimin
    Left = 560
    Top = 408
    object s_1CODRECEBIMENTO: TIntegerField
      FieldName = 'CODRECEBIMENTO'
      Required = True
    end
    object s_1TITULO: TStringField
      FieldName = 'TITULO'
      Required = True
      Size = 18
    end
    object s_1EMISSAO: TDateField
      FieldName = 'EMISSAO'
    end
    object s_1CODCLIENTE: TIntegerField
      FieldName = 'CODCLIENTE'
      Required = True
    end
    object s_1DATAVENCIMENTO: TDateField
      FieldName = 'DATAVENCIMENTO'
    end
    object s_1DATARECEBIMENTO: TDateField
      FieldName = 'DATARECEBIMENTO'
    end
    object s_1CAIXA: TSmallintField
      FieldName = 'CAIXA'
    end
    object s_1CONTADEBITO: TIntegerField
      FieldName = 'CONTADEBITO'
    end
    object s_1CONTACREDITO: TIntegerField
      FieldName = 'CONTACREDITO'
    end
    object s_1STATUS: TStringField
      FieldName = 'STATUS'
      Required = True
      FixedChar = True
      Size = 2
    end
    object s_1VIA: TStringField
      FieldName = 'VIA'
      Required = True
      FixedChar = True
      Size = 1
    end
    object s_1FORMARECEBIMENTO: TStringField
      FieldName = 'FORMARECEBIMENTO'
      Required = True
      FixedChar = True
      Size = 1
    end
    object s_1DATABAIXA: TDateField
      FieldName = 'DATABAIXA'
    end
    object s_1HISTORICO: TStringField
      FieldName = 'HISTORICO'
      Size = 150
    end
    object s_1CODVENDA: TIntegerField
      FieldName = 'CODVENDA'
    end
    object s_1CODALMOXARIFADO: TSmallintField
      FieldName = 'CODALMOXARIFADO'
    end
    object s_1CODVENDEDOR: TSmallintField
      FieldName = 'CODVENDEDOR'
    end
    object s_1CODUSUARIO: TSmallintField
      FieldName = 'CODUSUARIO'
    end
    object s_1N_DOCUMENTO: TStringField
      FieldName = 'N_DOCUMENTO'
    end
    object s_1DATASISTEMA: TSQLTimeStampField
      FieldName = 'DATASISTEMA'
    end
    object s_1VALORRECEBIDO: TFloatField
      FieldName = 'VALORRECEBIDO'
    end
    object s_1JUROS: TFloatField
      FieldName = 'JUROS'
    end
    object s_1DESCONTO: TFloatField
      FieldName = 'DESCONTO'
    end
    object s_1PERDA: TFloatField
      FieldName = 'PERDA'
    end
    object s_1TROCA: TFloatField
      FieldName = 'TROCA'
    end
    object s_1FUNRURAL: TFloatField
      FieldName = 'FUNRURAL'
    end
    object s_1VALOR_PRIM_VIA: TFloatField
      FieldName = 'VALOR_PRIM_VIA'
    end
    object s_1VALOR_RESTO: TFloatField
      FieldName = 'VALOR_RESTO'
    end
    object s_1VALORTITULO: TFloatField
      FieldName = 'VALORTITULO'
    end
    object s_1OUTRO_CREDITO: TFloatField
      FieldName = 'OUTRO_CREDITO'
    end
    object s_1OUTRO_DEBITO: TFloatField
      FieldName = 'OUTRO_DEBITO'
    end
    object s_1PARCELAS: TSmallintField
      FieldName = 'PARCELAS'
    end
    object s_1NOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      Required = True
      Size = 50
    end
    object s_1VALORREC: TFloatField
      FieldName = 'VALORREC'
    end
  end
  object d_1: TDataSetProvider
    DataSet = s_1
    Options = [poAllowCommandText]
    Left = 592
    Top = 408
  end
  object scdsCr_proc2: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'd_1'
    Left = 624
    Top = 408
    object scdsCr_proc2CODRECEBIMENTO: TIntegerField
      FieldName = 'CODRECEBIMENTO'
      Required = True
    end
    object scdsCr_proc2TITULO: TStringField
      FieldName = 'TITULO'
      Required = True
      Size = 18
    end
    object scdsCr_proc2EMISSAO: TDateField
      FieldName = 'EMISSAO'
    end
    object scdsCr_proc2CODCLIENTE: TIntegerField
      FieldName = 'CODCLIENTE'
      Required = True
    end
    object scdsCr_proc2DATAVENCIMENTO: TDateField
      FieldName = 'DATAVENCIMENTO'
    end
    object scdsCr_proc2DATARECEBIMENTO: TDateField
      FieldName = 'DATARECEBIMENTO'
    end
    object scdsCr_proc2CAIXA: TSmallintField
      FieldName = 'CAIXA'
    end
    object scdsCr_proc2CONTADEBITO: TIntegerField
      FieldName = 'CONTADEBITO'
    end
    object scdsCr_proc2CONTACREDITO: TIntegerField
      FieldName = 'CONTACREDITO'
    end
    object scdsCr_proc2STATUS: TStringField
      FieldName = 'STATUS'
      Required = True
      FixedChar = True
      Size = 2
    end
    object scdsCr_proc2VIA: TStringField
      FieldName = 'VIA'
      Required = True
      FixedChar = True
      Size = 1
    end
    object scdsCr_proc2FORMARECEBIMENTO: TStringField
      FieldName = 'FORMARECEBIMENTO'
      Required = True
      FixedChar = True
      Size = 1
    end
    object scdsCr_proc2DATABAIXA: TDateField
      FieldName = 'DATABAIXA'
    end
    object scdsCr_proc2HISTORICO: TStringField
      FieldName = 'HISTORICO'
      Size = 150
    end
    object scdsCr_proc2CODVENDA: TIntegerField
      FieldName = 'CODVENDA'
    end
    object scdsCr_proc2CODALMOXARIFADO: TSmallintField
      FieldName = 'CODALMOXARIFADO'
    end
    object scdsCr_proc2CODVENDEDOR: TSmallintField
      FieldName = 'CODVENDEDOR'
    end
    object scdsCr_proc2CODUSUARIO: TSmallintField
      FieldName = 'CODUSUARIO'
    end
    object scdsCr_proc2N_DOCUMENTO: TStringField
      FieldName = 'N_DOCUMENTO'
    end
    object scdsCr_proc2DATASISTEMA: TSQLTimeStampField
      FieldName = 'DATASISTEMA'
    end
    object scdsCr_proc2VALORRECEBIDO: TFloatField
      FieldName = 'VALORRECEBIDO'
    end
    object scdsCr_proc2JUROS: TFloatField
      FieldName = 'JUROS'
    end
    object scdsCr_proc2DESCONTO: TFloatField
      FieldName = 'DESCONTO'
    end
    object scdsCr_proc2PERDA: TFloatField
      FieldName = 'PERDA'
    end
    object scdsCr_proc2TROCA: TFloatField
      FieldName = 'TROCA'
    end
    object scdsCr_proc2FUNRURAL: TFloatField
      FieldName = 'FUNRURAL'
    end
    object scdsCr_proc2VALOR_PRIM_VIA: TFloatField
      FieldName = 'VALOR_PRIM_VIA'
    end
    object scdsCr_proc2VALOR_RESTO: TFloatField
      FieldName = 'VALOR_RESTO'
    end
    object scdsCr_proc2VALORTITULO: TFloatField
      FieldName = 'VALORTITULO'
    end
    object scdsCr_proc2OUTRO_CREDITO: TFloatField
      FieldName = 'OUTRO_CREDITO'
    end
    object scdsCr_proc2OUTRO_DEBITO: TFloatField
      FieldName = 'OUTRO_DEBITO'
    end
    object scdsCr_proc2PARCELAS: TSmallintField
      FieldName = 'PARCELAS'
    end
    object scdsCr_proc2NOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      Required = True
      Size = 50
    end
    object scdsCr_proc2VALORREC: TFloatField
      FieldName = 'VALORREC'
    end
  end
  object DataSource1: TDataSource
    DataSet = DM.cds_serie
    Left = 672
    Top = 408
  end
  object ds_Cr: TDataSource
    DataSet = scdsCr_proc
    Left = 251
    Top = 352
  end
  object PopupMenu1: TPopupMenu
    Left = 517
    Top = 411
    object TMenuItem
    end
    object Executarconsulta1: TMenuItem
      Caption = 'Executar consulta'
      ShortCut = 119
      OnClick = btnProcurarClick
    end
    object Receber1: TMenuItem
      Caption = 'Pagar'
      ShortCut = 113
      OnClick = btnIncluirClick
    end
    object Cancelar1: TMenuItem
      Caption = 'Cancelar'
      ShortCut = 114
    end
    object Sair1: TMenuItem
      Caption = '&Sair'
      ShortCut = 120
      OnClick = btnSairClick
    end
    object confirma1: TMenuItem
      Caption = 'confirma'
      ShortCut = 115
      OnClick = BitBtn9Click
    end
    object marcarTodosClick1: TMenuItem
      Caption = 'Marcar Todos'
      OnClick = marcarTodosClick1Click
    end
    object DesmarcarSeleo1: TMenuItem
      Caption = 'Desmarcar Sele'#231#227'o'
      OnClick = DesmarcarSeleo1Click
    end
  end
  object SQLDataSet1: TSQLDataSet
    CommandText = 
      'select rec.*, cli.NOMEFORNECEDOR, udf_rounddec( (VALORTITULO - V' +
      'ALOR_RESTO),2) as VALORREC  from PAGAMENTO rec '#13#10'inner join FORN' +
      'ECEDOR cli on cli.CODFORNECEDOR=rec.CODFORNECEDOR'#13#10
    MaxBlobSize = -1
    Params = <>
    SQLConnection = DM.sqlsisAdimin
    Left = 160
    Top = 352
    object SQLDataSet1CODPAGAMENTO: TIntegerField
      FieldName = 'CODPAGAMENTO'
      Required = True
    end
    object SQLDataSet1DUP_REC_NF: TStringField
      FieldName = 'DUP_REC_NF'
      ProviderFlags = [pfInUpdate]
      Size = 15
    end
    object SQLDataSet1DP: TSmallintField
      FieldName = 'DP'
      ProviderFlags = [pfInUpdate]
    end
    object SQLDataSet1StringField: TStringField
      FieldName = 'TITULO'
      ProviderFlags = [pfInUpdate]
      Required = True
      Size = 18
    end
    object SQLDataSet1DateField: TDateField
      FieldName = 'EMISSAO'
      ProviderFlags = [pfInUpdate]
      EditMask = '!99/99/00;1;_'
    end
    object SQLDataSet1DateField2: TDateField
      FieldName = 'DATAVENCIMENTO'
      ProviderFlags = [pfInUpdate]
      EditMask = '!99/99/00;1;_'
    end
    object SQLDataSet1StringField2: TStringField
      FieldName = 'STATUS'
      ProviderFlags = [pfInUpdate]
      Required = True
      FixedChar = True
      Size = 30
    end
    object SQLDataSet1StringField3: TStringField
      FieldName = 'VIA'
      ProviderFlags = [pfInUpdate]
      FixedChar = True
      Size = 4
    end
    object SQLDataSet1StringField4: TStringField
      FieldName = 'N_DOCUMENTO'
      ProviderFlags = [pfInUpdate]
    end
    object SQLDataSet1FloatField: TFloatField
      FieldName = 'VALORRECEBIDO'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
    end
    object SQLDataSet1FloatField2: TFloatField
      FieldName = 'VALOR_RESTO'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
    end
    object SQLDataSet1FloatField3: TFloatField
      FieldName = 'VALORTITULO'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
    end
    object SQLDataSet1FloatField4: TFloatField
      FieldName = 'VALORREC'
      ProviderFlags = [pfInUpdate]
      ReadOnly = True
      DisplayFormat = ',##0.00'
    end
    object SQLDataSet1FloatField5: TFloatField
      FieldName = 'VALOR_PRIM_VIA'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
    end
    object SQLDataSet1StringField5: TStringField
      FieldName = 'NOMEFORNECEDOR'
      ProviderFlags = []
      Required = True
      Size = 50
    end
    object SQLDataSet1CODFORNECEDOR: TIntegerField
      FieldName = 'CODFORNECEDOR'
      Required = True
    end
    object SQLDataSet1DATAPAGAMENTO: TDateField
      FieldName = 'DATAPAGAMENTO'
    end
    object SQLDataSet1DATACONSOLIDA: TDateField
      FieldName = 'DATACONSOLIDA'
    end
    object SQLDataSet1CAIXA: TSmallintField
      FieldName = 'CAIXA'
    end
    object SQLDataSet1FORMAPAGAMENTO: TStringField
      FieldName = 'FORMAPAGAMENTO'
      Required = True
      FixedChar = True
      Size = 1
    end
    object SQLDataSet1HISTORICO: TStringField
      FieldName = 'HISTORICO'
      Size = 150
    end
    object SQLDataSet1DATASISTEMA: TSQLTimeStampField
      FieldName = 'DATASISTEMA'
    end
  end
  object DataSetProvider1: TDataSetProvider
    DataSet = SQLDataSet1
    Options = [poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 191
    Top = 352
  end
  object scdsCr_proc: TClientDataSet
    Aggregates = <>
    AggregatesActive = True
    Params = <>
    ProviderName = 'DataSetProvider1'
    Left = 221
    Top = 352
    object f: TStringField
      FieldName = 'TITULO'
      ProviderFlags = [pfInUpdate]
      Required = True
      Size = 18
    end
    object scdsCr_procEMISSAO: TDateField
      FieldName = 'EMISSAO'
      ProviderFlags = [pfInUpdate]
      EditMask = '!99/99/00;1;_'
    end
    object scdsCr_procDATAVENCIMENTO: TDateField
      FieldName = 'DATAVENCIMENTO'
      ProviderFlags = [pfInUpdate]
      EditMask = '!99/99/00;1;_'
    end
    object scdsCr_procSTATUS: TStringField
      FieldName = 'STATUS'
      ProviderFlags = [pfInUpdate]
      Required = True
      FixedChar = True
      Size = 30
    end
    object scdsCr_procVIA: TStringField
      FieldName = 'VIA'
      ProviderFlags = [pfInUpdate]
      FixedChar = True
      Size = 4
    end
    object scdsCr_procN_DOCUMENTO: TStringField
      FieldName = 'N_DOCUMENTO'
      ProviderFlags = [pfInUpdate]
    end
    object scdsCr_procVALORRECEBIDO: TFloatField
      FieldName = 'VALORRECEBIDO'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
      EditFormat = ',##0.00'
    end
    object scdsCr_procVALOR_RESTO: TFloatField
      FieldName = 'VALOR_RESTO'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
      EditFormat = ',##0.00'
    end
    object scdsCr_procVALORTITULO: TFloatField
      FieldName = 'VALORTITULO'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
      EditFormat = ',##0.00'
    end
    object scdsCr_procVALORREC: TFloatField
      FieldName = 'VALORREC'
      ProviderFlags = [pfInUpdate]
      ReadOnly = True
      DisplayFormat = ',##0.00'
      EditFormat = ',##0.00'
    end
    object scdsCr_procVALOR_PRIM_VIA: TFloatField
      FieldName = 'VALOR_PRIM_VIA'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
      EditFormat = ',##0.00'
    end
    object scdsCr_procNOMEFORNECEDOR: TStringField
      FieldName = 'NOMEFORNECEDOR'
      ProviderFlags = []
      Required = True
      Size = 50
    end
    object scdsCr_procCODPAGAMENTO: TIntegerField
      FieldName = 'CODPAGAMENTO'
      ProviderFlags = [pfInUpdate]
      Required = True
    end
    object scdsCr_procCODFORNECEDOR: TIntegerField
      FieldName = 'CODFORNECEDOR'
      Required = True
    end
    object scdsCr_procDATAPAGAMENTO: TDateField
      FieldName = 'DATAPAGAMENTO'
      EditMask = '!99/99/00;1;_'
    end
    object scdsCr_procDUP_REC_NF: TStringField
      FieldName = 'DUP_REC_NF'
      ProviderFlags = [pfInUpdate]
      Size = 15
    end
    object scdsCr_procDP: TSmallintField
      FieldName = 'DP'
      ProviderFlags = [pfInUpdate]
    end
    object scdsCr_procDATACONSOLIDA: TDateField
      FieldName = 'DATACONSOLIDA'
    end
    object scdsCr_procCAIXA: TSmallintField
      FieldName = 'CAIXA'
    end
    object scdsCr_procFORMAPAGAMENTO: TStringField
      FieldName = 'FORMAPAGAMENTO'
      Required = True
      FixedChar = True
      Size = 1
    end
    object scdsCr_procDATASISTEMA: TSQLTimeStampField
      FieldName = 'DATASISTEMA'
    end
    object scdsCr_procHISTORICO: TStringField
      FieldName = 'HISTORICO'
      Size = 150
    end
    object scdsCr_procTotTitulo: TAggregateField
      Alignment = taRightJustify
      FieldName = 'TotTitulo'
      Active = True
      DisplayFormat = ',##0.00'
      Expression = 'sum(VALOR_RESTO)'
    end
    object scdsCr_procTotPendente: TAggregateField
      Alignment = taRightJustify
      FieldName = 'TotPendente'
      Active = True
      DisplayFormat = ',##0.00'
      Expression = 'sum(valorRec)'
    end
    object scdsCr_procTotPago: TAggregateField
      Alignment = taRightJustify
      FieldName = 'TotPago'
      Active = True
      Expression = 'sum(valorrecebido)'
    end
  end
  object repContasReceber: TVCLReport
    AsyncExecution = False
    Title = 'Untitled'
    Left = 312
    Top = 352
  end
  object sds_total: TSQLDataSet
    MaxBlobSize = -1
    Params = <>
    SQLConnection = DM.sqlsisAdimin
    Left = 328
    Top = 286
  end
  object dspTotal: TDataSetProvider
    DataSet = sds_total
    Options = [poAllowCommandText]
    Left = 359
    Top = 286
  end
  object cds_total: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspTotal'
    Left = 390
    Top = 286
  end
  object ImageList1: TImageList
    Left = 458
    Top = 288
    Bitmap = {
      
      000000000000}
  end
  object sdsTipoForn: TSQLDataSet
    CommandText = 'select * from DADOS_COMBOS '#13#10'where USO = '#39'regiaoFor'#39' '
    MaxBlobSize = -1
    Params = <>
    SQLConnection = DM.sqlsisAdimin
    Left = 328
    Top = 248
    object sdsTipoFornCODDADOS: TIntegerField
      FieldName = 'CODDADOS'
      Required = True
    end
    object sdsTipoFornDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 80
    end
    object sdsTipoFornUSO: TStringField
      FieldName = 'USO'
      Size = 30
    end
    object sdsTipoFornCODIGOS: TStringField
      FieldName = 'CODIGOS'
      Size = 50
    end
    object sdsTipoFornOUTROS: TStringField
      FieldName = 'OUTROS'
      Size = 30
    end
  end
  object dspTipoForn: TDataSetProvider
    DataSet = sdsTipoForn
    Left = 360
    Top = 248
  end
  object cdsTipoForn: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspTipoForn'
    Left = 392
    Top = 248
    object cdsTipoFornCODDADOS: TIntegerField
      FieldName = 'CODDADOS'
      Required = True
    end
    object cdsTipoFornDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 80
    end
    object cdsTipoFornUSO: TStringField
      FieldName = 'USO'
      Size = 30
    end
    object cdsTipoFornCODIGOS: TStringField
      FieldName = 'CODIGOS'
      Size = 50
    end
    object cdsTipoFornOUTROS: TStringField
      FieldName = 'OUTROS'
      Size = 30
    end
  end
  object DataSetProvider2: TDataSetProvider
    DataSet = SQLDataSet2
    Options = [poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 191
    Top = 392
  end
  object cds1: TClientDataSet
    Aggregates = <>
    AggregatesActive = True
    Params = <>
    ProviderName = 'DataSetProvider2'
    Left = 221
    Top = 392
    object cds1DUP_REC_NF: TStringField
      FieldName = 'DUP_REC_NF'
      Size = 15
    end
    object cds1DP: TSmallintField
      FieldName = 'DP'
    end
    object cds1TITULO: TStringField
      FieldName = 'TITULO'
      Required = True
      Size = 18
    end
    object cds1EMISSAO: TDateField
      FieldName = 'EMISSAO'
    end
    object cds1DATAVENCIMENTO: TDateField
      FieldName = 'DATAVENCIMENTO'
    end
    object cds1STATUS: TStringField
      FieldName = 'STATUS'
      Required = True
      FixedChar = True
      Size = 30
    end
    object cds1VIA: TStringField
      FieldName = 'VIA'
      FixedChar = True
      Size = 4
    end
    object cds1N_DOCUMENTO: TStringField
      FieldName = 'N_DOCUMENTO'
    end
    object cds1VALORRECEBIDO: TFloatField
      FieldName = 'VALORRECEBIDO'
    end
    object cds1VALOR_RESTO: TFloatField
      FieldName = 'VALOR_RESTO'
    end
    object cds1VALORTITULO: TFloatField
      FieldName = 'VALORTITULO'
    end
    object cds1VALORREC: TFloatField
      FieldName = 'VALORREC'
      ReadOnly = True
    end
    object cds1VALOR_PRIM_VIA: TFloatField
      FieldName = 'VALOR_PRIM_VIA'
    end
    object cds1NOMEFORNECEDOR: TStringField
      FieldName = 'NOMEFORNECEDOR'
      Required = True
      Size = 50
    end
    object cds1CODFORNECEDOR: TIntegerField
      FieldName = 'CODFORNECEDOR'
      Required = True
    end
    object cds1DATAPAGAMENTO: TDateField
      FieldName = 'DATAPAGAMENTO'
    end
    object cds1DATACONSOLIDA: TDateField
      FieldName = 'DATACONSOLIDA'
    end
    object cds1CAIXA: TSmallintField
      FieldName = 'CAIXA'
    end
    object cds1FORMAPAGAMENTO: TStringField
      FieldName = 'FORMAPAGAMENTO'
      Required = True
      FixedChar = True
      Size = 1
    end
    object cds1TotTitulo: TAggregateField
      Alignment = taRightJustify
      FieldName = 'TotTitulo'
      Active = True
      DisplayFormat = ',##0.00'
      Expression = 'sum(VALOR_RESTO)'
    end
    object cds1TotPendente: TAggregateField
      Alignment = taRightJustify
      FieldName = 'TotPendente'
      Active = True
      DisplayFormat = ',##0.00'
      Expression = 'sum(valorRec)'
    end
    object cds1TotPago: TAggregateField
      Alignment = taRightJustify
      FieldName = 'TotPago'
      Active = True
      Expression = 'sum(valorrecebido)'
    end
  end
  object DataSource2: TDataSource
    DataSet = cds1
    Left = 251
    Top = 392
  end
  object SQLDataSet2: TSQLDataSet
    CommandText = 
      'select rec.*, cli.NOMEFORNECEDOR, udf_rounddec( (VALORTITULO - V' +
      'ALOR_RESTO),2) as VALORREC  from PAGAMENTO rec '#13#10'inner join FORN' +
      'ECEDOR cli on cli.CODFORNECEDOR=rec.CODFORNECEDOR'#13#10
    MaxBlobSize = -1
    Params = <>
    SQLConnection = DM.sqlsisAdimin
    Left = 160
    Top = 392
    object StringField1: TStringField
      FieldName = 'DUP_REC_NF'
      ProviderFlags = [pfInUpdate]
      Size = 15
    end
    object SmallintField1: TSmallintField
      FieldName = 'DP'
      ProviderFlags = [pfInUpdate]
    end
    object StringField2: TStringField
      FieldName = 'TITULO'
      ProviderFlags = [pfInUpdate]
      Required = True
      Size = 18
    end
    object DateField1: TDateField
      FieldName = 'EMISSAO'
      ProviderFlags = [pfInUpdate]
      EditMask = '!99/99/00;1;_'
    end
    object DateField2: TDateField
      FieldName = 'DATAVENCIMENTO'
      ProviderFlags = [pfInUpdate]
      EditMask = '!99/99/00;1;_'
    end
    object StringField3: TStringField
      FieldName = 'STATUS'
      ProviderFlags = [pfInUpdate]
      Required = True
      FixedChar = True
      Size = 30
    end
    object StringField4: TStringField
      FieldName = 'VIA'
      ProviderFlags = [pfInUpdate]
      FixedChar = True
      Size = 4
    end
    object StringField5: TStringField
      FieldName = 'N_DOCUMENTO'
      ProviderFlags = [pfInUpdate]
    end
    object FloatField1: TFloatField
      FieldName = 'VALORRECEBIDO'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
    end
    object FloatField2: TFloatField
      FieldName = 'VALOR_RESTO'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
    end
    object FloatField3: TFloatField
      FieldName = 'VALORTITULO'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
    end
    object FloatField4: TFloatField
      FieldName = 'VALORREC'
      ProviderFlags = [pfInUpdate]
      ReadOnly = True
      DisplayFormat = ',##0.00'
    end
    object FloatField5: TFloatField
      FieldName = 'VALOR_PRIM_VIA'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
    end
    object StringField6: TStringField
      FieldName = 'NOMEFORNECEDOR'
      ProviderFlags = []
      Required = True
      Size = 50
    end
    object IntegerField2: TIntegerField
      FieldName = 'CODFORNECEDOR'
      Required = True
    end
    object DateField3: TDateField
      FieldName = 'DATAPAGAMENTO'
    end
    object DateField4: TDateField
      FieldName = 'DATACONSOLIDA'
    end
    object SmallintField2: TSmallintField
      FieldName = 'CAIXA'
    end
    object StringField7: TStringField
      FieldName = 'FORMAPAGAMENTO'
      Required = True
      FixedChar = True
      Size = 1
    end
  end
  object SQLDataSet3: TSQLDataSet
    CommandText = 
      'select rec.*, cli.NOMEFORNECEDOR, udf_rounddec( (VALORTITULO - V' +
      'ALOR_RESTO),2) as VALORREC  from PAGAMENTO rec '#13#10'inner join FORN' +
      'ECEDOR cli on cli.CODFORNECEDOR=rec.CODFORNECEDOR'#13#10
    MaxBlobSize = -1
    Params = <>
    SQLConnection = DM.sqlsisAdimin
    Left = 160
    Top = 424
    object StringField8: TStringField
      FieldName = 'DUP_REC_NF'
      ProviderFlags = [pfInUpdate]
      Size = 15
    end
    object SmallintField3: TSmallintField
      FieldName = 'DP'
      ProviderFlags = [pfInUpdate]
    end
    object StringField9: TStringField
      FieldName = 'TITULO'
      ProviderFlags = [pfInUpdate]
      Required = True
      Size = 18
    end
    object DateField5: TDateField
      FieldName = 'EMISSAO'
      ProviderFlags = [pfInUpdate]
      EditMask = '!99/99/00;1;_'
    end
    object DateField6: TDateField
      FieldName = 'DATAVENCIMENTO'
      ProviderFlags = [pfInUpdate]
      EditMask = '!99/99/00;1;_'
    end
    object StringField10: TStringField
      FieldName = 'STATUS'
      ProviderFlags = [pfInUpdate]
      Required = True
      FixedChar = True
      Size = 30
    end
    object StringField11: TStringField
      FieldName = 'VIA'
      ProviderFlags = [pfInUpdate]
      FixedChar = True
      Size = 4
    end
    object StringField12: TStringField
      FieldName = 'N_DOCUMENTO'
      ProviderFlags = [pfInUpdate]
    end
    object FloatField6: TFloatField
      FieldName = 'VALORRECEBIDO'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
    end
    object FloatField7: TFloatField
      FieldName = 'VALOR_RESTO'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
    end
    object FloatField8: TFloatField
      FieldName = 'VALORTITULO'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
    end
    object FloatField9: TFloatField
      FieldName = 'VALORREC'
      ProviderFlags = [pfInUpdate]
      ReadOnly = True
      DisplayFormat = ',##0.00'
    end
    object FloatField10: TFloatField
      FieldName = 'VALOR_PRIM_VIA'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
    end
    object StringField13: TStringField
      FieldName = 'NOMEFORNECEDOR'
      ProviderFlags = []
      Required = True
      Size = 50
    end
    object IntegerField1: TIntegerField
      FieldName = 'CODFORNECEDOR'
      Required = True
    end
    object DateField7: TDateField
      FieldName = 'DATAPAGAMENTO'
    end
    object DateField8: TDateField
      FieldName = 'DATACONSOLIDA'
    end
    object SmallintField4: TSmallintField
      FieldName = 'CAIXA'
    end
    object StringField14: TStringField
      FieldName = 'FORMAPAGAMENTO'
      Required = True
      FixedChar = True
      Size = 1
    end
    object SQLDataSet3CODPAGAMENTO: TIntegerField
      FieldName = 'CODPAGAMENTO'
      Required = True
    end
  end
  object DataSetProvider3: TDataSetProvider
    DataSet = SQLDataSet3
    Options = [poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 191
    Top = 424
  end
  object cds2: TClientDataSet
    Aggregates = <>
    AggregatesActive = True
    Params = <>
    ProviderName = 'DataSetProvider3'
    Left = 221
    Top = 424
    object cds2DUP_REC_NF: TStringField
      FieldName = 'DUP_REC_NF'
      Size = 15
    end
    object cds2DP: TSmallintField
      FieldName = 'DP'
    end
    object cds2TITULO: TStringField
      FieldName = 'TITULO'
      Required = True
      Size = 18
    end
    object cds2EMISSAO: TDateField
      FieldName = 'EMISSAO'
    end
    object cds2DATAVENCIMENTO: TDateField
      FieldName = 'DATAVENCIMENTO'
    end
    object cds2STATUS: TStringField
      FieldName = 'STATUS'
      Required = True
      FixedChar = True
      Size = 30
    end
    object cds2VIA: TStringField
      FieldName = 'VIA'
      FixedChar = True
      Size = 4
    end
    object cds2N_DOCUMENTO: TStringField
      FieldName = 'N_DOCUMENTO'
    end
    object cds2VALORRECEBIDO: TFloatField
      FieldName = 'VALORRECEBIDO'
    end
    object cds2VALOR_RESTO: TFloatField
      FieldName = 'VALOR_RESTO'
    end
    object cds2VALORTITULO: TFloatField
      FieldName = 'VALORTITULO'
    end
    object cds2VALORREC: TFloatField
      FieldName = 'VALORREC'
      ReadOnly = True
    end
    object cds2VALOR_PRIM_VIA: TFloatField
      FieldName = 'VALOR_PRIM_VIA'
    end
    object cds2NOMEFORNECEDOR: TStringField
      FieldName = 'NOMEFORNECEDOR'
      Required = True
      Size = 50
    end
    object cds2CODFORNECEDOR: TIntegerField
      FieldName = 'CODFORNECEDOR'
      Required = True
    end
    object cds2DATAPAGAMENTO: TDateField
      FieldName = 'DATAPAGAMENTO'
    end
    object cds2DATACONSOLIDA: TDateField
      FieldName = 'DATACONSOLIDA'
    end
    object cds2CAIXA: TSmallintField
      FieldName = 'CAIXA'
    end
    object cds2FORMAPAGAMENTO: TStringField
      FieldName = 'FORMAPAGAMENTO'
      Required = True
      FixedChar = True
      Size = 1
    end
    object cds2CODPAGAMENTO: TIntegerField
      FieldName = 'CODPAGAMENTO'
      Required = True
    end
    object AggregateField1: TAggregateField
      Alignment = taRightJustify
      FieldName = 'TotTitulo'
      Active = True
      DisplayFormat = ',##0.00'
      Expression = 'sum(VALOR_RESTO)'
    end
    object AggregateField2: TAggregateField
      Alignment = taRightJustify
      FieldName = 'TotPendente'
      Active = True
      DisplayFormat = ',##0.00'
      Expression = 'sum(valorRec)'
    end
    object AggregateField3: TAggregateField
      Alignment = taRightJustify
      FieldName = 'TotPago'
      Active = True
      Expression = 'sum(valorrecebido)'
    end
  end
  object JvFormStorage1: TJvFormStorage
    AppStorage = JvAppXMLFileStorage1
    AppStoragePath = 'fCpProc\'
    StoredProps.Strings = (
      'cbConta.Text'
      'cbForma.ItemIndex'
      'cbStatus.ItemIndex'
      'cbTipoFornecedor.ItemHeight'
      'edCliente.Text'
      'edCodCCusto.Text'
      'edCodCliente.Text'
      'edcodVendedor.Text'
      'Edit1.Text'
      'edNumeroDocumento.Text'
      'edTitulo.Text'
      'edValor.Text'
      'edVendedor.Text'
      'edCodCCusto.ItemIndex'
      'rbTipoConta.ItemIndex')
    StoredValues = <>
    Left = 560
    Top = 248
  end
  object JvAppXMLFileStorage1: TJvAppXMLFileStorage
    StorageOptions.BooleanStringTrueValues = 'TRUE, YES, Y'
    StorageOptions.BooleanStringFalseValues = 'FALSE, NO, N'
    StorageOptions.InvalidCharReplacement = '_'
    FileName = 'cpFiltro.xml'
    RootNodeName = 'Configuration'
    SubStorages = <>
    Left = 600
    Top = 240
  end
  object sdsAu: TSQLDataSet
    CommandText = 'select * from auditoria(:dia,:dataini,:datafin,:codalmox)'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftDate
        Name = 'dia'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'dataini'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'datafin'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'codalmox'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Left = 40
    Top = 288
    object sdsAuTITULO: TStringField
      FieldName = 'TITULO'
      Size = 18
    end
    object sdsAuEMISSAO: TDateField
      FieldName = 'EMISSAO'
    end
    object sdsAuDATACONSOLIDA: TDateField
      FieldName = 'DATACONSOLIDA'
    end
    object sdsAuCAIXA: TSmallintField
      FieldName = 'CAIXA'
    end
    object sdsAuFORMAPAGAMENTO: TStringField
      FieldName = 'FORMAPAGAMENTO'
      FixedChar = True
      Size = 1
    end
    object sdsAuDATAVENCIMENTO: TDateField
      FieldName = 'DATAVENCIMENTO'
    end
    object sdsAuCODFORNECEDOR: TIntegerField
      FieldName = 'CODFORNECEDOR'
    end
    object sdsAuVALORTITULO: TFloatField
      FieldName = 'VALORTITULO'
    end
    object sdsAuVALOR_RESTO: TFloatField
      FieldName = 'VALOR_RESTO'
    end
    object sdsAuVALOR_PRIM_VIA: TFloatField
      FieldName = 'VALOR_PRIM_VIA'
    end
    object sdsAuSTATUS: TStringField
      FieldName = 'STATUS'
      FixedChar = True
      Size = 16
    end
    object sdsAuDATAPAGAMENTO: TDateField
      FieldName = 'DATAPAGAMENTO'
    end
    object sdsAuDATAINCLUSAO: TDateField
      FieldName = 'DATAINCLUSAO'
    end
    object sdsAuVALORRECEBIDO: TFloatField
      FieldName = 'VALORRECEBIDO'
    end
    object sdsAuNOMEFORNECEDOR: TStringField
      FieldName = 'NOMEFORNECEDOR'
      Size = 60
    end
    object sdsAuVALORREC: TFloatField
      FieldName = 'VALORREC'
    end
    object sdsAuNOME: TStringField
      FieldName = 'NOME'
      Size = 200
    end
  end
  object cdsAu: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftDate
        Name = 'dia'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'dataini'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'datafin'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'codalmox'
        ParamType = ptInput
      end>
    ProviderName = 'dspAu'
    Left = 104
    Top = 288
    object cdsAuTITULO: TStringField
      FieldName = 'TITULO'
      Size = 18
    end
    object cdsAuEMISSAO: TDateField
      FieldName = 'EMISSAO'
    end
    object cdsAuDATACONSOLIDA: TDateField
      FieldName = 'DATACONSOLIDA'
    end
    object cdsAuCAIXA: TSmallintField
      FieldName = 'CAIXA'
    end
    object cdsAuFORMAPAGAMENTO: TStringField
      FieldName = 'FORMAPAGAMENTO'
      FixedChar = True
      Size = 1
    end
    object cdsAuDATAVENCIMENTO: TDateField
      FieldName = 'DATAVENCIMENTO'
    end
    object cdsAuCODFORNECEDOR: TIntegerField
      FieldName = 'CODFORNECEDOR'
    end
    object cdsAuVALORTITULO: TFloatField
      FieldName = 'VALORTITULO'
    end
    object cdsAuVALOR_RESTO: TFloatField
      FieldName = 'VALOR_RESTO'
    end
    object cdsAuVALOR_PRIM_VIA: TFloatField
      FieldName = 'VALOR_PRIM_VIA'
    end
    object cdsAuSTATUS: TStringField
      FieldName = 'STATUS'
      FixedChar = True
      Size = 16
    end
    object cdsAuDATAPAGAMENTO: TDateField
      FieldName = 'DATAPAGAMENTO'
    end
    object cdsAuDATAINCLUSAO: TDateField
      FieldName = 'DATAINCLUSAO'
    end
    object cdsAuVALORRECEBIDO: TFloatField
      FieldName = 'VALORRECEBIDO'
    end
    object cdsAuNOMEFORNECEDOR: TStringField
      FieldName = 'NOMEFORNECEDOR'
      Size = 60
    end
    object cdsAuVALORREC: TFloatField
      FieldName = 'VALORREC'
    end
    object cdsAuNOME: TStringField
      FieldName = 'NOME'
      Size = 200
    end
  end
  object dspAu: TDataSetProvider
    DataSet = sdsAu
    Options = [poAllowCommandText]
    Left = 72
    Top = 288
  end
  object scdsCr_procX: TClientDataSet
    Aggregates = <>
    AggregatesActive = True
    Params = <>
    ProviderName = 'DataSetProvider1'
    Left = 221
    Top = 312
    object StringField15: TStringField
      FieldName = 'TITULO'
      ProviderFlags = [pfInUpdate]
      Required = True
      Size = 18
    end
    object DateField9: TDateField
      FieldName = 'EMISSAO'
      ProviderFlags = [pfInUpdate]
      EditMask = '!99/99/00;1;_'
    end
    object DateField10: TDateField
      FieldName = 'DATAVENCIMENTO'
      ProviderFlags = [pfInUpdate]
      EditMask = '!99/99/00;1;_'
    end
    object StringField16: TStringField
      FieldName = 'STATUS'
      ProviderFlags = [pfInUpdate]
      Required = True
      FixedChar = True
      Size = 30
    end
    object StringField17: TStringField
      FieldName = 'VIA'
      ProviderFlags = [pfInUpdate]
      FixedChar = True
      Size = 4
    end
    object StringField18: TStringField
      FieldName = 'N_DOCUMENTO'
      ProviderFlags = [pfInUpdate]
    end
    object FloatField11: TFloatField
      FieldName = 'VALORRECEBIDO'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
      EditFormat = ',##0.00'
    end
    object FloatField12: TFloatField
      FieldName = 'VALOR_RESTO'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
      EditFormat = ',##0.00'
    end
    object FloatField13: TFloatField
      FieldName = 'VALORTITULO'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
      EditFormat = ',##0.00'
    end
    object FloatField14: TFloatField
      FieldName = 'VALORREC'
      ProviderFlags = [pfInUpdate]
      ReadOnly = True
      DisplayFormat = ',##0.00'
      EditFormat = ',##0.00'
    end
    object FloatField15: TFloatField
      FieldName = 'VALOR_PRIM_VIA'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = ',##0.00'
      EditFormat = ',##0.00'
    end
    object StringField19: TStringField
      FieldName = 'NOMEFORNECEDOR'
      ProviderFlags = []
      Required = True
      Size = 50
    end
    object IntegerField4: TIntegerField
      FieldName = 'CODFORNECEDOR'
      Required = True
    end
    object DateField11: TDateField
      FieldName = 'DATAPAGAMENTO'
      EditMask = '!99/99/00;1;_'
    end
    object StringField20: TStringField
      FieldName = 'DUP_REC_NF'
      ProviderFlags = [pfInUpdate]
      Size = 15
    end
    object SmallintField5: TSmallintField
      FieldName = 'DP'
      ProviderFlags = [pfInUpdate]
    end
    object DateField12: TDateField
      FieldName = 'DATACONSOLIDA'
    end
    object SmallintField6: TSmallintField
      FieldName = 'CAIXA'
    end
    object StringField21: TStringField
      FieldName = 'FORMAPAGAMENTO'
      Required = True
      FixedChar = True
      Size = 1
    end
    object SQLTimeStampField1: TSQLTimeStampField
      FieldName = 'DATASISTEMA'
    end
    object scdsCr_procXCODPAGAMENTO: TIntegerField
      FieldName = 'CODPAGAMENTO'
      Required = True
    end
    object AggregateField4: TAggregateField
      Alignment = taRightJustify
      FieldName = 'TotTitulo'
      Active = True
      DisplayFormat = ',##0.00'
      Expression = 'sum(VALOR_RESTO)'
    end
    object AggregateField5: TAggregateField
      Alignment = taRightJustify
      FieldName = 'TotPendente'
      Active = True
      DisplayFormat = ',##0.00'
      Expression = 'sum(valorRec)'
    end
    object AggregateField6: TAggregateField
      Alignment = taRightJustify
      FieldName = 'TotPago'
      Active = True
      Expression = 'sum(valorrecebido)'
    end
  end
  object sqcCaixa: TSQLDataSet
    CommandText = 
      'select * from PLANO where contapai = '#39'1.1.1.01'#39' '#13#10'and  nome = :C' +
      'ONTA'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'CONTA'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Left = 360
    Top = 352
    object sqcCaixaCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Required = True
    end
    object sqcCaixaCONTA: TStringField
      FieldName = 'CONTA'
      Required = True
      Size = 15
    end
    object sqcCaixaCONTAPAI: TStringField
      FieldName = 'CONTAPAI'
      Size = 10
    end
    object sqcCaixaNOME: TStringField
      FieldName = 'NOME'
      Required = True
      Size = 200
    end
    object sqcCaixaCONSOLIDA: TStringField
      FieldName = 'CONSOLIDA'
      FixedChar = True
      Size = 1
    end
    object sqcCaixaDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 50
    end
    object sqcCaixaRATEIO: TStringField
      FieldName = 'RATEIO'
      FixedChar = True
      Size = 1
    end
    object sqcCaixaCODREDUZIDO: TStringField
      FieldName = 'CODREDUZIDO'
      Size = 15
    end
  end
  object dpsCaixa: TDataSetProvider
    DataSet = sqcCaixa
    Left = 392
    Top = 352
  end
  object cdsCaixa: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'CONTA'
        ParamType = ptInput
      end>
    ProviderName = 'dpsCaixa'
    Left = 424
    Top = 352
    object cdsCaixaCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Required = True
    end
    object cdsCaixaCONTA: TStringField
      FieldName = 'CONTA'
      Required = True
      Size = 15
    end
    object cdsCaixaCONTAPAI: TStringField
      FieldName = 'CONTAPAI'
      Size = 10
    end
    object cdsCaixaNOME: TStringField
      FieldName = 'NOME'
      Required = True
      Size = 200
    end
    object cdsCaixaCONSOLIDA: TStringField
      FieldName = 'CONSOLIDA'
      FixedChar = True
      Size = 1
    end
    object cdsCaixaDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 50
    end
    object cdsCaixaRATEIO: TStringField
      FieldName = 'RATEIO'
      FixedChar = True
      Size = 1
    end
    object cdsCaixaCODREDUZIDO: TStringField
      FieldName = 'CODREDUZIDO'
      Size = 15
    end
  end
  object JvDBGridExcelExport1: TJvDBGridExcelExport
    Caption = 'Exporting to MS Excel...'
    Grid = DBGrid1
    AutoFit = False
    Left = 216
    Top = 60
  end
end
