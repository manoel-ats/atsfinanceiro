inherited fNotasFaltas: TfNotasFaltas
  Width = 672
  Height = 655
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Label7: TLabel [0]
    Left = 120
    Top = 48
    Width = 54
    Height = 16
    Caption = 'Per'#237'odo :'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object Label1: TLabel [1]
    Left = 331
    Top = 50
    Width = 88
    Height = 16
    Caption = 'Classe/Turma:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object Label2: TLabel [2]
    Left = 120
    Top = 89
    Width = 65
    Height = 16
    Caption = 'Disciplina :'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object Label6: TLabel [3]
    Left = 425
    Top = 90
    Width = 84
    Height = 16
    Caption = 'Aulas Dadas :'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object Label5: TLabel [4]
    Left = 454
    Top = 138
    Width = 43
    Height = 16
    Caption = 'Faltas :'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object Label4: TLabel [5]
    Left = 357
    Top = 141
    Width = 76
    Height = 13
    Caption = 'Nota/Conceito :'
  end
  object Label3: TLabel [6]
    Left = 268
    Top = 138
    Width = 74
    Height = 16
    Caption = 'N'#186'Chamada'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object BitBtn1: TBitBtn [7]
    Left = 538
    Top = 183
    Width = 90
    Height = 38
    Caption = '&Gravar'
    TabOrder = 12
    OnClick = BitBtn1Click
    Glyph.Data = {
      
      C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
  end
  object btnNovo: TBitBtn [8]
    Left = 538
    Top = 221
    Width = 90
    Height = 38
    Caption = '&Novo'
    TabOrder = 14
    OnClick = btnNovoClick
    Glyph.Data = {
      
      C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
  end
  inherited MMJPanel1: TMMJPanel
    Width = 664
    Height = 41
    object Label8: TLabel
      Left = 16
      Top = 8
      Width = 12
      Height = 20
      Caption = '...'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
  end
  inherited MMJPanel2: TMMJPanel
    Left = -8
    Top = 571
    Align = alNone
    Visible = False
    inherited btnProcurar: TBitBtn
      Left = 44
    end
  end
  object dblPeriodo: TDBLookupComboBox [11]
    Left = 120
    Top = 64
    Width = 209
    Height = 24
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    KeyField = 'IDPERIODO'
    ListField = 'PERIODO'
    ListSource = DataSourcePeriodo
    ParentFont = False
    TabOrder = 2
    OnClick = dblPeriodoClick
    OnEnter = dblPeriodoEnter
    OnKeyPress = FormKeyPress
  end
  object dblSerie: TDBLookupComboBox [12]
    Left = 330
    Top = 64
    Width = 203
    Height = 24
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    KeyField = 'SERIELETRA'
    ListField = 'DESC_CLASSE'
    ListSource = dsSeries
    ParentFont = False
    TabOrder = 3
    OnClick = dblSerieClick
    OnEnter = dblSerieEnter
    OnKeyPress = FormKeyPress
  end
  object dblMateria: TDBLookupComboBox [13]
    Left = 120
    Top = 105
    Width = 300
    Height = 24
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    KeyField = 'IDMATERIAS'
    ListField = 'MATERIAS'
    ListSource = ds_materia
    ParentFont = False
    TabOrder = 4
    OnClick = dblMateriaClick
    OnEnter = dblMateriaEnter
    OnKeyPress = FormKeyPress
  end
  object Edit1: TEdit [14]
    Left = 425
    Top = 105
    Width = 105
    Height = 24
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 5
    OnKeyPress = FormKeyPress
  end
  object BitBtn2: TBitBtn [15]
    Left = 119
    Top = 140
    Width = 122
    Height = 38
    Caption = 'Digitar Notas'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 6
    OnClick = BitBtn2Click
    Glyph.Data = {
      
      F8880078FF0FF0FFFF88FFFFF08778FFFF88808FF0FFF0FFFFF8}
  end
  object btnExclui: TBitBtn [16]
    Left = 538
    Top = 259
    Width = 90
    Height = 38
    Caption = '&Excluir'
    TabOrder = 7
    OnClick = btnExcluiClick
    Glyph.Data = {
      
      C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
  end
  object Edit4: TEdit [17]
    Left = 453
    Top = 154
    Width = 84
    Height = 24
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 10
    OnKeyPress = FormKeyPress
  end
  object Edit3: TEdit [18]
    Left = 356
    Top = 154
    Width = 91
    Height = 24
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 9
    OnKeyPress = FormKeyPress
  end
  object Edit2: TEdit [19]
    Left = 268
    Top = 154
    Width = 81
    Height = 24
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 8
    OnExit = Edit2Exit
    OnKeyPress = FormKeyPress
  end
  object DBGrid1: TDBGrid [20]
    Left = 120
    Top = 183
    Width = 417
    Height = 369
    TabStop = False
    DataSource = DtSrc
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
    ReadOnly = True
    TabOrder = 11
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnCellClick = DBGrid1CellClick
    OnDrawColumnCell = DBGrid1DrawColumnCell
    OnEndDrag = DBGrid1EndDrag
    OnKeyDown = DBGrid1KeyDown
    OnKeyUp = DBGrid1KeyUp
    OnMouseDown = DBGrid1MouseDown
    OnMouseUp = DBGrid1MouseUp
    Columns = <
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'NUMERO'
        Title.Alignment = taCenter
        Title.Caption = 'N'#250'mero'
        Width = 80
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'NOTA'
        Title.Alignment = taCenter
        Title.Caption = 'Nota'
        Width = 80
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'FREQUENCIA'
        Title.Caption = 'Frequ'#234'ncia'
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'AULASDADAS'
        Title.Caption = 'Total Aulas'
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'TURNO'
        Title.Caption = 'Turno'
        Width = 80
        Visible = True
      end>
  end
  object BitBtn3: TBitBtn [21]
    Left = 538
    Top = 297
    Width = 90
    Height = 38
    Caption = 'F9-Sair'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    PopupMenu = PopupMenu1
    TabOrder = 13
    OnClick = btnSairClick
    Glyph.Data = {
      
      C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
  end
  inherited DtSrc: TDataSource
    DataSet = cds_nota
  end
  object sds_materia: TSQLDataSet
    CommandText = 'select * from TABMATERIAS where TIPOENSINO = :PTIPO'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'PTIPO'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Top = 56
  end
  object dsp_materia: TDataSetProvider
    DataSet = sds_materia
    Left = 29
    Top = 56
  end
  object cds_materia: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'PTIPO'
        ParamType = ptInput
      end>
    ProviderName = 'dsp_materia'
    Left = 57
    Top = 56
    object cds_materiaMATERIAS: TStringField
      FieldName = 'MATERIAS'
      Size = 50
    end
    object cds_materiaIDMATERIAS: TIntegerField
      FieldName = 'IDMATERIAS'
      Required = True
    end
    object cds_materiaTIPOENSINO: TStringField
      FieldName = 'TIPOENSINO'
      Size = 15
    end
  end
  object ds_materia: TDataSource
    DataSet = cds_materia
    Left = 84
    Top = 56
  end
  object ds_nota: TDataSource
    DataSet = cds_nota
    Left = 85
    Top = 88
  end
  object cds_nota: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Name = 'pPER'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'pCLASSE'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'pMAT'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'pANO'
        ParamType = ptInput
      end>
    ProviderName = 'dsp_nota'
    Left = 57
    Top = 88
    object cds_notaIDNOTAFREQUENCIA: TIntegerField
      FieldName = 'IDNOTAFREQUENCIA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object cds_notaSERIELETRA: TStringField
      FieldName = 'SERIELETRA'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object cds_notaIDMATERIAS: TIntegerField
      FieldName = 'IDMATERIAS'
      ProviderFlags = [pfInUpdate]
    end
    object cds_notaTURNO: TStringField
      FieldName = 'TURNO'
      ProviderFlags = [pfInUpdate]
      Size = 7
    end
    object cds_notaRA: TStringField
      FieldName = 'RA'
      ProviderFlags = [pfInUpdate]
      Size = 10
    end
    object cds_notaNOTA: TStringField
      FieldName = 'NOTA'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object cds_notaFREQUENCIA: TSmallintField
      FieldName = 'FREQUENCIA'
      ProviderFlags = [pfInUpdate]
    end
    object cds_notaANOLETIVO: TIntegerField
      FieldName = 'ANOLETIVO'
      ProviderFlags = [pfInUpdate]
    end
    object cds_notaSERIE: TStringField
      FieldName = 'SERIE'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object cds_notaIDPERIODO: TIntegerField
      FieldName = 'IDPERIODO'
      ProviderFlags = [pfInUpdate]
    end
    object cds_notaAULASDADAS: TSmallintField
      FieldName = 'AULASDADAS'
      ProviderFlags = [pfInUpdate]
    end
    object cds_notaNUMERO: TSmallintField
      FieldName = 'NUMERO'
      ProviderFlags = []
    end
  end
  object dsp_nota: TDataSetProvider
    DataSet = sds_nota
    UpdateMode = upWhereKeyOnly
    Left = 28
    Top = 88
  end
  object sds_nota: TSQLDataSet
    CommandText = 
      'select mov.*, cla.NUMERO '#13#10'from TABMOVNOTASFREQ mov '#13#10'inner join' +
      ' TABCLASSEALUNO cla on cla.RA = mov.RA '#13#10'where mov.IDPERIODO = :' +
      'pPER '#13#10'and mov.SERIELETRA = :pCLASSE '#13#10'and mov.IDMATERIAS = :pMA' +
      'T '#13#10'and mov.ANOLETIVO = :pANO '#13#10'order by cla.NUMERO'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftUnknown
        Name = 'pPER'
        ParamType = ptInput
      end
      item
        DataType = ftUnknown
        Name = 'pCLASSE'
        ParamType = ptInput
      end
      item
        DataType = ftUnknown
        Name = 'pMAT'
        ParamType = ptInput
      end
      item
        DataType = ftUnknown
        Name = 'pANO'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Top = 88
    object sds_notaIDNOTAFREQUENCIA: TIntegerField
      FieldName = 'IDNOTAFREQUENCIA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object sds_notaSERIELETRA: TStringField
      FieldName = 'SERIELETRA'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object sds_notaIDMATERIAS: TIntegerField
      FieldName = 'IDMATERIAS'
      ProviderFlags = [pfInUpdate]
    end
    object sds_notaTURNO: TStringField
      FieldName = 'TURNO'
      ProviderFlags = [pfInUpdate]
      Size = 7
    end
    object sds_notaRA: TStringField
      FieldName = 'RA'
      ProviderFlags = [pfInUpdate]
      Size = 10
    end
    object sds_notaNOTA: TStringField
      FieldName = 'NOTA'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object sds_notaFREQUENCIA: TSmallintField
      FieldName = 'FREQUENCIA'
      ProviderFlags = [pfInUpdate]
    end
    object sds_notaANOLETIVO: TIntegerField
      FieldName = 'ANOLETIVO'
      ProviderFlags = [pfInUpdate]
    end
    object sds_notaSERIE: TStringField
      FieldName = 'SERIE'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object sds_notaIDPERIODO: TIntegerField
      FieldName = 'IDPERIODO'
      ProviderFlags = [pfInUpdate]
    end
    object sds_notaAULASDADAS: TSmallintField
      FieldName = 'AULASDADAS'
      ProviderFlags = [pfInUpdate]
    end
    object sds_notaNUMERO: TSmallintField
      FieldName = 'NUMERO'
      ProviderFlags = []
    end
  end
  object sds_classe: TSQLDataSet
    CommandText = 
      'select cla.*, alu.NOMECLIENTE as NOME from TABCLASSEALUNO cla in' +
      'ner join CLIENTES  alu on alu.RA = cla.RA where cla.SERIELETRA =' +
      ' :pClasse AND alu.SITUACAOESCOLAR = '#39'M'#39#13#10
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'pClasse'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Top = 120
  end
  object dsp_classe: TDataSetProvider
    DataSet = sds_classe
    Left = 29
    Top = 120
  end
  object cds_classe: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'pClasse'
        ParamType = ptInput
      end>
    ProviderName = 'dsp_classe'
    Left = 57
    Top = 120
    object cds_classeSERIELETRA: TStringField
      FieldName = 'SERIELETRA'
      Size = 4
    end
    object cds_classeRA: TStringField
      FieldName = 'RA'
      Size = 10
    end
    object cds_classeIDCLASSEALUNO: TIntegerField
      FieldName = 'IDCLASSEALUNO'
      Required = True
    end
    object cds_classeSERIE: TStringField
      FieldName = 'SERIE'
      Size = 4
    end
    object cds_classeNUMERO: TSmallintField
      FieldName = 'NUMERO'
    end
    object cds_classeNOME: TStringField
      FieldName = 'NOME'
      Size = 50
    end
  end
  object sds_boletim: TSQLDataSet
    CommandText = 'select * from TABBOLETIM where IDBOLETIM = :pBOL'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'pBOL'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Left = 600
    Top = 56
    object sds_boletimRA: TStringField
      FieldName = 'RA'
      ProviderFlags = [pfInUpdate]
      Size = 10
    end
    object sds_boletimSERIELETRA: TStringField
      FieldName = 'SERIELETRA'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object sds_boletimIDMATERIAS: TIntegerField
      FieldName = 'IDMATERIAS'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimIDBOLETIM: TStringField
      FieldName = 'IDBOLETIM'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object sds_boletimNOTAPER1: TStringField
      FieldName = 'NOTAPER1'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object sds_boletimNOTAPER2: TStringField
      FieldName = 'NOTAPER2'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object sds_boletimNOTAPER3: TStringField
      FieldName = 'NOTAPER3'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object sds_boletimNOTAPER4: TStringField
      FieldName = 'NOTAPER4'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object sds_boletimNOTAPER5: TStringField
      FieldName = 'NOTAPER5'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object sds_boletimNOTAPER6: TStringField
      FieldName = 'NOTAPER6'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object sds_boletimFALTAPER1: TSmallintField
      FieldName = 'FALTAPER1'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimFALTAPER2: TSmallintField
      FieldName = 'FALTAPER2'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimFALTAPER3: TSmallintField
      FieldName = 'FALTAPER3'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimFALTAPER4: TSmallintField
      FieldName = 'FALTAPER4'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimFALTAPER5: TSmallintField
      FieldName = 'FALTAPER5'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimFALTAPER6: TSmallintField
      FieldName = 'FALTAPER6'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimAULADADAPER1: TSmallintField
      FieldName = 'AULADADAPER1'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimAULADADAPER2: TSmallintField
      FieldName = 'AULADADAPER2'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimAULADADAPER3: TSmallintField
      FieldName = 'AULADADAPER3'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimAULADADAPER4: TSmallintField
      FieldName = 'AULADADAPER4'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimAULADADAPER5: TSmallintField
      FieldName = 'AULADADAPER5'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimAULADADAPER6: TSmallintField
      FieldName = 'AULADADAPER6'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimANOLETIVO: TIntegerField
      FieldName = 'ANOLETIVO'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimNOTAPER7: TStringField
      FieldName = 'NOTAPER7'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object sds_boletimNOTAPER8: TStringField
      FieldName = 'NOTAPER8'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object sds_boletimNOTAPER9: TStringField
      FieldName = 'NOTAPER9'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object sds_boletimNOTAPER10: TStringField
      FieldName = 'NOTAPER10'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object sds_boletimNOTAPER11: TStringField
      FieldName = 'NOTAPER11'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object sds_boletimNOTAPER12: TStringField
      FieldName = 'NOTAPER12'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object sds_boletimFALTAPER7: TSmallintField
      FieldName = 'FALTAPER7'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimFALTAPER8: TSmallintField
      FieldName = 'FALTAPER8'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimFALTAPER9: TSmallintField
      FieldName = 'FALTAPER9'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimFALTAPER10: TSmallintField
      FieldName = 'FALTAPER10'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimFALTAPER11: TSmallintField
      FieldName = 'FALTAPER11'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimFALTAPER12: TSmallintField
      FieldName = 'FALTAPER12'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimAULADADAPER7: TSmallintField
      FieldName = 'AULADADAPER7'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimAULADADAPER8: TSmallintField
      FieldName = 'AULADADAPER8'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimAULADADAPER9: TSmallintField
      FieldName = 'AULADADAPER9'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimAULADADAPER10: TSmallintField
      FieldName = 'AULADADAPER10'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimAULADADAPER11: TSmallintField
      FieldName = 'AULADADAPER11'
      ProviderFlags = [pfInUpdate]
    end
    object sds_boletimAULADADAPER12: TSmallintField
      FieldName = 'AULADADAPER12'
      ProviderFlags = [pfInUpdate]
    end
  end
  object dsp_boletim: TDataSetProvider
    DataSet = sds_boletim
    UpdateMode = upWhereKeyOnly
    Left = 600
    Top = 88
  end
  object cds_boletim: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'pBOL'
        ParamType = ptInput
      end>
    ProviderName = 'dsp_boletim'
    Left = 632
    Top = 88
    object cds_boletimRA: TStringField
      FieldName = 'RA'
      ProviderFlags = [pfInUpdate]
      Size = 10
    end
    object cds_boletimSERIELETRA: TStringField
      FieldName = 'SERIELETRA'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object cds_boletimIDMATERIAS: TIntegerField
      FieldName = 'IDMATERIAS'
      ProviderFlags = [pfInUpdate]
    end
    object cds_boletimIDBOLETIM: TStringField
      FieldName = 'IDBOLETIM'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object cds_boletimNOTAPER1: TStringField
      FieldName = 'NOTAPER1'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object cds_boletimNOTAPER2: TStringField
      FieldName = 'NOTAPER2'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object cds_boletimNOTAPER3: TStringField
      FieldName = 'NOTAPER3'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object cds_boletimNOTAPER4: TStringField
      FieldName = 'NOTAPER4'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object cds_boletimNOTAPER5: TStringField
      FieldName = 'NOTAPER5'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object cds_boletimNOTAPER6: TStringField
      FieldName = 'NOTAPER6'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object cds_boletimFALTAPER1: TSmallintField
      FieldName = 'FALTAPER1'
      ProviderFlags = [pfInUpdate]
    end
    object cds_boletimFALTAPER2: TSmallintField
      FieldName = 'FALTAPER2'
      ProviderFlags = [pfInUpdate]
    end
    object cds_boletimFALTAPER3: TSmallintField
      FieldName = 'FALTAPER3'
      ProviderFlags = [pfInUpdate]
    end
    object cds_boletimFALTAPER4: TSmallintField
      FieldName = 'FALTAPER4'
      ProviderFlags = [pfInUpdate]
    end
    object cds_boletimFALTAPER5: TSmallintField
      FieldName = 'FALTAPER5'
      ProviderFlags = [pfInUpdate]
    end
    object cds_boletimFALTAPER6: TSmallintField
      FieldName = 'FALTAPER6'
      ProviderFlags = [pfInUpdate]
    end
    object cds_boletimAULADADAPER1: TSmallintField
      FieldName = 'AULADADAPER1'
      ProviderFlags = [pfInUpdate]
    end
    object cds_boletimAULADADAPER2: TSmallintField
      FieldName = 'AULADADAPER2'
      ProviderFlags = [pfInUpdate]
    end
    object cds_boletimAULADADAPER3: TSmallintField
      FieldName = 'AULADADAPER3'
      ProviderFlags = [pfInUpdate]
    end
    object cds_boletimAULADADAPER4: TSmallintField
      FieldName = 'AULADADAPER4'
      ProviderFlags = [pfInUpdate]
    end
    object cds_boletimAULADADAPER5: TSmallintField
      FieldName = 'AULADADAPER5'
      ProviderFlags = [pfInUpdate]
    end
    object cds_boletimAULADADAPER6: TSmallintField
      FieldName = 'AULADADAPER6'
      ProviderFlags = [pfInUpdate]
    end
    object cds_boletimANOLETIVO: TIntegerField
      FieldName = 'ANOLETIVO'
      ProviderFlags = [pfInUpdate]
    end
    object cds_boletimNOTAPER7: TStringField
      FieldName = 'NOTAPER7'
      Size = 4
    end
    object cds_boletimNOTAPER8: TStringField
      FieldName = 'NOTAPER8'
      Size = 4
    end
    object cds_boletimNOTAPER9: TStringField
      FieldName = 'NOTAPER9'
      Size = 4
    end
    object cds_boletimNOTAPER10: TStringField
      FieldName = 'NOTAPER10'
      Size = 4
    end
    object cds_boletimNOTAPER11: TStringField
      FieldName = 'NOTAPER11'
      Size = 4
    end
    object cds_boletimNOTAPER12: TStringField
      FieldName = 'NOTAPER12'
      Size = 4
    end
    object cds_boletimFALTAPER7: TSmallintField
      FieldName = 'FALTAPER7'
    end
    object cds_boletimFALTAPER8: TSmallintField
      FieldName = 'FALTAPER8'
    end
    object cds_boletimFALTAPER9: TSmallintField
      FieldName = 'FALTAPER9'
    end
    object cds_boletimFALTAPER10: TSmallintField
      FieldName = 'FALTAPER10'
    end
    object cds_boletimFALTAPER11: TSmallintField
      FieldName = 'FALTAPER11'
    end
    object cds_boletimFALTAPER12: TSmallintField
      FieldName = 'FALTAPER12'
    end
    object cds_boletimAULADADAPER7: TSmallintField
      FieldName = 'AULADADAPER7'
    end
    object cds_boletimAULADADAPER8: TSmallintField
      FieldName = 'AULADADAPER8'
    end
    object cds_boletimAULADADAPER9: TSmallintField
      FieldName = 'AULADADAPER9'
    end
    object cds_boletimAULADADAPER10: TSmallintField
      FieldName = 'AULADADAPER10'
    end
    object cds_boletimAULADADAPER11: TSmallintField
      FieldName = 'AULADADAPER11'
    end
    object cds_boletimAULADADAPER12: TSmallintField
      FieldName = 'AULADADAPER12'
    end
  end
  object sds_aluno_inserido: TSQLDataSet
    CommandText = 
      'select RA from TABMOVNOTASFREQ '#13#10'where IDPERIODO = :pPER and IDM' +
      'ATERIAS = :pMAT and ANOLETIVO = :pANO and  RA = :pRA'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'pPER'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'pMAT'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'pANO'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'pRA'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Left = 632
    Top = 56
    object sds_aluno_inseridoRA: TStringField
      FieldName = 'RA'
      Size = 10
    end
  end
  object sds_escola: TSQLDataSet
    CommandText = 'select MEDIA_ESCOLA from EMPRESA'
    MaxBlobSize = -1
    Params = <>
    SQLConnection = DM.sqlsisAdimin
    Left = 632
    Top = 264
    object sds_escolaMEDIA_ESCOLA: TFloatField
      FieldName = 'MEDIA_ESCOLA'
    end
  end
  object sdsTablePeriodo: TSQLDataSet
    CommandText = 'select * from TABPERIODO'
    MaxBlobSize = -1
    Params = <>
    SQLConnection = DM.sqlsisAdimin
    Left = 544
    Top = 416
    object sdsTablePeriodoIDPERIODO: TIntegerField
      FieldName = 'IDPERIODO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object sdsTablePeriodoPERIODO: TStringField
      FieldName = 'PERIODO'
      ProviderFlags = [pfInUpdate]
      Size = 15
    end
    object sdsTablePeriodoNOTAFINAL: TStringField
      FieldName = 'NOTAFINAL'
      ProviderFlags = [pfInUpdate]
      FixedChar = True
      Size = 3
    end
  end
  object TableNomeEscola: TSQLDataSet
    CommandText = 'select * from EMPRESA'
    MaxBlobSize = -1
    Params = <>
    SQLConnection = DM.sqlsisAdimin
    Left = 576
    Top = 416
    object TableNomeEscolaEMPRESA: TStringField
      FieldName = 'EMPRESA'
      Required = True
      Size = 80
    end
    object TableNomeEscolaRAZAO: TStringField
      FieldName = 'RAZAO'
      Required = True
      Size = 80
    end
    object TableNomeEscolaCNPJ_CPF: TStringField
      FieldName = 'CNPJ_CPF'
      Required = True
    end
    object TableNomeEscolaENDERECO: TStringField
      FieldName = 'ENDERECO'
      Size = 80
    end
    object TableNomeEscolaLOGRADOURO: TStringField
      FieldName = 'LOGRADOURO'
      Size = 80
    end
    object TableNomeEscolaBAIRRO: TStringField
      FieldName = 'BAIRRO'
      Size = 40
    end
    object TableNomeEscolaCIDADE: TStringField
      FieldName = 'CIDADE'
      Size = 50
    end
    object TableNomeEscolaUF: TStringField
      FieldName = 'UF'
      FixedChar = True
      Size = 2
    end
    object TableNomeEscolaCEP: TStringField
      FieldName = 'CEP'
      FixedChar = True
      Size = 9
    end
    object TableNomeEscolaDDD: TStringField
      FieldName = 'DDD'
      FixedChar = True
      Size = 2
    end
    object TableNomeEscolaFONE: TStringField
      FieldName = 'FONE'
      Size = 12
    end
    object TableNomeEscolaFONE_1: TStringField
      FieldName = 'FONE_1'
      Size = 12
    end
    object TableNomeEscolaFONE_2: TStringField
      FieldName = 'FONE_2'
      Size = 12
    end
    object TableNomeEscolaFAX: TStringField
      FieldName = 'FAX'
      Size = 12
    end
    object TableNomeEscolaE_MAIL: TStringField
      FieldName = 'E_MAIL'
      Size = 50
    end
    object TableNomeEscolaWEB: TStringField
      FieldName = 'WEB'
      Size = 50
    end
    object TableNomeEscolaLOGOTIPO: TGraphicField
      FieldName = 'LOGOTIPO'
      BlobType = ftGraphic
    end
    object TableNomeEscolaSLOGAN: TStringField
      FieldName = 'SLOGAN'
      Size = 60
    end
    object TableNomeEscolaOUTRAS_INFO: TStringField
      FieldName = 'OUTRAS_INFO'
      Size = 60
    end
    object TableNomeEscolaCODIGO: TIntegerField
      FieldName = 'CODIGO'
    end
    object TableNomeEscolaTIPO: TStringField
      FieldName = 'TIPO'
      FixedChar = True
      Size = 1
    end
    object TableNomeEscolaIE_RG: TStringField
      FieldName = 'IE_RG'
      Size = 15
    end
    object TableNomeEscolaCODIGO_CONTA: TIntegerField
      FieldName = 'CODIGO_CONTA'
    end
    object TableNomeEscolaDIVERSOS1: TStringField
      FieldName = 'DIVERSOS1'
      Size = 50
    end
    object TableNomeEscolaDIVERSOS2: TStringField
      FieldName = 'DIVERSOS2'
      Size = 50
    end
    object TableNomeEscolaDIVERSOS3: TStringField
      FieldName = 'DIVERSOS3'
      Size = 50
    end
    object TableNomeEscolaANOLETIVO: TIntegerField
      FieldName = 'ANOLETIVO'
    end
    object TableNomeEscolaMEDIA_ESCOLA: TFloatField
      FieldName = 'MEDIA_ESCOLA'
    end
    object TableNomeEscolaPORTA: TIntegerField
      FieldName = 'PORTA'
    end
    object TableNomeEscolaSMTP: TStringField
      FieldName = 'SMTP'
      Size = 30
    end
    object TableNomeEscolaSENHA: TStringField
      FieldName = 'SENHA'
      Size = 10
    end
  end
  object dspTablePeriodo: TDataSetProvider
    DataSet = sdsTablePeriodo
    Left = 544
    Top = 445
  end
  object DataSetProvider2: TDataSetProvider
    Left = 576
    Top = 445
  end
  object TablePeriodo: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspTablePeriodo'
    Left = 544
    Top = 474
    object TablePeriodoIDPERIODO: TIntegerField
      FieldName = 'IDPERIODO'
      Required = True
    end
    object TablePeriodoPERIODO: TStringField
      FieldName = 'PERIODO'
      Size = 15
    end
    object TablePeriodoNOTAFINAL: TStringField
      FieldName = 'NOTAFINAL'
      FixedChar = True
      Size = 3
    end
  end
  object DataSourcePeriodo: TDataSource
    DataSet = TablePeriodo
    Left = 544
    Top = 504
  end
  object dsSeries: TDataSource
    DataSet = DM.TableSeries
    Left = 540
    Top = 56
  end
end
