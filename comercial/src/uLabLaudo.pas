...
MessageDlg('Já Existe Resultado Cadastrado '+#13+#10+'Voce somente podera  Alterar ?', mtInformation, [mbOK], 0);
  ...
  if (cdsTIPOEXAME.AsString = 'TEMPO DE SANGRAMENTO (TS)') then
    ...
    if (cdsTIPOEXAME.AsString = 'CURVA GLICÊMICA') then
    begin
  ...
  if (cdsTIPOEXAME.AsString = 'SECREÇÃO VAGINAL') then
    begin
    GroupBox28.Enabled := True;
      ...
      if (cdsTIPOEXAME.AsString = 'FEZES À FRESCO') then
      begin
         if(TabSheet7.Visible = False)then
         DBEdit141.setfocus;
      end;

       if (cdsTIPOEXAME.AsString = 'MICOLÓGICO DIRETO') then
      begin
         if(TabSheet7.Visible = False)then
         DBEdit138.setfocus;
      end;
...
begin
  inherited;
      ...
      begin
         ...
         TabSheet10.Caption := 'FEZES À FRESCO';
