...
begin
  ...
  if scdsCr_proc.Active then
   ...
   sqltexto1 := sqltexto1 + ' rec.VALOR_RESTO, rec.VALOR_PRIM_VIA, rec.STATUS ,rec.juros , ';
   ...
   sqltexto1 := sqltexto1 + '(rec.DESCONTO ) as VALORREC '; //rec.VALOR_RESTO - rec.VALORRECEBIDO - - rec.PERDA
...
begin
  ...
  if cCr.Active then
   ...
   sqltexto1 := sqltexto1 + 'Sum((rec.VALOR_RESTO - rec.VALORRECEBIDO - rec.DESCONTO - rec.PERDA)/2) as VALOR_RESTO, '; //'Sum(rec.VALOR_RESTO) as VALOR_RESTO, ';//
 ...
 if (cbStatus.Text <> '') then
  ...
  impCR := sqlTexto1;
  cCR.CommandText := sqlTexto1 + SqlCr + ' group by rec.DATAVENCIMENTO, cli.RA, cli.NOMECLIENTE, cli.RAZAOSOCIAL ,rec.BL ,rec.juros'; //rec.DATAVENCIMENTO;';
  //sds.CommandText := scdsCr_proc.CommandText;
  cCR.Open;
