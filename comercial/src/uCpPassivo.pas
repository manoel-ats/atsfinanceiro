...
var
  fCpPassivo: TfCpPassivo;
  SqlTexto, sqlTexto1: String;

implementation

uses UDm, uPassivo, uProcurar, ufDlgLogin, UUtils, sCtrlResize, ufcrtitulo;
...
procedure TfCpPassivo.BitBtn11Click(Sender: TObject);
begin
     edCodVendedor.Text:='';
     edVendedor.Text:='';
end;

procedure TfCpPassivo.BitBtn9Click(Sender: TObject);
begin
   if dm.cds_4_pagar.Active then
      dm.cds_4_pagar.Close;
   dm.cds_4_pagar.Params[0].AsInteger := fCpPassivo.scdsCr_procCODPAGAMENTO.AsInteger;
   dm.cds_4_pagar.Open;
   dm.cds_4_pagar.Edit;
   close;
end;

procedure TfCpPassivo.BitBtn4Click(Sender: TObject);
begin
   cbStatus.Text := '';
end;

procedure TfCpPassivo.btnIncluirClick(Sender: TObject);
...
procedure TfCpPassivo.btnProcurarClick(Sender: TObject);
 Var
...
begin
  ...
  sqlTexto1 := 'select rec.dp, rec.dup_rec_nf, rec.CODPAGAMENTO, rec.TITULO, rec.EMISSAO, '
   ...
   ' inner join MOVIMENTOCONT mc on mc.CODORIGEM = rec.CODPAGAMENTO ';
   sqlTexto1 := sqlTexto1 + ' inner join PLANO plano on plano.CODIGO = rec.CONTACREDITO ';
   ...
   begin
     ...
     begin
       ...
       dm.cds_parametro.Params[0].AsString := 'CONTASPASSIVO';
   ...
   if (sqlTexto = '') then
     sqlTexto := ' Where mc.TIPOORIGEM = ' + QuotedStr('PASSIVO')
   else
     sqlTexto := ' and mc.TIPOORIGEM = ' + QuotedStr('PASSIVO');
  //==============================================================================
  datastr:='  /  /  ';
  //------------------------------------------------------------------------------
...
procedure TfCpPassivo.FormCreate(Sender: TObject);
...
procedure TfCpPassivo.btnSairClick(Sender: TObject);
begin
  close;
end;

procedure TfCpPassivo.DBGrid1TitleClick(Column: TColumn);
begin
     scdsCr_proc.IndexFieldNames := Column.FieldName;
end;

procedure TfCpPassivo.btnImprimirClick(Sender: TObject);
  var
   sqlGrupo: String;
begin
...
procedure TfCpPassivo.BitBtn8Click(Sender: TObject);
begin
...
procedure TfCpPassivo.BitBtn1Click(Sender: TObject);
begin
  edCodCliente.Text:='';
     edCliente.Text:='';
end;

procedure TfCpPassivo.BitBtn12Click(Sender: TObject);
begin
...
procedure TfCpPassivo.edCodClienteExit(Sender: TObject);
begin
...
procedure TfCpPassivo.cbStatusKeyPress(Sender: TObject; var Key: Char);
begin
 if (key = #13) then
 begin
   //key:= #0;
...
procedure TfCpPassivo.BitBtn6Click(Sender: TObject);
begin
  medta5.Clear;
  medta6.Clear;  
end;

procedure TfCpPassivo.BitBtn7Click(Sender: TObject);
begin
  medta3.Clear;
  medta4.Clear;
end;

procedure TfCpPassivo.BitBtn2Click(Sender: TObject);
begin
  medta1.Clear;
  medta2.Clear;  
end;

procedure TfCpPassivo.ChkDBGridCellClick(Column: TColumn);
var
  Field: TField;
begin
  Field := Column.Field;
...
procedure TfCpPassivo.ChkDBGridColEnter(DBGrid: TDBGrid);
var
  Field: TField;
begin
...
procedure TfCpPassivo.ChkDBGridColExit(DBGrid: TDBGrid);
var
  Field: TField;
begin
  Field := DBGrid.SelectedField;
...
procedure TfCpPassivo.ChkDBGridDrawColumnCell(DBGrid: TDBGrid;
  const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
var
  X, Y, Index: integer;
...
procedure TfCpPassivo.ChkDBGridKeyPress(DBGrid: TDBGrid; var Key: Char);
var
  Field: TField;
begin
  Field := DBGrid.SelectedField;
...
procedure TfCpPassivo.DBGrid1CellClick(Column: TColumn);
begin
  ChkDBGridCellClick(Column);
end;

procedure TfCpPassivo.DBGrid1ColEnter(Sender: TObject);
begin
  ChkDBGridColEnter(Sender as TDBGrid);
end;

procedure TfCpPassivo.DBGrid1ColExit(Sender: TObject);
begin
 ChkDBGridColEnter(Sender as TDBGrid);
end;

procedure TfCpPassivo.DBGrid1DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
begin
//***************************************************
...
procedure TfCpPassivo.meDta1KeyPress(Sender: TObject; var Key: Char);
begin
  if (key = #13) then
  begin
    key:= #0;
...
procedure TfCpPassivo.DBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
     ChkDBGridKeyPress(Sender as TDBGrid, Key);
end;

procedure TfCpPassivo.edValorKeyPress(Sender: TObject; var Key: Char);
begin
  if Key in [','] then Key := '.';
end;

procedure TfCpPassivo.BitBtn3Click(Sender: TObject);
begin
  edValor.Text := '';
end;
