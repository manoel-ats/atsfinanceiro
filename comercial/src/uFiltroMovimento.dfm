object fFiltroMovimento: TfFiltroMovimento
  Left = 7
  Top = 1
  Width = 800
  Height = 552
  Align = alCustom
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  PopupMenu = PopupMenu1
  Position = poDesktopCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object MMJPanel1: TMMJPanel
    Left = 0
    Top = 0
    Width = 792
    Height = 46
    Align = alTop
    BevelInner = bvLowered
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindow
    Font.Height = -29
    Font.Name = 'Cooper Blk BT'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    Silhuette.Shape.ShapeText = 'Shape text'
    Silhuette.PictureShape.Picture.Data = {07544269746D617000000000}
    Silhuette.PictureShape.DisplayPicture = True
    Background.StartColor = clSilver
    Background.EndColor = clTeal
    Background.FillType = GradUpDown
    object Label10: TJvLabel
      Left = 2
      Top = 2
      Width = 788
      Height = 42
      Align = alClient
      Alignment = taCenter
      Caption = 'Pesquisa Vendas / Or'#231'amentos'
      ShadowColor = clNavy
      ShadowSize = 4
      Transparent = True
      HotTrackFont.Charset = DEFAULT_CHARSET
      HotTrackFont.Color = clWindowText
      HotTrackFont.Height = -29
      HotTrackFont.Name = 'Cooper Blk BT'
      HotTrackFont.Style = []
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 46
    Width = 792
    Height = 132
    Align = alTop
    PopupMenu = PopupMenu1
    TabOrder = 1
    object Label1: TLabel
      Left = 1
      Top = 47
      Width = 33
      Height = 13
      Caption = 'C'#243'digo'
    end
    object Label2: TLabel
      Left = 84
      Top = 47
      Width = 63
      Height = 13
      Caption = 'Nome Cliente'
    end
    object Label3: TLabel
      Left = 1
      Top = 89
      Width = 45
      Height = 13
      Caption = 'C'#243'd. Nat.'
    end
    object Label4: TLabel
      Left = 82
      Top = 89
      Width = 93
      Height = 13
      Caption = 'Natureza Opera'#231#227'o'
    end
    object Label7: TLabel
      Left = 584
      Top = 47
      Width = 43
      Height = 13
      Caption = 'N'#176' T'#237'tulo'
    end
    object Label8: TLabel
      Left = 674
      Top = 47
      Width = 24
      Height = 13
      Caption = 'S'#233'rie'
    end
    object Label5: TLabel
      Left = 2
      Top = 3
      Width = 76
      Height = 13
      Caption = 'Centro de Custo'
    end
    object Label6: TLabel
      Left = 368
      Top = 89
      Width = 34
      Height = 13
      Caption = 'Regi'#227'o'
    end
    object Edit1: TEdit
      Left = 95
      Top = 21
      Width = 99
      Height = 21
      TabOrder = 17
      Text = 'Edit1'
      Visible = False
    end
    object Edit5: TEdit
      Left = 383
      Top = 107
      Width = 99
      Height = 21
      TabOrder = 20
      Visible = False
    end
    object ComboBox1: TComboBox
      Left = 1
      Top = 19
      Width = 217
      Height = 24
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ItemHeight = 16
      ParentFont = False
      TabOrder = 0
      OnKeyPress = ComboBox1KeyPress
    end
    object edControle: TEdit
      Left = 1
      Top = 63
      Width = 78
      Height = 24
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      PopupMenu = PopupMenu1
      TabOrder = 1
      OnExit = edControleExit
      OnKeyPress = edControleKeyPress
    end
    object Edit2: TEdit
      Left = 81
      Top = 63
      Width = 400
      Height = 24
      TabStop = False
      Color = clInfoBk
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      PopupMenu = PopupMenu1
      ReadOnly = True
      TabOrder = 8
      OnKeyPress = FormKeyPress
    end
    object BitBtn2: TBitBtn
      Left = 483
      Top = 49
      Width = 42
      Height = 39
      PopupMenu = PopupMenu1
      TabOrder = 10
      TabStop = False
      OnClick = BitBtn2Click
      Glyph.Data = {
        
        C0C0C0C0C0C0C0C00000}
    end
    object BitBtn1: TBitBtn
      Left = 525
      Top = 49
      Width = 42
      Height = 39
      PopupMenu = PopupMenu1
      TabOrder = 9
      TabStop = False
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        C0C0C0C0C0C0C0C00000}
    end
    object Edit3: TEdit
      Left = 1
      Top = 105
      Width = 78
      Height = 24
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      PopupMenu = PopupMenu1
      TabOrder = 4
      OnExit = Edit3Exit
      OnKeyPress = ComboBox1KeyPress
    end
    object Edit4: TEdit
      Left = 81
      Top = 105
      Width = 200
      Height = 24
      TabStop = False
      Color = clInfoBk
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      PopupMenu = PopupMenu1
      ReadOnly = True
      TabOrder = 11
      OnKeyPress = FormKeyPress
    end
    object BitBtn3: TBitBtn
      Left = 282
      Top = 91
      Width = 42
      Height = 39
      PopupMenu = PopupMenu1
      TabOrder = 12
      TabStop = False
      OnClick = BitBtn3Click
      Glyph.Data = {
        
        C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
    end
    object BitBtn4: TBitBtn
      Left = 324
      Top = 91
      Width = 42
      Height = 39
      PopupMenu = PopupMenu1
      TabOrder = 13
      TabStop = False
      OnClick = BitBtn4Click
      Glyph.Data = {
        
        0000}
    end
    object BitBtn6: TBitBtn
      Left = 220
      Top = 5
      Width = 42
      Height = 39
      PopupMenu = PopupMenu1
      TabOrder = 14
      TabStop = False
      OnClick = BitBtn6Click
      Glyph.Data = {
        
        0000}
    end
    object edtNF: TEdit
      Left = 584
      Top = 63
      Width = 86
      Height = 24
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      PopupMenu = PopupMenu1
      TabOrder = 2
      OnKeyPress = FormKeyPress
    end
    object Edit8: TEdit
      Left = 673
      Top = 63
      Width = 73
      Height = 24
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      PopupMenu = PopupMenu1
      TabOrder = 3
      OnKeyPress = ComboBox1KeyPress
    end
    object BitBtn7: TBitBtn
      Left = 747
      Top = 48
      Width = 42
      Height = 39
      PopupMenu = PopupMenu1
      TabOrder = 15
      TabStop = False
      OnClick = BitBtn7Click
      Glyph.Data = {
        
        0000}
    end
    object BitBtn10: TBitBtn
      Left = 748
      Top = 92
      Width = 42
      Height = 39
      PopupMenu = PopupMenu1
      TabOrder = 16
      TabStop = False
      OnClick = BitBtn10Click
      Glyph.Data = {
        
        0000}
    end
    object RadioGroup1: TRadioGroup
      Left = 387
      Top = 1
      Width = 401
      Height = 43
      Caption = 'Finalizado'
      Columns = 4
      ItemIndex = 3
      Items.Strings = (
        'Or'#231'amento'
        'Pedido'
        'Venda'
        'Todos')
      PopupMenu = PopupMenu1
      TabOrder = 18
      OnClick = RadioGroup1Click
    end
    object GroupBox5: TGroupBox
      Left = 266
      Top = 3
      Width = 119
      Height = 41
      Caption = 'Placa'
      PopupMenu = PopupMenu1
      TabOrder = 19
      Visible = False
      object MaskEdit1: TMaskEdit
        Left = 8
        Top = 13
        Width = 77
        Height = 24
        EditMask = '>LLL-0000;1;_'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        MaxLength = 8
        ParentFont = False
        PopupMenu = PopupMenu1
        TabOrder = 0
        Text = '   -    '
        OnKeyPress = FormKeyPress
      end
      object BitBtn11: TBitBtn
        Left = 85
        Top = 7
        Width = 31
        Height = 31
        PopupMenu = PopupMenu1
        TabOrder = 1
        TabStop = False
        OnClick = BitBtn11Click
        Glyph.Data = {
          
          0000}
      end
    end
    object ComboBox2: TComboBox
      Left = 367
      Top = 105
      Width = 214
      Height = 24
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ItemHeight = 16
      ParentFont = False
      TabOrder = 5
      OnKeyPress = ComboBox1KeyPress
    end
    object meDta1: TJvDateEdit
      Left = 584
      Top = 106
      Width = 82
      Height = 21
      TabOrder = 6
    end
    object meDta2: TJvDateEdit
      Left = 667
      Top = 106
      Width = 80
      Height = 21
      TabOrder = 7
    end
    object rbData: TJvCheckBox
      Left = 584
      Top = 88
      Width = 145
      Height = 17
      Caption = 'Pedido/Or'#231'am. ou  Venda'
      Checked = True
      State = cbChecked
      TabOrder = 21
      LinkedControls = <>
      HotTrackFont.Charset = DEFAULT_CHARSET
      HotTrackFont.Color = clWindowText
      HotTrackFont.Height = -11
      HotTrackFont.Name = 'MS Sans Serif'
      HotTrackFont.Style = []
    end
  end
  object MMJPanel2: TMMJPanel
    Left = 0
    Top = 479
    Width = 792
    Height = 46
    Align = alBottom
    BevelInner = bvLowered
    PopupMenu = PopupMenu1
    TabOrder = 2
    Silhuette.Shape.ShapeText = 'Shape text'
    Silhuette.PictureShape.Picture.Data = {07544269746D617000000000}
    Silhuette.PictureShape.DisplayPicture = True
    Background.StartColor = clSilver
    Background.EndColor = clTeal
    Background.FillType = GradUpDown
    object btnProcurar: TBitBtn
      Left = 221
      Top = 6
      Width = 112
      Height = 34
      Caption = 'F8-&Procurar'
      PopupMenu = PopupMenu1
      TabOrder = 0
      OnClick = btnProcurarClick
      Glyph.Data = {
        
        7F7F7F7F7F7F7F7F828282C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
    end
    object btnSair: TBitBtn
      Left = 579
      Top = 6
      Width = 112
      Height = 34
      Caption = 'F9-Sair'
      PopupMenu = PopupMenu1
      TabOrder = 1
      OnClick = btnSairClick
      Glyph.Data = {
        
        BFBFBFBFBFBFBFBFBF00}
    end
    object BitBtn8: TBitBtn
      Left = 341
      Top = 6
      Width = 112
      Height = 34
      Caption = 'F6-Movimento'
      PopupMenu = PopupMenu1
      TabOrder = 2
      OnClick = BitBtn8Click
      Glyph.Data = {
        
        BFBFBFBF0000}
    end
    object BitBtn9: TBitBtn
      Left = 460
      Top = 6
      Width = 112
      Height = 34
      Caption = 'F4-Confirma'
      PopupMenu = PopupMenu1
      TabOrder = 3
      OnClick = BitBtn9Click
      Glyph.Data = {
        
        C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C00000}
    end
    object BitBtn5: TBitBtn
      Left = 101
      Top = 6
      Width = 112
      Height = 34
      Caption = 'Romaneio'
      PopupMenu = PopupMenu1
      TabOrder = 4
      OnClick = BitBtn5Click
      Glyph.Data = {
        
        0000080808080808080808080808080808080808080808000000}
    end
  end
  object DBGrid1: TJvDBGrid
    Left = 0
    Top = 178
    Width = 792
    Height = 301
    Align = alClient
    DataSource = ds_Cr
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
    ParentFont = False
    TabOrder = 3
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnTitleClick = JvDBGrid1TitleClick
    AlternateRowColor = clMoneyGreen
    AutoSizeColumns = True
    SelectColumnsDialogStrings.Caption = 'Select columns'
    SelectColumnsDialogStrings.OK = '&OK'
    SelectColumnsDialogStrings.NoSelectionWarning = 'At least one column must be visible!'
    EditControls = <>
    RowsHeight = 17
    TitleRowHeight = 17
    Columns = <
      item
        Expanded = False
        FieldName = 'CODMOVIMENTO'
        Title.Caption = 'Cod.'
        Width = 46
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NOTAFISCAL'
        Title.Caption = 'T'#237'tulo'
        Width = 46
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'SERIE'
        Title.Caption = 'S'#233'rie'
        Width = 90
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DATAMOVIMENTO'
        Title.Caption = 'Data'
        Width = 46
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CODCLIENTE'
        Title.Caption = 'C'#243'd.Cli.'
        Width = 46
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NOMECLIENTE'
        Title.Caption = 'Cliente'
        Width = 220
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'VALOR'
        Title.Caption = 'Valor R$'
        Width = 46
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CODNATUREZA'
        Title.Caption = 'C'#243'd. Nat.'
        Width = 46
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DESCNATUREZA'
        Title.Caption = 'Natureza'
        Width = 133
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'STATUS'
        Title.Caption = 'Status'
        Width = 47
        Visible = True
      end>
  end
  object XPMenu1: TXPMenu
    DimLevel = 30
    GrayLevel = 10
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clMenuText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    Color = clBtnFace
    DrawMenuBar = False
    IconBackColor = clBtnFace
    MenuBarColor = clBtnFace
    SelectColor = clHighlight
    SelectBorderColor = clHighlight
    SelectFontColor = clMenuText
    DisabledColor = clInactiveCaption
    SeparatorColor = clBtnFace
    CheckedColor = clHighlight
    IconWidth = 24
    DrawSelect = True
    UseSystemColors = True
    UseDimColor = False
    OverrideOwnerDraw = False
    Gradient = False
    FlatMenu = False
    AutoDetect = False
    Active = False
    Left = 456
    Top = 384
  end
  object ds_Cr: TDataSource
    DataSet = cds_cns
    Left = 424
    Top = 387
  end
  object cds_cns: TClientDataSet
    Aggregates = <>
    PacketRecords = 15
    Params = <>
    ProviderName = 'dsp_cns'
    Left = 390
    Top = 388
    object cds_cnsCODMOVIMENTO: TIntegerField
      FieldName = 'CODMOVIMENTO'
      Required = True
    end
    object cds_cnsCODCLIENTE: TIntegerField
      FieldName = 'CODCLIENTE'
      Required = True
    end
    object cds_cnsCODNATUREZA: TSmallintField
      FieldName = 'CODNATUREZA'
      Required = True
    end
    object cds_cnsDATAMOVIMENTO: TDateField
      FieldName = 'DATAMOVIMENTO'
      Required = True
    end
    object cds_cnsSTATUS: TSmallintField
      FieldName = 'STATUS'
      Required = True
    end
    object cds_cnsNOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      Required = True
      Size = 50
    end
    object cds_cnsDESCNATUREZA: TStringField
      FieldName = 'DESCNATUREZA'
      Required = True
      Size = 30
    end
    object cds_cnsCODFORNECEDOR: TIntegerField
      FieldName = 'CODFORNECEDOR'
    end
    object cds_cnsNOMEFORNECEDOR: TStringField
      FieldName = 'NOMEFORNECEDOR'
      Size = 50
    end
    object cds_cnsNOTAFISCAL: TIntegerField
      FieldName = 'NOTAFISCAL'
    end
    object cds_cnsSERIE: TStringField
      FieldName = 'SERIE'
    end
    object cds_cnsVALOR: TFloatField
      FieldName = 'VALOR'
      ProviderFlags = []
      DisplayFormat = ',#0.00'
    end
    object cds_cnsAPAGAR: TFloatField
      FieldName = 'APAGAR'
      ProviderFlags = [pfInUpdate]
    end
  end
  object dsp_cns: TDataSetProvider
    DataSet = sds_cns
    Options = [poAllowCommandText]
    Left = 358
    Top = 387
  end
  object sds_cns: TSQLDataSet
    CommandText = 
      'select mov.CODCLIENTE, mov.CODFORNECEDOR, mov.CODMOVIMENTO, mov.' +
      'CODNATUREZA, mov.DATAMOVIMENTO, '#13#10'mov.STATUS, cli.NOMECLIENTE, n' +
      'at.DESCNATUREZA,  forn.NOMEFORNECEDOR, ven.NOTAFISCAL, ven.SERIE' +
      ', ven.VALOR, '#13#10'ven.APAGAR   '#13#10'from MOVIMENTO mov '#13#10'left outer jo' +
      'in CLIENTES cli on cli.CODCLIENTE = mov.CODCLIENTE '#13#10'inner join ' +
      'NATUREZAOPERACAO nat on'#13#10' nat.CODNATUREZA = mov.CODNATUREZA '#13#10'le' +
      'ft outer join FORNECEDOR forn on forn.CODFORNECEDOR = mov.CODFOR' +
      'NECEDOR '#13#10'left outer join VENDA ven on ven.CODMOVIMENTO = mov.CO' +
      'DMOVIMENTO'#13#10
    MaxBlobSize = -1
    Params = <>
    SQLConnection = DM.sqlsisAdimin
    Left = 326
    Top = 387
    object sds_cnsCODMOVIMENTO: TIntegerField
      FieldName = 'CODMOVIMENTO'
      Required = True
    end
    object sds_cnsCODCLIENTE: TIntegerField
      FieldName = 'CODCLIENTE'
      Required = True
    end
    object sds_cnsCODFORNECEDOR: TIntegerField
      FieldName = 'CODFORNECEDOR'
    end
    object sds_cnsCODNATUREZA: TSmallintField
      FieldName = 'CODNATUREZA'
      Required = True
    end
    object sds_cnsDATAMOVIMENTO: TDateField
      FieldName = 'DATAMOVIMENTO'
      Required = True
    end
    object sds_cnsSTATUS: TSmallintField
      FieldName = 'STATUS'
      Required = True
    end
    object sds_cnsNOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      Size = 50
    end
    object sds_cnsDESCNATUREZA: TStringField
      FieldName = 'DESCNATUREZA'
      Required = True
      Size = 30
    end
    object sds_cnsNOMEFORNECEDOR: TStringField
      FieldName = 'NOMEFORNECEDOR'
      Size = 50
    end
    object sds_cnsNOTAFISCAL: TIntegerField
      FieldName = 'NOTAFISCAL'
    end
    object sds_cnsSERIE: TStringField
      FieldName = 'SERIE'
    end
    object sds_cnsVALOR: TFloatField
      FieldName = 'VALOR'
      ProviderFlags = []
    end
    object sds_cnsAPAGAR: TFloatField
      FieldName = 'APAGAR'
      ProviderFlags = [pfInUpdate]
    end
  end
  object PopupMenu1: TPopupMenu
    Left = 293
    Top = 386
    object Consultar1: TMenuItem
      Caption = '&Consultar'
      ShortCut = 119
      OnClick = btnProcurarClick
    end
    object VerMovimento1: TMenuItem
      Caption = '&Ver Movimento'
      ShortCut = 117
      OnClick = BitBtn8Click
    end
    object Fechar1: TMenuItem
      Caption = '&Sair'
      ShortCut = 120
      OnClick = btnSairClick
    end
    object Confirma1: TMenuItem
      Caption = 'Confirma'
      ShortCut = 115
      OnClick = BitBtn9Click
    end
  end
  object sds_Veiculocli: TSQLDataSet
    CommandText = 
      'select * from VEICULO '#13#10'where PLACA = :pplaca or COD_VEICULO =  ' +
      ':PVEIC'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'pplaca'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'PVEIC'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Left = 239
    Top = 385
    object sds_VeiculocliCOD_VEICULO: TIntegerField
      FieldName = 'COD_VEICULO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object sds_VeiculocliCOD_CLIENTE: TIntegerField
      FieldName = 'COD_CLIENTE'
      ProviderFlags = [pfInUpdate]
      Required = True
    end
    object sds_VeiculocliPLACA: TStringField
      FieldName = 'PLACA'
      ProviderFlags = [pfInUpdate]
      Size = 10
    end
    object sds_VeiculocliMARCA_MODELO: TStringField
      FieldName = 'MARCA_MODELO'
      ProviderFlags = [pfInUpdate]
      Size = 60
    end
    object sds_VeiculocliTIPO: TStringField
      FieldName = 'TIPO'
      ProviderFlags = [pfInUpdate]
    end
    object sds_VeiculocliCOMBUSTIVEL: TStringField
      FieldName = 'COMBUSTIVEL'
      ProviderFlags = [pfInUpdate]
    end
    object sds_VeiculocliANO_FAB: TStringField
      FieldName = 'ANO_FAB'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object sds_VeiculocliANO_MOD: TStringField
      FieldName = 'ANO_MOD'
      ProviderFlags = [pfInUpdate]
      Size = 4
    end
    object sds_VeiculocliCOR: TStringField
      FieldName = 'COR'
      ProviderFlags = [pfInUpdate]
    end
  end
  object JvAppXMLFileStorage1: TJvAppXMLFileStorage
    StorageOptions.BooleanStringTrueValues = 'TRUE, YES, Y'
    StorageOptions.BooleanStringFalseValues = 'FALSE, NO, N'
    StorageOptions.InvalidCharReplacement = '_'
    FileName = 'FiltroVenda.xml'
    RootNodeName = 'Configuration'
    SubStorages = <>
    Left = 408
    Top = 94
  end
  object JvFormStorage1: TJvFormStorage
    AppStorage = JvAppXMLFileStorage1
    AppStoragePath = 'fFiltroMovimento\'
    StoredProps.Strings = (
      'ComboBox1.Text'
      'edControle.Text'
      'meDta1.Date'
      'meDta2.Date'
      'Edit2.Text'
      'Edit3.Text'
      'Edit4.Text'
      'MaskEdit1.Text'
      'RadioGroup1.ItemIndex'
      'Edit8.Text'
      'edtNF.Text'
      'ComboBox2.Text')
    StoredValues = <>
    Left = 440
    Top = 96
  end
  object sdsRegiao: TSQLDataSet
    CommandText = 'select * from DADOS_COMBOS'
    MaxBlobSize = -1
    Params = <>
    SQLConnection = DM.sqlsisAdimin
    Left = 484
    Top = 327
    object sdsRegiaoCODDADOS: TIntegerField
      FieldName = 'CODDADOS'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object sdsRegiaoDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      ProviderFlags = [pfInUpdate]
      Size = 80
    end
    object sdsRegiaoUSO: TStringField
      FieldName = 'USO'
      ProviderFlags = [pfInUpdate]
      Size = 30
    end
    object sdsRegiaoCODIGOS: TStringField
      FieldName = 'CODIGOS'
      ProviderFlags = [pfInUpdate]
      Size = 50
    end
    object sdsRegiaoOUTROS: TStringField
      FieldName = 'OUTROS'
      ProviderFlags = [pfInUpdate]
      Size = 30
    end
  end
  object VCLReport1: TVCLReport
    AsyncExecution = False
    Title = 'Untitled'
    Left = 240
    Top = 240
  end
end
