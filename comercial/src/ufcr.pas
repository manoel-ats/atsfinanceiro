...
uses uComercial, UDm, uListaClientes, uProcurar, ucrTitulo, uDuplicata,
  ...
  sCtrlResize, uNotaFiscalCr, UDMNF, uNotaf, uCh;
...
begin
  ...
  //==============================================================================
  //------------------------------------------------------------------------------
  //Verifica se a data de Geração do Arquivo  foi preenchido
  //------------------------------------------------------------------------------
  try
  if (medta7.Text<>datastr) then
  StrToDate(medta7.Text);
  if (medta8.Text<>datastr) then
  StrToDate(medta8.Text);
  if (medta7.Text<>datastr) then
  if (medta8.Text<>datastr) then
  begin
    if SqlCr='' then
      SqlCr := SqlCr + ' WHERE rec.DATAGERARQREM BETWEEN '
    else
      SqlCr := SqlCr + ' AND rec.DATAGERARQREM BETWEEN ';
      SqlCr := SqlCr + '''' + formatdatetime('mm/dd/yy', StrToDate(medta7.Text)) + '''';
      SqlCr := SqlCr + ' AND ';
      SqlCr := SqlCr + '''' + formatdatetime('mm/dd/yy', StrToDate(medta8.Text)) + '''';
  end;
  except
  on EConvertError do
  begin
...
{    if SqlCr='' then
  ...
  if (cbarquivo.Text <> '') then
  begin
    if SqlCr='' then
      SqlCr := SqlCr + ' WHERE rec.GERARQREM = '
    else
      SqlCr := SqlCr + ' AND rec.GERARQREM = ';
      SqlCr := SqlCr + cbarquivo.Text ;
  end;
...
procedure Tfcrproc.BitBtn14Click(Sender: TObject);
begin
 meDta7.Text := '  /  /  ';
 meDta8.Text := '  /  /  ';
end;
