object fEnvelope: TfEnvelope
  Left = 339
  Top = 130
  Width = 516
  Height = 494
  BorderIcons = [biSystemMenu]
  Caption = 'Imprimir Envelope'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 154
    Top = 45
    Width = 127
    Height = 13
    Caption = 'C'#243'digo - ou - TODOS CLIE'
  end
  object SpeedButton2: TSpeedButton
    Left = 324
    Top = 13
    Width = 29
    Height = 30
    Glyph.Data = {
      
      00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    Visible = False
  end
  object lbl3: TLabel
    Left = 402
    Top = 132
    Width = 95
    Height = 13
    Caption = 'Qtd. Ja Utilizada na '
    Visible = False
  end
  object lbl4: TLabel
    Left = 407
    Top = 147
    Width = 83
    Height = 13
    Caption = 'Folha de Etiqueta'
    Visible = False
  end
  object GroupBox1: TGroupBox
    Left = 56
    Top = 318
    Width = 361
    Height = 121
    Caption = 'Avisos'
    TabOrder = 8
    object Label2: TLabel
      Left = 96
      Top = 16
      Width = 53
      Height = 13
      Caption = 'Data Inicial'
    end
    object Label3: TLabel
      Left = 192
      Top = 16
      Width = 48
      Height = 13
      Caption = 'Data Final'
    end
  end
  object BitBtn1: TBitBtn
    Left = 149
    Top = 101
    Width = 145
    Height = 41
    Caption = 'Imprimir Envelope (individual)'
    TabOrder = 2
    OnClick = BitBtn1Click
    OnKeyPress = meDta3KeyPress
  end
  object Edit1: TEdit
    Left = 149
    Top = 61
    Width = 137
    Height = 28
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 1
    OnKeyPress = meDta3KeyPress
  end
  object BitBtn2: TBitBtn
    Left = 150
    Top = 149
    Width = 145
    Height = 41
    Caption = 'Imprimir Envelope (Todos)'
    TabOrder = 3
    OnClick = BitBtn2Click
    OnKeyPress = meDta3KeyPress
  end
  object ComboBox1: TComboBox
    Left = 149
    Top = 13
    Width = 145
    Height = 21
    ItemHeight = 13
    TabOrder = 0
    Text = 'ASH'
    OnKeyPress = meDta3KeyPress
    Items.Strings = (
      'ASH'
      'PCMSO'
      'PARTICULAR')
  end
  object BitBtn3: TBitBtn
    Left = 164
    Top = 393
    Width = 129
    Height = 37
    Caption = 'Imprimir Avisos'
    TabOrder = 6
    OnClick = BitBtn3Click
  end
  object meDta4: TMaskEdit
    Left = 245
    Top = 351
    Width = 75
    Height = 24
    BevelKind = bkFlat
    BorderStyle = bsNone
    EditMask = '!99/99/00;1;_'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    MaxLength = 8
    ParentFont = False
    TabOrder = 5
    Text = '  /  /  '
    OnKeyPress = meDta3KeyPress
  end
  object meDta3: TMaskEdit
    Left = 132
    Top = 351
    Width = 81
    Height = 24
    BevelKind = bkFlat
    BorderStyle = bsNone
    EditMask = '!99/99/00;1;_'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    MaxLength = 8
    ParentFont = False
    TabOrder = 4
    Text = '  /  /  '
    OnKeyPress = meDta3KeyPress
  end
  object BitBtn7: TBitBtn
    Left = 329
    Top = 347
    Width = 32
    Height = 32
    TabOrder = 7
    TabStop = False
    OnClick = BitBtn7Click
    Glyph.Data = {
      
      0000}
  end
  object escolha: TRadioGroup
    Left = 304
    Top = 64
    Width = 145
    Height = 57
    Caption = 'Impressora'
    Columns = 2
    ItemIndex = 1
    Items.Strings = (
      'laser'
      'matricial')
    TabOrder = 9
  end
  object BitBtn4: TBitBtn
    Left = 416
    Top = 8
    Width = 75
    Height = 25
    Caption = 'BitBtn4'
    TabOrder = 10
    Visible = False
    OnClick = BitBtn4Click
  end
  object RadioGroup1: TRadioGroup
    Left = 12
    Top = 31
    Width = 97
    Height = 105
    Hint = 'Escolher Tipo a Imprimir'
    Caption = 'IMPRIMIR'
    ItemIndex = 0
    Items.Strings = (
      'KLOK'
      'INTERMEDIA'#199#195'O')
    TabOrder = 11
    Visible = False
  end
  object BitBtn5: TBitBtn
    Left = 108
    Top = 196
    Width = 249
    Height = 41
    Caption = 'Imprimir Envelope (individual Todos Associados) '
    TabOrder = 12
    OnClick = BitBtn5Click
    OnKeyPress = meDta3KeyPress
  end
  object BitBtn6: TBitBtn
    Left = 108
    Top = 238
    Width = 250
    Height = 41
    Caption = 'Imprimir Envelope  ASH (Todos Varej'#227'o e Correio)'
    TabOrder = 13
    OnClick = BitBtn6Click
    OnKeyPress = meDta3KeyPress
  end
  object Imprimir: TBitBtn
    Left = 0
    Top = 272
    Width = 91
    Height = 33
    Caption = 'Imprimir - Teste'
    TabOrder = 14
    Visible = False
    OnClick = ImprimirClick
  end
  object BitBtn8: TBitBtn
    Left = 109
    Top = 280
    Width = 250
    Height = 31
    Caption = 'BitBtn8'
    TabOrder = 15
    OnClick = BitBtn8Click
  end
  object BitBtn9: TBitBtn
    Left = 384
    Top = 168
    Width = 89
    Height = 25
    Caption = 'Etiqueta - ASH'
    TabOrder = 16
    Visible = False
    OnClick = BitBtn9Click
  end
  object edt1: TEdit
    Left = 354
    Top = 134
    Width = 35
    Height = 21
    TabOrder = 17
    Text = '0'
    Visible = False
  end
  object VCLReport1: TVCLReport
    AsyncExecution = False
    Title = 'Untitled'
    Left = 16
    Top = 168
  end
  object sql_imprimi: TSQLClientDataSet
    CommandText = 
      'select  CLI.CODCLIENTE'#13#10'           ,CLI.NOMECLIENTE'#13#10'           ' +
      ',CLI.GRUPO_CLIENTE'#13#10'           ,CLI.STATUS'#13#10'           ,CLI.RA'#13#10 +
      '           ,ENDE.LOGRADOURO'#13#10'           ,ENDE.CIDADE'#13#10'          ' +
      ' ,ENDE.BAIRRO'#13#10'           ,ENDE.UF'#13#10'           ,ENDE.CEP '#13#10'     ' +
      '     ,ENDE.COMPLEMENTO      '#13#10'           '#13#10'      '#13#10'from CLIENTES' +
      ' CLI'#13#10#13#10'LEFT OUTER JOIN  ENDERECOCLIENTE ENDE ON ENDE.CODCLIENTE' +
      ' = CLI.CODCLIENTE '#13#10#13#10'where GERAENV = '#39'S'#39' and  CLI.GRUPO_CLIENTE' +
      ' = :GRUPO and ENDE.TIPOEND = 0 and CLI.STATUS = 0'#13#10'        '#13#10'ord' +
      'er by cli.codcliente'
    Aggregates = <>
    Options = [poAllowCommandText]
    ObjectView = True
    Params = <
      item
        DataType = ftString
        Name = 'GRUPO'
        ParamType = ptInput
      end>
    DBConnection = DM.sqlsisAdimin
    Left = 8
    Top = 232
    object sql_imprimiCODCLIENTE: TIntegerField
      FieldName = 'CODCLIENTE'
      Required = True
    end
    object sql_imprimiNOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      Required = True
      Size = 50
    end
    object sql_imprimiGRUPO_CLIENTE: TStringField
      FieldName = 'GRUPO_CLIENTE'
      Size = 30
    end
    object sql_imprimiSTATUS: TSmallintField
      FieldName = 'STATUS'
      Required = True
    end
    object sql_imprimiRA: TStringField
      FieldName = 'RA'
      Size = 10
    end
    object sql_imprimiLOGRADOURO: TStringField
      FieldName = 'LOGRADOURO'
      Size = 50
    end
    object sql_imprimiCIDADE: TStringField
      FieldName = 'CIDADE'
      Size = 40
    end
    object sql_imprimiBAIRRO: TStringField
      FieldName = 'BAIRRO'
      Size = 30
    end
    object sql_imprimiUF: TStringField
      FieldName = 'UF'
      FixedChar = True
      Size = 2
    end
    object sql_imprimiCEP: TStringField
      FieldName = 'CEP'
      Size = 10
    end
    object sql_imprimiCOMPLEMENTO: TStringField
      FieldName = 'COMPLEMENTO'
      Size = 30
    end
  end
  object VDODmPrinter1: TVDODmPrinter
    ShowDialog = True
    ShowPreview = True
    ShowProgress = True
    Title = 'VDOPrint Document'
    CharMode = cmClear
    Font.Align = faLeft
    Font.FontType = ftDraft
    Font.NLQFont = ntSansSerif
    Font.Pitch = fp10cpp
    Font.Size = fsDefault
    Font.Style = []
    LineSpacing = ls6lpp
    LineSpacingCustomValue = 36
    Paper.AutoNewPage = False
    Paper.AutoNewPageLines = 3
    Paper.Columns = 80
    Paper.Lines = 66
    Paper.MarginLeft = 0
    Paper.MarginRight = 0
    Left = 376
    Top = 24
  end
  object OpenDialog1: TOpenDialog
    Left = 8
    Top = 424
  end
  object sParametro: TSQLDataSet
    CommandText = 
      'select  PARAMETRO, DADOS, D1, D2, D3, D4, D5, D6, D7, D8, D9 fro' +
      'm PARAMETRO '#13#10'where PARAMETRO = '#39'TERMINAL'#39
    MaxBlobSize = -1
    Params = <>
    SQLConnection = DM.sqlsisAdimin
    Left = 37
    Top = 422
    object sParametroPARAMETRO: TStringField
      FieldName = 'PARAMETRO'
      Required = True
      Size = 40
    end
    object sParametroDADOS: TStringField
      FieldName = 'DADOS'
      Size = 40
    end
    object sParametroD1: TStringField
      FieldName = 'D1'
      Size = 30
    end
    object sParametroD2: TStringField
      FieldName = 'D2'
      Size = 30
    end
    object sParametroD3: TStringField
      FieldName = 'D3'
      Size = 30
    end
    object sParametroD4: TStringField
      FieldName = 'D4'
      Size = 30
    end
    object sParametroD5: TStringField
      FieldName = 'D5'
      Size = 30
    end
    object sParametroD6: TStringField
      FieldName = 'D6'
      Size = 30
    end
    object sParametroD7: TStringField
      FieldName = 'D7'
      Size = 30
    end
    object sParametroD8: TStringField
      FieldName = 'D8'
      Size = 30
    end
    object sParametroD9: TStringField
      FieldName = 'D9'
      Size = 30
    end
  end
  object pParametro: TDataSetProvider
    DataSet = sParametro
    Left = 69
    Top = 422
  end
  object cParametro: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'pParametro'
    Left = 101
    Top = 422
    object cParametroPARAMETRO: TStringField
      FieldName = 'PARAMETRO'
      Required = True
      Size = 40
    end
    object cParametroDADOS: TStringField
      FieldName = 'DADOS'
      Size = 40
    end
    object cParametroD1: TStringField
      FieldName = 'D1'
      Size = 30
    end
    object cParametroD2: TStringField
      FieldName = 'D2'
      Size = 30
    end
    object cParametroD3: TStringField
      FieldName = 'D3'
      Size = 30
    end
    object cParametroD4: TStringField
      FieldName = 'D4'
      Size = 30
    end
    object cParametroD5: TStringField
      FieldName = 'D5'
      Size = 30
    end
    object cParametroD6: TStringField
      FieldName = 'D6'
      Size = 30
    end
    object cParametroD7: TStringField
      FieldName = 'D7'
      Size = 30
    end
    object cParametroD8: TStringField
      FieldName = 'D8'
      Size = 30
    end
    object cParametroD9: TStringField
      FieldName = 'D9'
      Size = 30
    end
  end
  object dspASH: TDataSetProvider
    DataSet = sqlASH
    Left = 416
    Top = 240
  end
  object sqlASH: TSQLDataSet
    CommandText = 
      'select  CLI.CODCLIENTE'#13#10'       ,CLI.RA  '#13#10'      , '#39'('#39' ||CLI.RA |' +
      '| '#39')'#39' || CLI.NOMECLIENTE as NOMECLIENTE '#13#10'       ,CLI.GRUPO_CLIE' +
      'NTE'#13#10'       ,CLI.STATUS'#13#10'       ,ENDE.LOGRADOURO'#13#10'       ,ENDE.C' +
      'IDADE'#13#10'       ,ENDE.BAIRRO'#13#10'       ,ENDE.UF'#13#10'       ,ENDE.CEP   ' +
      '    '#13#10'           '#13#10'      '#13#10'from CLIENTES CLI'#13#10'LEFT OUTER JOIN  E' +
      'NDERECOCLIENTE ENDE ON ENDE.CODCLIENTE = CLI.CODCLIENTE '#13#10#13#10'wher' +
      'e GERAENV = '#39'S'#39' '#13#10'      and grupo_cliente = '#39'PCMSO'#39#13#10'      and s' +
      'tatus = 0  '#13#10'      and segmento = 0 '#13#10'order by RA'
    MaxBlobSize = -1
    Params = <>
    SQLConnection = DM.sqlsisAdimin
    Left = 376
    Top = 240
  end
  object dsASH: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspASH'
    Left = 456
    Top = 240
    object dsASHCODCLIENTE: TIntegerField
      FieldName = 'CODCLIENTE'
      Required = True
    end
    object dsASHRA: TStringField
      FieldName = 'RA'
      Size = 10
    end
    object dsASHNOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      Required = True
      Size = 50
    end
    object dsASHGRUPO_CLIENTE: TStringField
      FieldName = 'GRUPO_CLIENTE'
      Size = 30
    end
    object dsASHSTATUS: TSmallintField
      FieldName = 'STATUS'
      Required = True
    end
    object dsASHLOGRADOURO: TStringField
      FieldName = 'LOGRADOURO'
      Size = 50
    end
    object dsASHCIDADE: TStringField
      FieldName = 'CIDADE'
      Size = 40
    end
    object dsASHBAIRRO: TStringField
      FieldName = 'BAIRRO'
      Size = 30
    end
    object dsASHUF: TStringField
      FieldName = 'UF'
      FixedChar = True
      Size = 2
    end
    object dsASHCEP: TStringField
      FieldName = 'CEP'
      Size = 10
    end
  end
end
