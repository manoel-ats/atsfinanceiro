inherited fcrTituloPagto: TfcrTituloPagto
  Left = 220
  Top = 107
  Width = 816
  Height = 573
  Caption = 'Lan'#231'ar Despesa'
  OldCreateOrder = True
  OnDestroy = FormDestroy
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Label26: TLabel [0]
    Left = 449
    Top = 193
    Width = 20
    Height = 20
    Caption = '.....'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clRed
    Font.Height = -16
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object Label3: TLabel [1]
    Left = 6
    Top = 54
    Width = 28
    Height = 13
    Caption = 'T'#237'tulo'
  end
  object Label4: TLabel [2]
    Left = 238
    Top = 54
    Width = 65
    Height = 13
    Caption = 'Data Emiss'#227'o'
  end
  object Label5: TLabel [3]
    Left = 348
    Top = 54
    Width = 82
    Height = 13
    Caption = 'Data Vencimento'
  end
  object Label6: TLabel [4]
    Left = 442
    Top = 54
    Width = 97
    Height = 13
    Caption = 'Centro de Resultado'
  end
  object Label7: TLabel [5]
    Left = 6
    Top = 90
    Width = 136
    Height = 13
    Caption = 'C'#243'd. Reduzido C. Despesas:'
  end
  object Label8: TLabel [6]
    Left = 221
    Top = 90
    Width = 78
    Height = 13
    Caption = 'Conta Despesas'
  end
  object Label9: TLabel [7]
    Left = 6
    Top = 129
    Width = 126
    Height = 13
    Caption = 'C'#243'digo Fornecedor/ Conta'
  end
  object Label10: TLabel [8]
    Left = 221
    Top = 129
    Width = 136
    Height = 13
    Caption = 'Fornecedor / Conta Passivo:'
  end
  object Label11: TLabel [9]
    Left = 6
    Top = 196
    Width = 70
    Height = 13
    Caption = 'Valor '#224' Pagar :'
  end
  object Label12: TLabel [10]
    Left = 169
    Top = 196
    Width = 55
    Height = 13
    Caption = 'N. Parcelas'
  end
  object Label13: TLabel [11]
    Left = 280
    Top = 196
    Width = 54
    Height = 13
    Caption = 'Entrada R$'
  end
  object Label14: TLabel [12]
    Left = 376
    Top = 196
    Width = 68
    Height = 13
    Caption = 'Caixa / Banco'
  end
  object Label15: TLabel [13]
    Left = 75
    Top = 261
    Width = 41
    Height = 13
    Caption = 'Hist'#243'rico'
  end
  object Label16: TLabel [14]
    Left = 6
    Top = 459
    Width = 78
    Height = 13
    Caption = 'Data do Sistema'
  end
  object Label17: TLabel [15]
    Left = 391
    Top = 459
    Width = 36
    Height = 13
    Caption = 'Usu'#225'rio'
  end
  object Label18: TLabel [16]
    Left = 600
    Top = 194
    Width = 35
    Height = 13
    Caption = 'Forma :'
  end
  object Label19: TLabel [17]
    Left = 10
    Top = 240
    Width = 89
    Height = 13
    Caption = 'N'#250'm. Documento :'
  end
  object Label20: TLabel [18]
    Left = 242
    Top = 242
    Width = 80
    Height = 13
    Caption = 'Data Pagamento'
  end
  object Label21: TLabel [19]
    Left = 512
    Top = 242
    Width = 84
    Height = 13
    Caption = 'Data Consolidado'
  end
  object Label22: TLabel [20]
    Left = 106
    Top = 194
    Width = 54
    Height = 13
    Caption = 'N'#186' Eventos'
  end
  object lbl1: TLabel [21]
    Left = 135
    Top = 54
    Width = 63
    Height = 13
    Caption = 'Data Entrada'
  end
  object Label23: TLabel [22]
    Left = 49
    Top = 176
    Width = 33
    Height = 13
    Caption = 'C'#243'digo'
    PopupMenu = PopupMenu1
  end
  object Label24: TLabel [23]
    Left = 170
    Top = 176
    Width = 63
    Height = 13
    Caption = 'Nome Cliente'
    PopupMenu = PopupMenu1
  end
  object Label25: TLabel [24]
    Left = 670
    Top = 166
    Width = 91
    Height = 13
    Caption = 'Data Procedimento'
  end
  object JvDBSearchEdit3: TJvDBSearchEdit [25]
    Left = 328
    Top = 142
    Width = 261
    Height = 24
    DataSource = DataSource3
    DataField = 'NOMECLIENTE'
    Color = 13562879
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    PopupMenu = PopupMenu1
    TabOrder = 41
    Text = 'JvDBSearchEdit3'
    OnExit = JvDBSearchEdit3Exit
  end
  object Edit2: TEdit [26]
    Left = 239
    Top = 193
    Width = 61
    Height = 19
    BevelKind = bkFlat
    BorderStyle = bsNone
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    PopupMenu = PopupMenu1
    TabOrder = 43
    Visible = False
    OnChange = Edit2Change
  end
  inherited MMJPanel1: TMMJPanel
    Top = 481
    Width = 800
    TabOrder = 26
    inherited btnGravar: TBitBtn
      Left = 332
    end
    inherited btnIncluir: TBitBtn
      Left = 332
    end
    inherited btnCancelar: TBitBtn
      Left = 461
    end
    inherited btnExcluir: TBitBtn
      Left = 462
    end
    inherited btnProcurar: TBitBtn
      Left = 206
      OnClick = btnProcurarClick
    end
    inherited btnSair: TBitBtn
      Left = 589
    end
    object btnLancar_socio: TBitBtn
      Left = 74
      Top = 7
      Width = 106
      Height = 41
      Caption = 'Lan'#231'ar Socio(Ctrl+s)'
      PopupMenu = PopupMenu1
      TabOrder = 6
      OnClick = btnLancar_socioClick
    end
  end
  inherited MMJPanel2: TMMJPanel
    Width = 800
    Font.Charset = ANSI_CHARSET
    Font.Color = clWhite
    Font.Height = -32
    Font.Name = 'Cooper Blk BT'
    ParentFont = False
    TabOrder = 27
    inherited Label1: TLabel
      Left = 234
      Top = -81
      Width = 309
      Height = 44
      Caption = 'Lan'#231'ar Despesas'
      Font.Name = 'Cooper Black'
    end
    inherited Label2: TLabel
      Left = 236
      Top = -80
      Width = 309
      Height = 44
      Caption = 'Lan'#231'ar Despesas'
      Font.Name = 'Cooper Black'
    end
    object RxLabel1: TRxLabel
      Left = 5
      Top = -75
      Width = 247
      Height = 36
      Alignment = taCenter
      Caption = 'Lan'#231'ar Despesas'
      ShadowColor = clBlack
      ShadowSize = 2
      Transparent = True
    end
    object JvLabel1: TJvLabel
      Left = 1
      Top = 1
      Width = 798
      Height = 52
      Align = alClient
      Alignment = taCenter
      AutoSize = False
      Caption = 'Lan'#231'ar Despesas'
      ShadowColor = clNavy
      ShadowSize = 4
      Transparent = True
      AutoOpenURL = False
      HotTrackFont.Charset = ANSI_CHARSET
      HotTrackFont.Color = clWindowText
      HotTrackFont.Height = -32
      HotTrackFont.Name = 'Cooper Blk BT'
      HotTrackFont.Style = []
    end
  end
  object DBEdit1: TDBEdit [29]
    Left = 6
    Top = 67
    Width = 115
    Height = 24
    BevelKind = bkFlat
    BorderStyle = bsNone
    DataField = 'TITULO'
    DataSource = DtSrc
    TabOrder = 0
    OnKeyPress = FormKeyPress
  end
  object DBEdit2: TDBEdit [30]
    Left = 238
    Top = 67
    Width = 95
    Height = 24
    BevelKind = bkFlat
    BorderStyle = bsNone
    DataField = 'EMISSAO'
    DataSource = DtSrc
    MaxLength = 8
    TabOrder = 2
    OnExit = DBEdit2Exit
    OnKeyPress = FormKeyPress
  end
  object DBEdit3: TDBEdit [31]
    Left = 346
    Top = 67
    Width = 89
    Height = 24
    BevelKind = bkFlat
    BorderStyle = bsNone
    DataField = 'DATAVENCIMENTO'
    DataSource = DtSrc
    MaxLength = 8
    TabOrder = 3
    OnKeyPress = FormKeyPress
  end
  object DBLookupComboBox1: TDBLookupComboBox [32]
    Left = 442
    Top = 67
    Width = 345
    Height = 21
    Ctl3D = True
    DataField = 'CODALMOXARIFADO'
    DataSource = DtSrc
    KeyField = 'CODIGO'
    ListField = 'NOME'
    ListSource = ds_ccusto
    ParentCtl3D = False
    TabOrder = 4
    OnKeyPress = FormKeyPress
  end
  object BitBtn12: TBitBtn [33]
    Left = 179
    Top = 99
    Width = 39
    Height = 31
    Caption = 'F5'
    PopupMenu = PopupMenu1
    TabOrder = 6
    TabStop = False
    OnClick = BitBtn12Click
  end
  object BitBtn1: TBitBtn [34]
    Left = 179
    Top = 136
    Width = 39
    Height = 32
    Caption = 'F6'
    PopupMenu = PopupMenu1
    TabOrder = 21
    TabStop = False
    OnClick = BitBtn1Click
  end
  object DBEdit8: TDBEdit [35]
    Left = 6
    Top = 212
    Width = 94
    Height = 24
    BevelKind = bkFlat
    BorderStyle = bsNone
    DataField = 'VALORTITULO'
    DataSource = DtSrc
    TabOrder = 11
    OnKeyPress = FormKeyPress
  end
  object DBEdit9: TDBEdit [36]
    Left = 169
    Top = 212
    Width = 75
    Height = 24
    BevelKind = bkFlat
    BorderStyle = bsNone
    DataField = 'PARCELAS'
    DataSource = DtSrc
    TabOrder = 13
    OnExit = DBEdit9Exit
    OnKeyPress = FormKeyPress
  end
  object CheckBox1: TCheckBox [37]
    Left = 238
    Top = 255
    Width = 97
    Height = 17
    Caption = 'Desp. Mensal ?'
    Enabled = False
    TabOrder = 22
    Visible = False
    OnClick = CheckBox1Click
    OnKeyPress = FormKeyPress
  end
  object DBEdit10: TDBEdit [38]
    Left = 280
    Top = 212
    Width = 94
    Height = 24
    BevelKind = bkFlat
    BorderStyle = bsNone
    DataField = 'VALORRECEBIDO'
    DataSource = DtSrc
    TabOrder = 14
    OnExit = DBEdit10Exit
    OnKeyPress = FormKeyPress
  end
  object DBLookupComboBox3: TDBLookupComboBox [39]
    Left = 375
    Top = 212
    Width = 221
    Height = 21
    Ctl3D = True
    DataField = 'CAIXA'
    DataSource = DtSrc
    KeyField = 'CODIGO'
    ListField = 'NOME'
    ListSource = d_3_caixa_banco
    ParentCtl3D = False
    TabOrder = 15
    OnExit = DBLookupComboBox3Exit
    OnKeyPress = FormKeyPress
  end
  object dxButton1: TdxButton [40]
    Left = 648
    Top = 275
    Width = 135
    Height = 37
    About = 'Design eXperience. '#169' 2002 M. Hoffmann'
    Version = '1.0.2e'
    OnClick = dxButton1Click
    Caption = '  &Rateio '
    Glyph.Data = {
      
      00FF000000FF000000FF000000FF000000FF000000FF000000FF}
    TabOrder = 23
  end
  object dxButton2: TdxButton [41]
    Left = 510
    Top = 275
    Width = 134
    Height = 37
    About = 'Design eXperience. '#169' 2002 M. Hoffmann'
    Version = '1.0.2e'
    OnClick = dxButton2Click
    Caption = 'Relaciona Docs.'
    Glyph.Data = {
      
      0707070707070707070707070707070707070707070707070707}
    TabOrder = 24
  end
  object DBMemo1: TDBMemo [42]
    Left = 72
    Top = 276
    Width = 433
    Height = 35
    DataField = 'HISTORICO'
    DataSource = DtSrc
    TabOrder = 20
    OnKeyPress = FormKeyPress
  end
  object DBGrid1: TDBGrid [43]
    Left = 6
    Top = 313
    Width = 723
    Height = 141
    TabStop = False
    Color = clInfoBk
    DataSource = DataSource2
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 25
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'TITULO'
        Title.Caption = 'T'#237'tulo'
        Width = 70
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'N_DOCUMENTO'
        Title.Caption = 'N'#186' Doc'
        Width = 82
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'EMISSAO'
        Title.Caption = 'Emiss'#227'o'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DATAVENCIMENTO'
        Title.Caption = 'Vencimento'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DATAPAGAMENTO'
        Title.Caption = 'Pagamento'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'STATUS'
        Title.Caption = 'Situa'#231#227'o'
        Width = 90
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'VIA'
        Title.Caption = 'Via'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PARCELAS'
        Title.Caption = 'Parcelas'
        Width = 50
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'VALORTITULO'
        Title.Caption = 'Valor'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'VALOR_RESTO'
        Title.Caption = 'Pendente'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'VALORRECEBIDO'
        Title.Caption = 'Pago'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'RA'
        Title.Caption = 'Codigo'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NOMECLIENTE'
        Title.Caption = 'Nome'
        Visible = True
      end>
  end
  object edtcodred: TEdit [44]
    Left = 5
    Top = 106
    Width = 171
    Height = 24
    BevelKind = bkFlat
    BorderStyle = bsNone
    TabOrder = 5
    OnExit = edtcodredExit
    OnKeyPress = FormKeyPress
  end
  object edtconta: TEdit [45]
    Left = 220
    Top = 104
    Width = 564
    Height = 24
    TabStop = False
    BevelKind = bkFlat
    BorderStyle = bsNone
    Color = clInfoBk
    ReadOnly = True
    TabOrder = 28
  end
  object dbeCliente: TEdit [46]
    Left = 6
    Top = 143
    Width = 171
    Height = 24
    BevelKind = bkFlat
    BorderStyle = bsNone
    TabOrder = 7
    OnExit = dbeClienteExit
    OnKeyPress = FormKeyPress
  end
  object DBEdit4: TEdit [47]
    Left = 220
    Top = 143
    Width = 564
    Height = 24
    TabStop = False
    BevelKind = bkFlat
    BorderStyle = bsNone
    Color = clInfoBk
    ReadOnly = True
    TabOrder = 29
    OnExit = DBEdit4Exit
    OnKeyPress = FormKeyPress
  end
  object DBEdit5: TDBEdit [48]
    Left = 123
    Top = 239
    Width = 107
    Height = 25
    BevelKind = bkFlat
    BorderStyle = bsNone
    DataField = 'N_DOCUMENTO'
    DataSource = DtSrc
    TabOrder = 17
    OnKeyPress = FormKeyPress
  end
  object ComboBox1: TComboBox [49]
    Left = 598
    Top = 212
    Width = 185
    Height = 21
    ItemHeight = 13
    TabOrder = 16
    OnKeyPress = FormKeyPress
  end
  object DBEdit6: TDBEdit [50]
    Left = 354
    Top = 241
    Width = 106
    Height = 24
    BevelKind = bkFlat
    BorderStyle = bsNone
    DataField = 'DATAPAGAMENTO'
    DataSource = DtSrc
    MaxLength = 8
    TabOrder = 18
    OnKeyPress = FormKeyPress
  end
  object DBEdit7: TDBEdit [51]
    Left = 624
    Top = 241
    Width = 111
    Height = 24
    BevelKind = bkFlat
    BorderStyle = bsNone
    DataField = 'DATACONSOLIDA'
    DataSource = DtSrc
    MaxLength = 8
    TabOrder = 19
    OnKeyPress = FormKeyPress
  end
  object BitBtn4: TBitBtn [52]
    Left = 461
    Top = 240
    Width = 31
    Height = 30
    PopupMenu = PopupMenu1
    TabOrder = 30
    TabStop = False
    OnClick = BitBtn4Click
    Glyph.Data = {
      
      0000}
  end
  object BitBtn2: TBitBtn [53]
    Left = 736
    Top = 240
    Width = 31
    Height = 30
    PopupMenu = PopupMenu1
    TabOrder = 31
    TabStop = False
    OnClick = BitBtn2Click
    Glyph.Data = {
      
      0000}
  end
  object BitBtn3: TBitBtn [54]
    Left = 730
    Top = 314
    Width = 37
    Height = 39
    TabOrder = 32
    OnClick = SpeedButton2Click
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
  end
  object DBEdit11: TDBEdit [55]
    Left = 89
    Top = 456
    Width = 296
    Height = 24
    BevelKind = bkFlat
    BorderStyle = bsNone
    Color = clInfoBk
    DataField = 'DATASISTEMA'
    DataSource = DtSrc
    MaxLength = 8
    ReadOnly = True
    TabOrder = 33
    OnExit = DBEdit2Exit
    OnKeyPress = FormKeyPress
  end
  object DBEdit12: TDBEdit [56]
    Left = 433
    Top = 456
    Width = 296
    Height = 24
    BevelKind = bkFlat
    BorderStyle = bsNone
    Color = clInfoBk
    DataField = 'NOMEUSUARIO'
    DataSource = DtSrc
    ReadOnly = True
    TabOrder = 34
    OnKeyPress = FormKeyPress
  end
  object DBEdit13: TDBEdit [57]
    Left = 111
    Top = 211
    Width = 41
    Height = 24
    BevelKind = bkFlat
    BorderStyle = bsNone
    DataField = 'OUTRO_CREDITO'
    DataSource = DtSrc
    TabOrder = 12
    OnKeyPress = FormKeyPress
  end
  object dbedtEMISSAO: TDBEdit [58]
    Left = 134
    Top = 67
    Width = 97
    Height = 24
    BevelKind = bkFlat
    BorderStyle = bsNone
    DataField = 'DATAINCLUSAO'
    DataSource = DtSrc
    MaxLength = 8
    TabOrder = 1
    OnExit = DBEdit2Exit
    OnKeyPress = FormKeyPress
  end
  object editCodCliente: TEdit [59]
    Left = 31
    Top = 4
    Width = 71
    Height = 24
    BevelKind = bkFlat
    BorderStyle = bsNone
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    PopupMenu = PopupMenu1
    TabOrder = 35
    Visible = False
    OnChange = editCodClienteChange
    OnKeyPress = FormKeyPress
  end
  object edCliente: TJvDBSearchEdit [60]
    Left = 32
    Top = 28
    Width = 217
    Height = 24
    DataSource = DataSource3
    DataField = 'NOMECLIENTE'
    CharCase = ecUpperCase
    Color = 13562879
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    PopupMenu = PopupMenu1
    TabOrder = 36
    Text = 'EDCLIENTE'
    Visible = False
    OnExit = edClienteExit
    OnKeyPress = FormKeyPress
  end
  object btnLimpa: TBitBtn [61]
    Left = 167
    Top = 4
    Width = 32
    Height = 22
    PopupMenu = PopupMenu1
    TabOrder = 37
    TabStop = False
    Visible = False
    Glyph.Data = {
      
      0000}
  end
  object JvDBSearchEdit1: TJvDBSearchEdit [62]
    Left = 87
    Top = 173
    Width = 81
    Height = 21
    DataSource = DataSource3
    DataField = 'RA'
    ClearOnEnter = False
    TabOrder = 8
    Text = 'JvDBSearchEdit1'
    OnKeyPress = FormKeyPress
  end
  object JvDBSearchEdit2: TJvDBSearchComboBox [63]
    Left = 235
    Top = 173
    Width = 374
    Height = 21
    DataField = 'NOMECLIENTE'
    DataSource = DataSource3
    CharCase = ecUpperCase
    ItemHeight = 13
    TabOrder = 9
    Text = 'ADILSON DO REGO'
    OnExit = JvDBSearchEdit2Exit
    OnKeyPress = FormKeyPress
  end
  object chk: TCheckBox [64]
    Left = 9
    Top = 176
    Width = 40
    Height = 15
    Caption = 'Sim'
    TabOrder = 38
  end
  object codcliente: TDBEdit [65]
    Left = 179
    Top = 193
    Width = 57
    Height = 19
    BevelKind = bkFlat
    BorderStyle = bsNone
    DataField = 'CODCLIENTE'
    DataSource = DtSrc
    TabOrder = 39
    Visible = False
    OnKeyPress = FormKeyPress
  end
  object DBEdit14: TDBEdit [66]
    Left = 670
    Top = 179
    Width = 97
    Height = 24
    BevelKind = bkFlat
    BorderStyle = bsNone
    Enabled = False
    MaxLength = 8
    TabOrder = 46
    OnExit = DBEdit14Exit
    OnKeyPress = FormKeyPress
  end
  object fgrupo: TBitBtn [67]
    Left = 611
    Top = 169
    Width = 54
    Height = 26
    Caption = 'Ctrl+B'
    PopupMenu = PopupMenu1
    TabOrder = 40
    OnClick = fgrupoClick
  end
  object Edit1: TEdit [68]
    Left = 263
    Top = 116
    Width = 58
    Height = 14
    BevelKind = bkFlat
    BorderStyle = bsNone
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    PopupMenu = PopupMenu1
    TabOrder = 42
    Visible = False
  end
  object data_recibo: TJvDateEdit [69]
    Left = 656
    Top = 88
    Width = 113
    Height = 21
    TabOrder = 44
    Visible = False
    OnKeyPress = FormKeyPress
  end
  object BitBl: TBitBtn [70]
    Left = 768
    Top = 176
    Width = 28
    Height = 30
    PopupMenu = PopupMenu1
    TabOrder = 45
    TabStop = False
    OnClick = BitBlClick
    Glyph.Data = {
      
      0000}
  end
  object JvDBDateEdit1: TJvDBDateEdit [71]
    Left = 670
    Top = 179
    Width = 98
    Height = 21
    DataField = 'DATAPROCEDIMENTO'
    DataSource = DtSrc
    TabOrder = 10
    OnExit = JvDBDateEdit1Exit
    OnKeyPress = FormKeyPress
  end
  inherited XPMenu1: TXPMenu
    Left = 696
    Top = 40
  end
  inherited PopupMenu1: TPopupMenu
    Left = 728
    Top = 40
    object BuscaFornecedor1: TMenuItem [0]
      Caption = 'Busca Fornecedor'
      ShortCut = 117
      OnClick = BitBtn1Click
    end
    object BuscaCodDespesas1: TMenuItem [1]
      Caption = 'Busca Cod. Despesas'
      ShortCut = 116
      OnClick = BitBtn12Click
    end
    inherited Procurar1: TMenuItem
      OnClick = btnProcurarClick
    end
    object LanarSocio1: TMenuItem
      Caption = 'Lan'#231'ar Socio'
      ShortCut = 16467
      OnClick = btnLancar_socioClick
    end
    object BuscarSocio1: TMenuItem
      Caption = 'Buscar Socio'
      ShortCut = 16450
      OnClick = fgrupoClick
    end
  end
  inherited DtSrc: TDataSource
    DataSet = DM.cds_4_pagar
    Left = 520
    Top = 27
  end
  object s_forn: TSQLDataSet
    CommandText = 
      'select CODFORNECEDOR, NOMEFORNECEDOR from FORNECEDOR where CONTA' +
      '_FORNECEDOR like :pCLIENTE  order by NOMEFORNECEDOR'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'pCLIENTE'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Left = 432
    Top = 408
  end
  object d_forn: TDataSetProvider
    DataSet = s_forn
    Left = 464
    Top = 408
  end
  object cds_forn: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'pCLIENTE'
        ParamType = ptInput
      end>
    ProviderName = 'd_forn'
    Left = 496
    Top = 408
    object cds_fornCODFORNECEDOR: TIntegerField
      FieldName = 'CODFORNECEDOR'
      Required = True
    end
    object cds_fornNOMEFORNECEDOR: TStringField
      FieldName = 'NOMEFORNECEDOR'
      Required = True
      Size = 50
    end
  end
  object scds_forn: TSQLClientDataSet
    CommandText = 
      'select CODFORNECEDOR, NOMEFORNECEDOR from FORNECEDOR where NOMEF' +
      'ORNECEDOR like:pCLIENTE or RAZAOSOCIAL like:pRAZAO or CODFORNECE' +
      'DOR=:pCODCLIENTE order by NOMEFORNECEDOR'
    Aggregates = <>
    Options = [poAllowCommandText]
    ObjectView = True
    Params = <
      item
        DataType = ftString
        Name = 'pCLIENTE'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'pRAZAO'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'pCODCLIENTE'
        ParamType = ptInput
      end>
    DBConnection = DM.sqlsisAdimin
    Left = 528
    Top = 408
    object scds_fornCODFORNECEDOR: TIntegerField
      FieldName = 'CODFORNECEDOR'
      Required = True
    end
    object scds_fornNOMEFORNECEDOR: TStringField
      FieldName = 'NOMEFORNECEDOR'
      Required = True
      Size = 50
    end
  end
  object sds_desp: TSQLDataSet
    CommandText = 
      'select CODIGO, NOME from PLANO plano1 where EXISTS (SELECT CONTA' +
      ' FROM PLANO plano2 WHERE plano2.NOME = :PNOME AND plano2.CONTA =' +
      ' plano1.CONTAPAI)'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'PNOME'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Left = 560
    Top = 408
  end
  object dsp_desp: TDataSetProvider
    DataSet = sds_desp
    Left = 592
    Top = 408
  end
  object cds_desp: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'PNOME'
        ParamType = ptInput
      end>
    ProviderName = 'dsp_desp'
    Left = 624
    Top = 408
    object cds_despCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Required = True
    end
    object cds_despNOME: TStringField
      FieldName = 'NOME'
      Required = True
      Size = 200
    end
  end
  object sds: TSQLDataSet
    CommandText = 
      'select CODIGO, CODREDUZIDO, NOME, CONTA from PLANO where CODIGO ' +
      '= :PCOD'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'PCOD'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Left = 656
    Top = 408
    object sdsCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Required = True
    end
    object sdsCODREDUZIDO: TStringField
      FieldName = 'CODREDUZIDO'
      Size = 15
    end
    object sdsNOME: TStringField
      FieldName = 'NOME'
      Required = True
      Size = 200
    end
    object sdsCONTA: TStringField
      FieldName = 'CONTA'
      Required = True
      Size = 15
    end
  end
  object ds_contas: TDataSource
    DataSet = cds_desp
    OnStateChange = DtSrcStateChange
    Left = 552
    Top = 27
  end
  object ds_ccusto: TDataSource
    DataSet = DM.cds_ccusto
    OnStateChange = DtSrcStateChange
    Left = 584
    Top = 27
  end
  object d_3_caixa_banco: TDataSource
    DataSet = DM.cds_7_contas
    OnStateChange = DtSrcStateChange
    Left = 624
    Top = 35
  end
  object DataSource1: TDataSource
    DataSet = DM.cds_ccusto
    Left = 662
    Top = 35
  end
  object s_4: TSQLDataSet
    CommandText = 
      'select pag.*, cli.RA ,cli.nomecliente '#13#10'from PAGAMENTO pag '#13#10'inn' +
      'er join CLIENTES cli on cli.CODCLIENTE = pag.CODCLIENTE'#13#10'where p' +
      'ag.codfornecedor = :pforn and  pag.TITULO = :ptitulo'#13#10
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'pforn'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'ptitulo'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Left = 56
    Top = 376
    object s_4CODPAGAMENTO: TIntegerField
      FieldName = 'CODPAGAMENTO'
      Required = True
    end
    object s_4TITULO: TStringField
      FieldName = 'TITULO'
      Required = True
      Size = 18
    end
    object s_4EMISSAO: TDateField
      FieldName = 'EMISSAO'
    end
    object s_4CODFORNECEDOR: TIntegerField
      FieldName = 'CODFORNECEDOR'
      Required = True
    end
    object s_4DATAVENCIMENTO: TDateField
      FieldName = 'DATAVENCIMENTO'
    end
    object s_4DATAPAGAMENTO: TDateField
      FieldName = 'DATAPAGAMENTO'
    end
    object s_4CAIXA: TSmallintField
      FieldName = 'CAIXA'
    end
    object s_4CONTADEBITO: TIntegerField
      FieldName = 'CONTADEBITO'
    end
    object s_4CONTACREDITO: TIntegerField
      FieldName = 'CONTACREDITO'
    end
    object s_4STATUS: TStringField
      FieldName = 'STATUS'
      Required = True
      FixedChar = True
      Size = 2
    end
    object s_4FORMAPAGAMENTO: TStringField
      FieldName = 'FORMAPAGAMENTO'
      Required = True
      FixedChar = True
      Size = 1
    end
    object s_4DATABAIXA: TDateField
      FieldName = 'DATABAIXA'
    end
    object s_4HISTORICO: TStringField
      FieldName = 'HISTORICO'
      Size = 150
    end
    object s_4CODCOMPRA: TIntegerField
      FieldName = 'CODCOMPRA'
    end
    object s_4CODALMOXARIFADO: TSmallintField
      FieldName = 'CODALMOXARIFADO'
    end
    object s_4CODCOMPRADOR: TSmallintField
      FieldName = 'CODCOMPRADOR'
    end
    object s_4CODUSUARIO: TSmallintField
      FieldName = 'CODUSUARIO'
    end
    object s_4N_DOCUMENTO: TStringField
      FieldName = 'N_DOCUMENTO'
    end
    object s_4DATASISTEMA: TSQLTimeStampField
      FieldName = 'DATASISTEMA'
    end
    object s_4VALORRECEBIDO: TFloatField
      FieldName = 'VALORRECEBIDO'
    end
    object s_4JUROS: TFloatField
      FieldName = 'JUROS'
    end
    object s_4DESCONTO: TFloatField
      FieldName = 'DESCONTO'
    end
    object s_4PERDA: TFloatField
      FieldName = 'PERDA'
    end
    object s_4TROCA: TFloatField
      FieldName = 'TROCA'
    end
    object s_4FUNRURAL: TFloatField
      FieldName = 'FUNRURAL'
    end
    object s_4VALOR_PRIM_VIA: TFloatField
      FieldName = 'VALOR_PRIM_VIA'
    end
    object s_4VALOR_RESTO: TFloatField
      FieldName = 'VALOR_RESTO'
    end
    object s_4VALORTITULO: TFloatField
      FieldName = 'VALORTITULO'
    end
    object s_4OUTRO_CREDITO: TFloatField
      FieldName = 'OUTRO_CREDITO'
    end
    object s_4OUTRO_DEBITO: TFloatField
      FieldName = 'OUTRO_DEBITO'
    end
    object s_4PARCELAS: TIntegerField
      FieldName = 'PARCELAS'
    end
    object s_4VIA: TStringField
      FieldName = 'VIA'
      FixedChar = True
      Size = 4
    end
    object s_4CODCLIENTE: TIntegerField
      FieldName = 'CODCLIENTE'
    end
    object s_4RA: TStringField
      FieldName = 'RA'
      Size = 10
    end
    object s_4NOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      Required = True
      Size = 50
    end
  end
  object d_4: TDataSetProvider
    DataSet = s_4
    UpdateMode = upWhereKeyOnly
    Left = 112
    Top = 376
  end
  object cds_4_pagar: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Name = 'pforn'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'ptitulo'
        ParamType = ptInput
      end>
    ProviderName = 'd_4'
    Left = 176
    Top = 376
    object cds_4_pagarCODPAGAMENTO: TIntegerField
      FieldName = 'CODPAGAMENTO'
      Required = True
    end
    object cds_4_pagarTITULO: TStringField
      FieldName = 'TITULO'
      Required = True
      Size = 18
    end
    object cds_4_pagarEMISSAO: TDateField
      FieldName = 'EMISSAO'
    end
    object cds_4_pagarCODFORNECEDOR: TIntegerField
      FieldName = 'CODFORNECEDOR'
      Required = True
    end
    object cds_4_pagarDATAVENCIMENTO: TDateField
      FieldName = 'DATAVENCIMENTO'
    end
    object cds_4_pagarDATAPAGAMENTO: TDateField
      FieldName = 'DATAPAGAMENTO'
    end
    object cds_4_pagarCAIXA: TSmallintField
      FieldName = 'CAIXA'
    end
    object cds_4_pagarCONTADEBITO: TIntegerField
      FieldName = 'CONTADEBITO'
    end
    object cds_4_pagarCONTACREDITO: TIntegerField
      FieldName = 'CONTACREDITO'
    end
    object cds_4_pagarSTATUS: TStringField
      FieldName = 'STATUS'
      Required = True
      OnGetText = cds_4_pagarSTATUSGetText
      FixedChar = True
      Size = 2
    end
    object cds_4_pagarFORMAPAGAMENTO: TStringField
      FieldName = 'FORMAPAGAMENTO'
      Required = True
      OnGetText = cds_4_pagarFORMAPAGAMENTOGetText
      FixedChar = True
      Size = 1
    end
    object cds_4_pagarDATABAIXA: TDateField
      FieldName = 'DATABAIXA'
    end
    object cds_4_pagarHISTORICO: TStringField
      FieldName = 'HISTORICO'
      Size = 150
    end
    object cds_4_pagarCODCOMPRA: TIntegerField
      FieldName = 'CODCOMPRA'
    end
    object cds_4_pagarCODALMOXARIFADO: TSmallintField
      FieldName = 'CODALMOXARIFADO'
    end
    object cds_4_pagarCODCOMPRADOR: TSmallintField
      FieldName = 'CODCOMPRADOR'
    end
    object cds_4_pagarCODUSUARIO: TSmallintField
      FieldName = 'CODUSUARIO'
    end
    object cds_4_pagarN_DOCUMENTO: TStringField
      FieldName = 'N_DOCUMENTO'
    end
    object cds_4_pagarDATASISTEMA: TSQLTimeStampField
      FieldName = 'DATASISTEMA'
    end
    object cds_4_pagarVALORRECEBIDO: TFloatField
      FieldName = 'VALORRECEBIDO'
      DisplayFormat = ',#0.00'
    end
    object cds_4_pagarJUROS: TFloatField
      FieldName = 'JUROS'
      DisplayFormat = ',#0.00'
    end
    object cds_4_pagarDESCONTO: TFloatField
      FieldName = 'DESCONTO'
      DisplayFormat = ',#0.00'
    end
    object cds_4_pagarPERDA: TFloatField
      FieldName = 'PERDA'
      DisplayFormat = ',#0.00'
    end
    object cds_4_pagarTROCA: TFloatField
      FieldName = 'TROCA'
      DisplayFormat = ',#0.00'
    end
    object cds_4_pagarFUNRURAL: TFloatField
      FieldName = 'FUNRURAL'
      DisplayFormat = ',#0.00'
    end
    object cds_4_pagarVALOR_PRIM_VIA: TFloatField
      FieldName = 'VALOR_PRIM_VIA'
      DisplayFormat = ',#0.00'
    end
    object cds_4_pagarVALOR_RESTO: TFloatField
      FieldName = 'VALOR_RESTO'
      DisplayFormat = ',#0.00'
    end
    object cds_4_pagarVALORTITULO: TFloatField
      FieldName = 'VALORTITULO'
      DisplayFormat = ',#0.00'
    end
    object cds_4_pagarOUTRO_CREDITO: TFloatField
      FieldName = 'OUTRO_CREDITO'
      DisplayFormat = ',#0.00'
    end
    object cds_4_pagarOUTRO_DEBITO: TFloatField
      FieldName = 'OUTRO_DEBITO'
      DisplayFormat = ',#0.00'
    end
    object cds_4_pagarPARCELAS: TIntegerField
      FieldName = 'PARCELAS'
    end
    object cds_4_pagarVIA: TStringField
      FieldName = 'VIA'
      FixedChar = True
      Size = 4
    end
    object cds_4_pagarCODCLIENTE: TIntegerField
      FieldName = 'CODCLIENTE'
    end
    object cds_4_pagarRA: TStringField
      FieldName = 'RA'
      Size = 10
    end
    object cds_4_pagarNOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      Required = True
      Size = 50
    end
  end
  object DataSource2: TDataSource
    DataSet = cds_4_pagar
    Left = 240
    Top = 376
  end
  object sds_for: TSQLDataSet
    CommandText = 
      'select CODFORNECEDOR, NOMEFORNECEDOR from FORNECEDOR '#13#10'where COD' +
      'FORNECEDOR = :pcod'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'pcod'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Left = 688
    Top = 408
    object sds_forCODFORNECEDOR: TIntegerField
      FieldName = 'CODFORNECEDOR'
      Required = True
    end
    object sds_forNOMEFORNECEDOR: TStringField
      FieldName = 'NOMEFORNECEDOR'
      Required = True
      Size = 50
    end
  end
  object HistoricoReceber: TUCControlHistorico
    UserControl = fFinanceiro.UserControlFinanceiro
    Options.SaveNewRecord = True
    Options.SaveDelete = True
    Options.SavePostInsert = True
    Options.SavePostEdit = True
    Options.TypeSavePostEdit = tpSaveAllFields
    TableHistory.TableName = 'UCTABHistory'
    TableHistory.FieldApplicationID = 'ApplicationID'
    TableHistory.FieldUserID = 'UserID'
    TableHistory.FieldEventDate = 'EventDate'
    TableHistory.FieldEventTime = 'EventTime'
    TableHistory.FieldForm = 'Form'
    TableHistory.FieldCaptionForm = 'FormCaption'
    TableHistory.FieldEvent = 'Event'
    TableHistory.FieldObs = 'Obs'
    TableHistory.FieldTableName = 'tName'
    HistoryMsg.Evento_Insert = 'Inserido'
    HistoryMsg.Evento_Delete = 'Apagado'
    HistoryMsg.Evento_Edit = 'Editado'
    HistoryMsg.Evento_NewRecord = 'Novo registro'
    HistoryMsg.Hist_All = 'Todos'
    HistoryMsg.Msg_LimpHistorico = 'Excluir todo o conte'#250'do do hist'#243'rico ?'
    HistoryMsg.Msg_MensConfirma = 'Confirma'#231#227'o'
    HistoryMsg.Msg_LogEmptyHistory = 'Usu'#225'rio %s apagou todo o hist'#243'rico as %s'
    HistoryMsg.LabelDescricao = 'Hist'#243'rico de tabelas'
    HistoryMsg.LabelUser = 'Usu'#225'rio'
    HistoryMsg.LabelForm = 'Formul'#225'rio'
    HistoryMsg.LabelEvento = 'Evento'
    HistoryMsg.LabelTabela = 'Tabela'
    HistoryMsg.LabelDataEvento = 'Data'
    HistoryMsg.LabelHoraEvento = 'Hora'
    HistoryMsg.Msg_NewRecord = '%s inseriu um novo registro'
    HistoryMsg.Hist_MsgExceptPropr = 'Favor informar a propriedade %s'
    HistoryMsg.Hist_BtnFiltro = '&Aplicar Filtro'
    HistoryMsg.Hist_BtnExcluir = '&Excluir Hist'#243'rico'
    HistoryMsg.Hist_BtnFechar = '&Fechar'
    Left = 744
    Top = 378
  end
  object Hist_DataSetDespesa: TUCHist_DataSet
    DataSet = cds_4_pagar
    ControlHistorico = HistoricoReceber
    Left = 752
    Top = 426
  end
  object DataSource3: TDataSource
    DataSet = cdsClienteBusca
    Left = 440
    Top = 168
  end
  object cdsClienteBusca: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DataSetProvider1'
    Left = 408
    Top = 168
    object cdsClienteBuscaCODCLIENTE: TIntegerField
      FieldName = 'CODCLIENTE'
    end
    object cdsClienteBuscaNOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      FixedChar = True
      Size = 60
    end
    object cdsClienteBuscaRA: TStringField
      FieldName = 'RA'
      Size = 10
    end
  end
  object DataSetProvider1: TDataSetProvider
    DataSet = SQLDataSet1
    Left = 368
    Top = 168
  end
  object SQLDataSet1: TSQLDataSet
    CommandText = 
      'select * from busca_br'#13#10#13#10'/*'#13#10'select CODCLIENTE,NOMECLIENTE , RA' +
      '  from CLIENTES'#13#10'     where status = 0 and GRUPO_CLIENTE = '#39'ASH'#39 +
      ' order by NOMECLIENTE */'
    MaxBlobSize = -1
    Params = <>
    SQLConnection = DM.sqlsisAdimin
    Left = 336
    Top = 168
    object SQLDataSet1CODCLIENTE: TIntegerField
      FieldName = 'CODCLIENTE'
    end
    object SQLDataSet1NOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      FixedChar = True
      Size = 60
    end
    object SQLDataSet1RA: TStringField
      FieldName = 'RA'
      Size = 10
    end
  end
  object sqlP: TSQLDataSet
    CommandText = 
      'select CODCLIENTE, NOMECLIENTE, RA from CLIENTES where '#13#10'codclie' +
      'nte = :pCodP'#13#10#13#10
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'pCodP'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Left = 312
    Top = 360
    object sqlPCODCLIENTE: TIntegerField
      FieldName = 'CODCLIENTE'
      Required = True
    end
    object sqlPNOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      Required = True
      Size = 50
    end
    object sqlPRA: TStringField
      FieldName = 'RA'
      Size = 10
    end
  end
  object dsCDSV: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'user'
        ParamType = ptInput
      end>
    ProviderName = 'DSPV'
    Left = 568
    Top = 168
    object dsCDSVNOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      Required = True
      Size = 50
    end
    object dsCDSVRA: TStringField
      FieldName = 'RA'
      Size = 10
    end
    object dsCDSVDATACADASTRO: TDateField
      FieldName = 'DATACADASTRO'
      Required = True
    end
    object dsCDSVEMVIAGEM: TStringField
      FieldName = 'EMVIAGEM'
      FixedChar = True
      Size = 1
    end
  end
  object DSPV: TDataSetProvider
    DataSet = sqlV
    Left = 536
    Top = 176
  end
  object sqlV: TSQLDataSet
    CommandText = 
      'select nomecliente , ra , datacadastro ,emviagem from clientes w' +
      'here GRUPO_CLIENTE = '#39'ASH'#39' AND emviagem ='#39'S'#39' and  status = 0 and' +
      '  ra = :user'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'user'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Left = 504
    Top = 176
    object sqlVNOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      Required = True
      Size = 50
    end
    object sqlVRA: TStringField
      FieldName = 'RA'
      Size = 10
    end
    object sqlVDATACADASTRO: TDateField
      FieldName = 'DATACADASTRO'
      Required = True
    end
    object sqlVEMVIAGEM: TStringField
      FieldName = 'EMVIAGEM'
      FixedChar = True
      Size = 1
    end
  end
  object SQLc: TSQLDataSet
    CommandText = 
      'select nomecliente , ra , datacadastro , tem_ie from clientes wh' +
      'ere GRUPO_CLIENTE = '#39'ASH'#39' AND (datacadastro + 180 ) >= (:datarec' +
      ') and  status = 0 and tem_ie = '#39'S'#39'  and ra = :user'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftDate
        Name = 'datarec'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'user'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Left = 672
    Top = 144
    object SQLcNOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      Required = True
      Size = 50
    end
    object SQLcRA: TStringField
      FieldName = 'RA'
      Size = 10
    end
    object SQLcDATACADASTRO: TDateField
      FieldName = 'DATACADASTRO'
      Required = True
    end
    object SQLcTEM_IE: TStringField
      FieldName = 'TEM_IE'
      FixedChar = True
      Size = 1
    end
  end
  object DSPc: TDataSetProvider
    DataSet = SQLc
    Left = 712
    Top = 144
  end
  object dsCDSc: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftDate
        Name = 'datarec'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'user'
        ParamType = ptInput
      end>
    ProviderName = 'DSPc'
    Left = 752
    Top = 144
    object dsCDScNOMECLIENTE: TStringField
      FieldName = 'NOMECLIENTE'
      Required = True
      Size = 50
    end
    object dsCDScRA: TStringField
      FieldName = 'RA'
      Size = 10
    end
    object dsCDScDATACADASTRO: TDateField
      FieldName = 'DATACADASTRO'
      Required = True
    end
    object dsCDScTEM_IE: TStringField
      FieldName = 'TEM_IE'
      FixedChar = True
      Size = 1
    end
  end
  object sqlSoma: TSQLDataSet
    CommandText = 
      'select   sum(VALOR_RESTO)as soma_VALOR_RESTO ,VALOR_RESTO , VALO' +
      'RTITULO '#13#10'from PAGAMENTO pag '#13#10'where pag.codfornecedor = :pforn ' +
      'and  pag.TITULO = :ptitulo'#13#10#13#10'group by    VALORTITULO ,VALOR_RES' +
      'TO'#13#10#13#10#13#10#13#10
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'pforn'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'ptitulo'
        ParamType = ptInput
      end>
    SQLConnection = DM.sqlsisAdimin
    Left = 56
    Top = 408
    object sqlSomaSOMA_VALOR_RESTO: TFloatField
      FieldName = 'SOMA_VALOR_RESTO'
      ReadOnly = True
    end
    object sqlSomaVALOR_RESTO: TFloatField
      FieldName = 'VALOR_RESTO'
      ReadOnly = True
    end
    object sqlSomaVALORTITULO: TFloatField
      FieldName = 'VALORTITULO'
      ReadOnly = True
    end
  end
  object dspSoma: TDataSetProvider
    DataSet = sqlSoma
    UpdateMode = upWhereKeyOnly
    Left = 112
    Top = 408
  end
  object cdsSoma: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Name = 'pforn'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'ptitulo'
        ParamType = ptInput
      end>
    ProviderName = 'dspSoma'
    Left = 176
    Top = 408
    object cdsSomaSOMA_VALOR_RESTO: TFloatField
      FieldName = 'SOMA_VALOR_RESTO'
      ReadOnly = True
    end
    object cdsSomaVALOR_RESTO: TFloatField
      FieldName = 'VALOR_RESTO'
      ReadOnly = True
    end
    object cdsSomaVALORTITULO: TFloatField
      FieldName = 'VALORTITULO'
      ReadOnly = True
    end
  end
end
