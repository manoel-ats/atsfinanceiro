...
begin
  ...
  begin
    try
      ...
      // Data Entrada do Reembolso = a Data de Emissão
      str1 := str1 + ',';
      if(data_reem.Text = '  /  /    ') then
        str1 := str1 + ' null'
      else
      str1 := str1 + QuotedStr(FormatDateTime('mm/dd/yy',data_reem.Date)); // DATA ENTRADA DO REEMBOLSO
    ...
    try
      ...
      cdsTitular.Open;
     {
      ...
      // Valor , Produto , Socio , Fornecedor ,Historico , Data Recibo , Data Reembolso , Valor Recibo
    ...
    begin
      try
          ...
          dm.sqlsisAdimin.Commit(TD);
        {
          DecimalSeparator := '.';
          dm.sqlsisAdimin.StartTransaction(TD);
          str1 := 'UPDATE RECEBIMENTO set VALOR_PRIM_VIA = VALOR_PRIM_VIA - ' + FloatToStr(JvCalcEdit2.Value) +
          //  ', desconto = desconto + ' + FloatToStr(JvCalcEdit1.Value) +
            ' where CODRECEBIMENTO = ' + IntToStr(codReceb);
          DecimalSeparator := ',';
          dm.sqlsisAdimin.ExecuteDirect(str1);
...
procedure TfLancaProcSaude.btntrocarClick(Sender: TObject);
 ...
 // inherited;
  dmSaude.cdsFornecedor.Locate('NOMEFORNECEDOR', JvDBSearchEdit6.Text, [loCaseInsensitive]);
      if (not cds.Active) then
         cds.Open;
      cds.Locate('PRODUTO', JvDBSearchEdit4.Text, [loCaseInsensitive]);
      str1 := 'UPDATE MOVIMENTODETALHE SET CODPRODUTO = 745 '+
        {745 +
        ' ,CODPRODUTO = ' + IntToStr(cdsCODPRODUTO.asInteger) +
        ' ,CONTROLE = ' + IntToStr(sql_dependenteCODCLIENTE.asInteger) +
        ' ,COD_COMISSAO = ' + IntToStr(dmSaude.cdsFornecedorCODFORNECEDOR.asInteger) +
    ...
    MessageDlg('Alterado Para Mensalidade Anteceipada', mtInformation, [mbOK], 0);
...
procedure TfLancaProcSaude.btnSairClick(Sender: TObject);
begin
  inherited;
  btntrocar.Enabled := True;
end;

procedure TfLancaProcSaude.btnDataReembolsoClick(Sender: TObject);
begin
//  inherited;
