...
begin
  ...
  begin
    if sqlTexto='' then
      sqlTexto := sqlTexto + ' where comp.SERIE is null '
    else
      sqlTexto := sqlTexto + ' and comp.SERIE is null ';
...
begin
   ...
   begin
     ...
     // NOVO  fCompra.btnProdutoProcura.Enabled := true;
   end
   else
   begin
   ...
   fCompra.Label8.Caption := IntToStr(fCompra.cds_Mov_det.RecordCount);
...
begin
  ...
  sql := 'select prod.CODPRO, prod.PRODUTO, prod.VALORUNITARIOATUAL , ' +
    ...
    ' , it.CONTROLE ,cli.ra , cli.nomecliente ,it.CODDETALHE' +
    ' from MOVIMENTO mov ' +
    ' left outer join COMPRA comp on comp.codmovimento = mov.codmovimento ' +
    ...
    ' inner join clientes cli on cli.codcliente = it.controle' +
  ...
  VCLReport1.Report.DataInfo.Items[1].SQL := sql + sqlTexto +
    ...
    ' mov.STATUS, mov.CODNATUREZA, mov.CODALMOXARIFADO, mov.CONTROLE ' +
    ' , it.CONTROLE ,cli.ra , cli.nomecliente, it.CODDETALHE' +
    ' order by it.CODDETALHE, forn.NOMEFORNECEDOR, mov.DATAMOVIMENTO ';
  VCLReport1.Execute;
end;
