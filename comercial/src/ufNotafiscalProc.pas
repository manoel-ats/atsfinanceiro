...
begin
  if (CheckBox1.Checked = False) then
  begin
  str_sql := 'select nf.IMPRESSA, nf.NOTASERIE, nf.DTAEMISSAO, nf.DTASAIDA, nf.UF, ' +
    ' nf.NUMNF, nf.NOTAFISCAL, nf.VALOR_TOTAL_NOTA, nf.codCliente, ' +
    ' cli.NOMECLIENTE as CLIFOR, nf.NATUREZA from NOTAFISCAL nf ' +
    ' left outer join CLIENTES cli on  cli.CODCLIENTE = nf.CODCLIENTE ' +
  ...
  begin
    ...
    ' left outer join FORNECEDOR forn on  forn.CODFORNECEDOR = nf.CODCLIENTE' +
    ' left outer join VENDA v on v.codVenda = nf.codVenda ' +
    ' left outer join COMPRA c on c.codCompra = nf.codVenda' ;
  end;
  if (edit1.Text <> '') then
