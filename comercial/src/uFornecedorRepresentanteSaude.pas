unit uFornecedorRepresentanteSaude;
...
uses
  ...
  FMTBcd, DBCtrls, Mask, DBClient, Provider, SqlExpr, DBLocal, DBLocalS,
  Grids, DBGrids, uPai, XPMenu;

type
  TfFornecedorRepresentanteSaude = class(TfPai)
    ...
    DBText2: TDBText;
    BitBtn1: TBitBtn;
    procCBO: TSQLClientDataSet;
    procCBONM_CBO_SAUDE: TStringField;
    procCBOCD_CBO_SAUDE: TStringField;
    procCBONM_CONSELHO_PROF: TStringField;
    DBGrid1: TDBGrid;
    Label16: TLabel;
    DBEdit14: TDBEdit;
    DBEdit15: TDBEdit;
...
var
  fFornecedorRepresentanteSaude: TfFornecedorRepresentanteSaude;
...
procedure TfFornecedorRepresentanteSaude.cds_repforNewRecord(DataSet: TDataSet);
begin
  inherited;
  cds_repforCOD_FORNECEDOR.AsInteger :=
   fFornecedorCadastroSaude.cds_fornecedorCODFORNECEDOR.AsInteger;
end;

procedure TfFornecedorRepresentanteSaude.FormShow(Sender: TObject);
begin
  inherited;
  if cds_repfor.Active then
     cds_repfor.Close;
  cds_repfor.Params[0].AsInteger:=fFornecedorCadastroSaude.cds_fornecedorCODFORNECEDOR.AsInteger;
  cds_repfor.Open;
  btnIncluir.SetFocus;
end;

procedure TfFornecedorRepresentanteSaude.DtSrcStateChange(Sender: TObject);
begin
  Incluir := 'S';
  Alterar := 'S';
  Excluir := 'S';
...
procedure TfFornecedorRepresentanteSaude.btnGravarClick(Sender: TObject);
begin
    ...
    dm.c_6_genid.Close;
  cds_repforFUNCAO.AsString := cod_cbo;   
  inherited;

end;

procedure TfFornecedorRepresentanteSaude.btnIncluirClick(Sender: TObject);
begin
  inherited;
  DBEdit2.SetFocus;
end;

procedure TfFornecedorRepresentanteSaude.BitBtn1Click(Sender: TObject);
begin
  inherited;
  fProcurar:= TfProcurar.Create(self,procCBO);
  try
   fProcurar.BtnProcurar.Click;
   fProcurar.EvDBFind1.DataField := 'NM_CBO_SAUDE';
   if fProcurar.ShowModal=mrOk then
   begin
     if(DtSrc.State in [dsbrowse,dsinactive]) then
       cds_repfor.Edit;  
     DBEdit1.Text := procCBONM_CBO_SAUDE.AsString;
     cod_cbo := procCBOCD_CBO_SAUDE.AsString;
   end;
   finally
    procCBO.Close;
    fProcurar.Free;
...
procedure TfFornecedorRepresentanteSaude.FormCreate(Sender: TObject);
begin
//  inherited;
