CREATE OR ALTER PROCEDURE AUDITORIA_C_P (
    dia date,
    dataini date,
    datafin date,
    codalmox integer)
...
BEGIN
  ...
  --  sum(rec.VALORRECEBIDO + CASE when FUNRURAL > 0 then FUNRURAL else 0 end + CASE when JUROS > 0 then JUROS else 0 end) as VALORRECEBIDO,
    ...
    group by  rec.DATAVENCIMENTO, cli.NOMEFORNECEDOR,  rec.CODFORNECEDOR,rec.CAIXA , rec.formapagamento , rec.EMISSAO,   rec.TITULO,
    rec.STATUS, rec.DATAPAGAMENTO,rec.N_DOCUMENTO,rec.DATACONSOLIDA ,rec.DATAINCLUSAO
    /*, rec.VALORRECEBIDO , rec.DESCONTO , rec.PERDA  , plano.NOME  */
    INTO :TITULO,:EMISSAO,:DATACONSOLIDA,:CAIXA,:FORMAPAGAMENTO,:DATAVENCIMENTO,:CODFORNECEDOR,:VALORTITULO,
         ...
         :VALORRECEBIDO,:NOMEFORNECEDOR,:VALORREC , :PERDA /*,:NOME */
