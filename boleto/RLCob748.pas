...
begin
   ...
   With ATitulo do begin
      //Código numérico correspondente ao Tipo de Cobrança: "3" - sem registro e 1 com Registro BANSICREDI
      ATipoCobranca:='1'; //Fixo
      //ATipoCobranca:= Formatar(Cedente.ContaBancaria.fConvenio,1,False,'0');  // 26/02/2015 campo convenio tab banco

      //Código numérico correspondente ao Tipo de Carteira: "1" - Carteira Simples
      ATipoCarteira:='1'; //Fixo
      //ATipoCarteira:= Formatar(Carteira,1,False,'0');   // 26/02/2015  campo carteira tb banco
      //Nosso número
...
procedure TRLBanco748.FormatarBoleto(ATitulo: TRLBTitulo;
 var AAgenciaCodigoCedente,ANOSSONUMERO, ACarteira, AEspecieDocumento: string);
begin
   With ATitulo do begin
      ...
      ANossoNumero2 := FormatDateTime('yy',Date) + '2' + Formatar(NossoNumero,5,False,'0') + DigitoNossoNumero;
...
begin
   ...
   begin
      ...
      { GERAR REGISTRO-HEADER DA REMESSA }
                    ...
                    // COM REGISTRO   MANOEL 18/01/2015
      ...
      Remessa.Add('0'+ // IDENT.DO REGISTRO
                 ...
                 // Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,15,true,' ')+ // IDENT.DO CEDENTE NO BANCO
                  ...
                  Formatar(Titulos[NumeroRegistro].Cedente.NumeroCPFCGC,14,false,'0')+ // CGC/CPF DO SACADO
                  {
                  ...
                  Formatar('SICREDI',15,true,' ')+ // NOME DO BANCO POR EXTENSO
                  FormatDateTime('YYYYMMDD',Today )+ // DATA DA GRAVAÇÃO DO ARQUIVO
                  //FormatDateTime('YYYYMMDD',Titulos[NumeroRegistro].DataProcessamento)+ // DATA DA GRAVAÇÃO DO ARQUIVO
...
//            toRemessaAlterarSeuNumero          : ATitulosTipoOcorrencia := '08';
        ...
        case Titulos[NumeroRegistro].AceiteDocumento of
         ...
         Registro := Registro + 'A';
         Registro := Registro + 'A';
         Registro := Registro + '            ' ; // 12 BRANCOS
         Registro := Registro + 'A';
         Registro := Registro + 'A';
         Registro := Registro + 'A';
         Registro := Registro + '    '; // 4 BRANCOS
         Registro := Registro + '    '; // 4 BRANCOS
         Registro := Registro + '    '; // 4 BRANCOS
         Registro := Registro + '                '; // 16 BRANCOS


        // Registro := Registro + Formatar(aNossoNumero2,3,true,' ');

         Registro := Registro + Formatar(Titulos[NumeroRegistro].NossoNumero,8,False,'0'); /// é ´ra ser 9 so que ai coloca um ZERO e fica errado

         Registro := Registro + Formatar(Titulos[NumeroRegistro].DigitoNossoNumero,1,False,'0');

         Registro := Registro + '      '; // 6 BRANCOS

         ///   Registro := Registro + Formatar(Titulos[NumeroRegistro].NossoNumero,15,true,'0');
         Registro := Registro + FormatDateTime('YYYYMMDD',Titulos[NumeroRegistro].DataVencimento); // data instrução
         Registro := Registro + ' '; // 1 BRANCO
         Registro := Registro + 'N'; //
         Registro := Registro + ' '; //
         Registro := Registro + 'B'; // impressão pelo Cedente
         Registro := Registro + '        '; // 8 BRANCOS
         Registro := Registro + '0000000000';
         Registro := Registro + '0200';  // JUROS 2%  93 a 96
         Registro := Registro + '            '; // 12 BRANCOS  97 a 108
         Registro := Registro + '01'; //
         Registro := Registro + Formatar(Titulos[NumeroRegistro].NossoNumero,10,true,' ');
         ...
         Registro := Registro + FormatDateTime('DDMMYY',Titulos[NumeroRegistro].DataVencimento); // 151 a 156
         Registro := Registro + FormatCurr('0000000000000',Titulos[NumeroRegistro].ValorDocumento * 100); // VALOR NOMINAL DO TITULO
         Registro := Registro + '         '; // 9 BRANCOS
         Registro := Registro + 'A';
         Registro := Registro + 'N';
         ...
         Registro := Registro + FormatDateTime('ddmmyy',Titulos[NumeroRegistro].DataDocumento); // DATA DE EMISSAO DO TITULO
         Registro := Registro + '00'; // 06 protesto atomatico , 00 Não protestar automaticamente.
         Registro := Registro + '30'; //Apos 30 dias protestar .(minimo 05 dias)  // 159 a 160
         //  tava assim
         //Registro := Registro + '0000000000200'; // 2% juros  161 a 173
         Registro := Registro + '0000000000004'; // 2% juros  161 a 173
         {
         ...
         Registro := Registro + '000000'; // 6 ZEROS   174 a 179  Data limite p/ Concessão de desconto  Informar data no padrão:DDMMAA ou preencher com zeros
        ...
        // Registro := Registro + 'X';
         ...
         Registro := Registro + Formatar(ASacadoTipoInscricao,1,false,'0'); // IDENT. TIPO INSCRIÇÃO DO SACADO

         Registro := Registro + '0'; // 220 a 200 um campo 0 (Zero)
...
//         Registro := Registro + '<>';
         Registro := Registro + Formatar(Titulos[NumeroRegistro].Sacado.Endereco.Rua+
                                ...
                                ' '+Titulos[NumeroRegistro].Sacado.Endereco.Complemento,40); // ENDEREÇO DO SACADO 275/314
         Registro := Registro + '00000';  //  315/319   Codigo do sacado na cooperativa cedente .
         // ficou a linha de cima  Campo deve ser preenchido com zeros.
         //  Registro := Registro + Formatar(IntToStr(Titulos[NumeroRegistro].Sacado.Codigo),5,False,'0'); // Codigo sacado 315/319
         Registro := Registro + '000000'; //  320/325   Zeros
         Registro := Registro + Formatar('',1); // USO DO BANCO
         Registro := Registro + Formatar(Titulos[NumeroRegistro].Sacado.Endereco.CEP,8,true,'0'); // CEP DO SACADO  327/334
          // manoel 03/01/17
       //  Registro := Registro + Formatar(IntToStr(Titulos[NumeroRegistro].Sacado.Codigo),5,False,'0'); // Codigo sacado 335/339
         Registro := Registro + '00000';
         Registro := Registro + Formatar('',26); // USO DO BANCO


         Registro := Registro + Formatar('',29); // USO DO BANCO
