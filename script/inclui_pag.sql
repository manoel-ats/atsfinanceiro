...
begin
   ...
   begin
     ...
     VALUES 
       ...
       cast('NOW' as date), :VLR_PRIM_VIA, :VLR_RESTO, :VLR_TITULO, NEW.ENTRADA, NEW.MULTA_JUROS, 0, 0, 
       0, NEW.DATAVENCIMENTO, NEW.N_PARCELA,0, new.CAIXA, NEW.N_DOCUMENTO, new.DATAVENCIMENTO);
     i = 1;
   ...
   begin
     ...
     VALUES 
       (NEW.NOTAFISCAL, NEW.DATACOMPRA, NEW.CODFORNECEDOR, :dtaVenc,
       :status_venda, CAST((:i + 1) as CHAR(3)), :forma, NEW.CODCOMPRA, NEW.CODCCUSTO, NEW.CODCOMPRADOR, NEW.CODUSUARIO,  
       cast('NOW' as date), :VLR_PRIM_VIA, :VLR_RESTO, :VLR_TITULO, NEW.MULTA_JUROS, 0, 0,0,0, NEW.N_PARCELA,0, :CAIXA, :DTAPAG, NEW.N_DOCUMENTO);
     i = i + 1;    
   end
