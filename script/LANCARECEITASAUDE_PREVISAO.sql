...
BEGIN
  ...
  INSERT INTO movimentoDetalhe (codDetalhe, codMovimento, codProduto, controle, quantidade, preco, cod_comissao, lote , historico,DTAVCTO ,PRECOCUSTO ,DTAFAB  )
    Values(gen_id(genmovdet, 1), :codMov, :contaDebito, :clienteProc, 1, :valor, :FORNECEDOR, :HISTORICO , :HIST ,:datasistema ,:outras_desp,:DATA_ENT_REE );
  Insert into venda(codVenda, codMovimento, codCliente, dataVenda, dataVencimento, notaFiscal, serie, n_parcela
      , valor, status, entrada, desconto, multa_juros, obs, codccusto , n_boleto , datasistema ,outras_desp,DATA_ENT_REE  )
    Values(gen_id(genvenda, 1), :codMov, :cliente, :emissao, :dtvcto, :cliente, :mesAno, 1
      , :vlrReceber, 0, 0, :VLRREEMBOLSOCLI, 0, :HISTORICO, :codccusto , :n_boleto ,:datasistema  , :outras_desp ,:DATA_ENT_REE );
