...
begin
  valorA = 0;
    ...
    for select valorrecebido+juros, formaRecebimento, n_documento, contaCredito, dataRecebimento , Juros from RECEBIMENTO
    ...
    do begin
      ...
      if (formaR = '1') then 
        ...
        formaRec = 'Cartão Crédito';
      else if (formaR = '7') then 
        formaRec = 'Cartão Débito';
      else if (formaR = '8') then 
        formaRec = 'Depósito';
      else if (formaR = '9') then 
        formaRec = 'Débito Automático';
  ...
  for select valorrecebido+juros, codRecebimento, formaRecebimento, n_documento , juros from RECEBIMENTO
     where dataRecebimento BETWEEN :dataIni and :dataFin 
     and ((caixa = :pcaixa) or (:pcaixa = 0))
     and ((FormaRecebimento = :pForma) or (:pForma = '0'))
