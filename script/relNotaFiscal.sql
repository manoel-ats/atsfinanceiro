...
begin
  ...
  begin 
    ...
    for select nf.notaserie, nf.dtaemissao, nf.valor_total_nota, cli.razaosocial, 
       nf.valor_icms  ,nf.NUMNF, nf.PLACATRANSP, nf.NOMETRANSP, VALOR_ICMS_SUBST
       from notafiscal nf 
    ...
    where ((nf.NATUREZA = 3) or (nf.NATUREZA = 15) or (nf.NATUREZA = 12)) and dtaemissao = :data1
    and ((nf.CODCLIENTE = :CodCliFor) OR (:CODCLIFOR = 0)) and ((v.SERIE = :SERIE) 
     or (:SERIE = 'Z'))
    order by nf.dtaemissao, nf.notaserie 
    into :nunNota, :dataEmissao, :valorNotaSai, :fornCliente, :valorIcmsSai, :nf, :placa, :motorista, :ValorIcmsSubst
    do begin 
