CREATE OR ALTER PROCEDURE LANCARECEITASAUDE_MENSALIDADE (
    emissao date,
    dtvcto date,
    cliente integer,
    clienteproc integer,
...
BEGIN
 ...
 /*
  ...
  INSERT INTO movimentoDetalhe (codDetalhe, codMovimento, codProduto, controle, quantidade, preco, cod_comissao, lote ,DTAVCTO ,PRECOCUSTO ,DTAFAB  )
    Values(gen_id(genmovdet, 1), :codMov, :contaDebito, :clienteProc, 1, :valor, :FORNECEDOR, :HISTORICO ,:datasistema ,:outras_desp,:DATA_ENT_REE );
  */
  if(codpag > 0) then
  codmov = codpag ;
