CREATE OR ALTER PROCEDURE BOLETO (
    banco varchar(18),
...
BEGIN
  ...
  For Select cli.ra,  rec.CODCLIENTE, rec.EMISSAO, rec.VALOR_RESTO,rec.DATAVENCIMENTO , udf_digits(rec.TITULO),rec.CODVENDA , rec.CODRECEBIMENTO
  FROM RECEBIMENTO rec inner join clientes cli on rec.codcliente = cli.codcliente where rec.DP = 1
  and ((cli.geradebitoau is null)or (cli.geradebitoau = ''))
  order by cli.ra ,cli.CODCLIENTE
  into :rarec ,:codCli, :DATADOC, :VALOR ,:DATAREC ,:NUMTITULO ,:CODVENDA , :CODRECEBIMENTO
  ...
  do begin 
    ...
    from CLIENTES c 
      where c.CODCLIENTE = :codCli   ORDER BY C.CODCLIENTE
