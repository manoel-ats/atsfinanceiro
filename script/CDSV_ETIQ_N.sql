CREATE OR ALTER PROCEDURE CDSV_ETIQ_N 
returns (
    ra varchar(10),
...
BEGIN
  ...
  codr = 0 ;
  FOR
    select distinct cli.razaosocial, ende.complemento , cli.ra , ende.logradouro,
    ende.bairro,ende.cidade,ende.uf, ende.cep, ende.ramal
    ...
    where rec.datarecebimento between '01/01/14' and '12/31/14'  /*:dta1 and :dta2 /* AND ((movd.codproduto = 744) or (movd.codproduto = 745)) * / */
     /* and cli.razaosocial = :titular */
      ...
      and cli.segmento = 0
      and ende.complemento LIKE 'VAREJÃO%'
      and clientes.gerabol = 'N'
   order by  movd.controle,  cli.nomecliente ,  rec.datavencimento, rec.codrecebimento

       INTO :razaosocial ,:complemento , :ra , :logradouro ,:bairro ,:cidade ,:uf ,:cep ,:ramal
  DO
  BEGIN
