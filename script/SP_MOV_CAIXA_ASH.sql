CREATE PROCEDURE SP_MOV_CAIXA_ASH (
    dtaini date,
    dtafim date,
    cod_caixa smallint)
returns (
...
BEGIN
  ...
  select first 1 valor from caixa_fechado where caixa_fechado.data_fim = udf_incday(:dtaini, -1) and caixa_fechado.cod_caixa = :cod_caixa
  order by valor desc
